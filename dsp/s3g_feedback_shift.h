    ...
    FeedbackPedalType type, uint32_t control)
{
    ...
    case FeedbackPedalType::Repeater: {
        static constexpr const char* names[4u] {
            "WINDOW", "REPEATS", "DECAY", "MIX" };
        return names[control];
    }
    case FeedbackPedalType::TimeMangler: {
        static constexpr const char* names[4u] {
            "WINDOW", "PITCH", "MODE", "MIX" };
        return names[control];
    }
...
inline uint32_t feedbackPedalControlCount(FeedbackPedalType type)
{
    ...
    case FeedbackPedalType::Repeater: return 5u;
    case FeedbackPedalType::TimeMangler: return 5u;
...
// The CLAP parameter remains expressed in Hz. These helpers define only the
// performance slider taper: sub-Hz and low-Hz gestures receive most of the
// physical travel, while either edge still reaches +/-6000 Hz.
inline float feedbackShiftFrequencyFromControlNorm(float normalized)
{
    ...
    constexpr float base = 1000.0f;
    const float distance = std::abs(signedDistance);
    const float hertz = 6000.0f
        * (std::pow(base, distance) - 1.0f) / (base - 1.0f);
...
inline float feedbackShiftFrequencyControlNorm(float frequencyHz)
{
    ...
    constexpr float base = 1000.0f;
    const float distance = std::log(1.0f
        + std::abs(frequencyHz) * (base - 1.0f) / 6000.0f)
        / std::log(base);
...
public:
    ...
    void processFrame(float* output) noexcept
    {
        ...
        for (uint32_t destination = 0u;
            ...
            const float burst = burstEnvelope_[destination]
                * burstEnvelope_[destination] * 0.44f;
            const float excitation = (noise - state.noiseLowpass)
                * (exciteGain_ + burst) * runGain_;
            ...
            matrixInput[destination] = std::clamp(sum * normalization
                * smoothed_.nodes[destination].regeneration
        ...
        for (uint32_t node = 0u; node < kFeedbackShiftChannels; ++node) {
            ...
            const auto& params = smoothed_.nodes[node];
...
private:
    ...
    float processTemporal(NodeState& state, float input,
        const FeedbackShiftNodeParams& params, bool mangler) noexcept
    {
        ...
        const bool onset = temporalOnset(state, input,
            0.35f + params.pedalTone * 0.55f);
        ...
        if (state.temporalPhase == 1u) {
            state.temporalBuffer[state.temporalPosition++] = input;
            if (state.temporalPosition >= state.temporalFrames) {
                ...
                state.temporalRead = mangler && mode == 0u
        ...
        const uint32_t edge = std::min<uint32_t>(64u,
            std::max<uint32_t>(2u, state.temporalFrames / 8u));
        const uint32_t frameInLoop = state.temporalPlayback
            % state.temporalFrames;
        const float fadeIn = std::min(1.0f,
            static_cast<float>(frameInLoop + 1u) / edge);
        const float fadeOut = std::min(1.0f,
            static_cast<float>(state.temporalFrames - frameInLoop) / edge);
        wet = input + (wet - input) * std::min(fadeIn, fadeOut);

        if (!mangler) {
            const uint32_t repeat = state.temporalPlayback
                / state.temporalFrames;
            const float decay = (params.pedalBias + 1.0f) * 0.5f;
            wet *= std::pow(std::max(0.08f, 1.0f - decay * 0.16f),
                static_cast<float>(repeat));
            const uint32_t direction = static_cast<uint32_t>(std::lround(
                params.pedalExtra[0u] * 2.0f));
            const bool reverse = direction == 1u
                || (direction == 2u && (repeat & 1u) != 0u);
            state.temporalRead += reverse ? -1.0 : 1.0;
            ++state.temporalPlayback;
            if (state.temporalPlayback >= state.temporalFrames
                    * state.temporalRepeats) {
                state.temporalPhase = 0u;
            }
        } else {
        ...
        const FeedbackShiftNodeParams& params) noexcept
    {
        ...
        case FeedbackPedalType::Repeater:
            wet = processTemporal(state, input, params, false);
            break;
        case FeedbackPedalType::TimeMangler:
            wet = processTemporal(state, input, params, true);
            break;
        case FeedbackPedalType::DrumEcho: {
            DrumEchoParams echo;
