...
public:
    ...
    float process(uint32_t source, float sample, float distance)
    {
        ...
        if (!primed_[source]) {
            prevDistance_[source] = distance;
            smoothedDelay_[source] = targetDelaySamples(distance);
            primed_[source] = true;
        }

        float out = sample;
        if (params_.doppler > 0.0001f) {
            out = processDoppler(source, sample, distance);
        }

        if (params_.air > 0.0001f) {
...
private:
    ...
    float processDoppler(uint32_t source, float sample, float distance)
    {
        ...
        smoothedDelay_[source] += (target - smoothedDelay_[source]) * 0.025f;
