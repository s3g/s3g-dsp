...
public:
    ...
    bool setKernel(uint32_t node, const float* kernel, uint32_t frames)
    {
        ...
        if (!setup_ || !kernel || frames == 0u) {
            hasKernel_[node] = false;
            activePartitions_[node] = 0u;
            return false;
        }
        ...
        const size_t nodeOffset = static_cast<size_t>(node)
            * partitionCount_ * halfSize_;
        std::fill_n(kernelReal_.data() + nodeOffset,
            static_cast<size_t>(partitionCount_) * halfSize_, 0.0f);
        std::fill_n(kernelImag_.data() + nodeOffset,
            static_cast<size_t>(partitionCount_) * halfSize_, 0.0f);
...
public:
    bool prepare(double sampleRate)
    {
        sampleRate_ = std::clamp(std::isfinite(sampleRate) ? sampleRate : 48000.0,
            1000.0, 768000.0);
        maximumResponseFrames_ = static_cast<uint32_t>(std::ceil(
    ...
    void setParams(AmbiEffectResponseTraceParams params)
    {
        ...
        if (capturing_) next.responseEnabled = 0u;
        ...
        spatial_.setParams(spatialParams(params_));
        const bool bodyChanged = spatial_.resolvedBody() != previousBody;
        if (capturing_ && bodyChanged) cancelCapture();
        if (hasStoredResponse() && bodyChanged) beginConvolutionKernelRebuild();
        updateMask();
    }
    ...
    void startCapture()
    {
        if (captureBuffer_[0].empty()) return;
        ...
        for (uint32_t node = 0u; node < capturePickupCount_; ++node) {
            std::fill(captureBuffer_[node].begin(),
                captureBuffer_[node].end(), 0.0f);
        }
    }
    ...
    {
        return hasResponse() && params_.responseEnabled != 0u && !capturing_;
    }
    bool capturing() const { return capturing_; }
    bool preparingResponse() const { return kernelRebuildPending_; }
    ...
    {
        return kernelRebuildPending_ && kernelBuildTargetCount_ > 0u
            ? clamp(static_cast<float>(kernelBuildNode_)
                / static_cast<float>(kernelBuildTargetCount_), 0.0f, 1.0f)
            : (kernelsReady_ ? 1.0f : 0.0f);
    }
    ...
    {
        return responseApplied() ? latencyFrames() + responseFrames_ : 0u;
    }
    ...
    {
        return node < maskGain_.size() ? maskGain_[node] : 1.0f;
    }
    ...
    {
        if (!output) return;
        serviceConvolutionKernelRebuild();
        ...
        for (uint32_t frameIndex = 0u; frameIndex < frames; ++frameIndex) {
            ...
            const float excitationTarget = hasResponse()
            ...
            const float cutoff = 500.0f * std::pow(
                19000.0f / 500.0f, params_.tone);
            const float toneCoefficient = 1.0f - std::exp(
                -2.0f * kPi * cutoff / static_cast<float>(sampleRate_));
            for (uint32_t node = 0u; node < pickupCount; ++node) {
                const float convolved = convolution_.processSample(node,
                    ears[node] * excitationGain_);
                toneState_[node] += (convolved - toneState_[node])
                    * toneCoefficient;
                response[node] = flushDenormal(
                    std::tanh(toneState_[node] * currentResponseGain_));
            ...
            if (responseApplied()) {
                spatial_.routeNodes(response, routedFrom, previousTopology_,
                    roamingPhase_);
                spatial_.routeNodes(response, routedTo, targetTopology_,
                    roamingPhase_);
                for (uint32_t node = 0u; node < pickupCount; ++node) {
                    ...
                    const float target = lerp(delayedEars[node] * params_.maskDry,
            ...
            float peak = 0.0f;
            for (uint32_t channel = 0u; channel < activeOut; ++channel) {
                const float value = (delayedField[channel]
                    + correction[channel] * currentMix_) * currentOutputGain_;
            ...
            for (uint32_t channel = activeOut; channel < outputChannels;
                ++channel) {
...
private:
        ...
        uint32_t pickupCount)
    {
        ...
        for (uint32_t node = 0u; node < capturedPickupCount_; ++node) {
            std::copy_n(captureBuffer_[node].data(), responseFrames_,
                responseBuffer_[node].data());
        }
        conditionResponse(responseFrames_);
        responseSampleRate_ = sampleRate_;
    ...
    {
        beginConvolutionKernelRebuild();
        while (kernelRebuildPending_) serviceConvolutionKernelRebuild();
