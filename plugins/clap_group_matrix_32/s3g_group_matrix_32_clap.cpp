...
bool paramsTextToValue(const clap_plugin_t*, clap_id paramId, const char* display, double* value)
{
    ...
    if (paramId == kParamGroupSize) {
        for (uint32_t index = 0u; index <= 2u; ++index) {
            if (std::strcmp(display, s3g::groupMatrix32SizeName(static_cast<s3g::GroupMatrix32Size>(index))) == 0) {
                *value = static_cast<double>(index);
                return true;
            }
        }
...
- (void)randomizeMatrix
{
    ...
    for (uint32_t src = 0; src < groups; ++src) {
        for (uint32_t dst = 0; dst < groups; ++dst) {
            ...
            applyParam(*p, kCrosspointBase + s3g::groupMatrix32Index(src, dst), db);
        }
    }
    [self setNeedsDisplay:YES];
}
...
- (void)drawRect:(NSRect)dirty
{
    ...
    menu(@"GROUP", [NSString stringWithUTF8String:
        s3g::groupMatrix32SizeName(prm.groupSize)], 0u);
    menu(@"SHAPE", [NSString stringWithUTF8String:
        s3g::matrixFlowShapeName(prm.shape)], 1u);
    menu(@"MODE", prm.mode == s3g::GroupMatrix32FlowMode::Sync
        ? @"SYNC" : @"FREE", 2u);
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    if (p->guiView) {
        p->guiVisible = false;
        [static_cast<S3GGroupMatrix32View*>(p->guiView) stopRefreshTimer];
        s3g::clap_gui::destroyResponsiveViewport(p->guiViewport, p->guiView);
    }
}
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible = true; [static_cast<S3GGroupMatrix32View*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GGroupMatrix32View*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
