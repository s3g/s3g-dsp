...
constexpr uint32_t kGuiWidth=900,kGuiHeight=792;
void portableGuiDestroy(const clap_plugin_t*);
using EncoderPaths=std::array<s3g::AmbiPath,s3g::kAmbiPathEncoderMaxPaths>;
struct PathEdit{uint64_t revision=0;EncoderPaths paths{};};
...
struct Plugin {
    ...
    std::array<std::atomic<double>,21> encoderValues{};
    EncoderPaths guiPaths{};
    s3g::clap_gui::CaptureSnapshot<PathEdit> pendingPaths;
    uint64_t pathRevision=0,appliedPathRevision=0;
...
#include "../common/s3g_input_encoder_path_bridge.inc"
#else
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
 auto& p=*self(plugin);p.encoder.prepare(p.sampleRate);p.encoder.setParams(p.params);p.params=p.encoder.params();publishEncoderParams(p,p.params);publishEncoderPaths(p,p.encoder.paths());
#endif
 return true;
}
...
bool activate(const clap_plugin_t* plugin, double sampleRate, uint32_t, uint32_t)
{
    ...
    #if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    syncEncoderParams(*p);
    const auto paths=p->encoder.paths();
    #endif
    p->encoder.prepare(sampleRate);
    #if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    p->encoder.setPaths(paths);
    #endif
    p->encoder.setParams(p->params);
    p->params = p->encoder.params();
    return true;
...
clap_process_status process(const clap_plugin_t* plugin, const clap_process_t* proc)
{
    auto* p = self(plugin);
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    syncEncoderParams(*p);serviceEncoderGui(*p,proc?proc->out_events:nullptr);
#endif
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
if(!value||id<1||id>20)return false;*value=readEncoderValue(*self(plugin),id);return true;
#else
...
bool stateSave(const clap_plugin_t* plugin, const clap_ostream_t* stream)
{
    ...
    auto* p = self(plugin);
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    SavedState state{kStateVersion,snapshotEncoderParams(*p),p->guiPaths};
#else
...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream)
{
    ...
    for (const auto& path : state.paths) {
        if (path.pointCount > path.points.size()) return false;
        for (unsigned i = 0; i < path.pointCount; ++i) {
            const auto& point = path.points[i];
            if (!std::isfinite(point.x) || !std::isfinite(point.y)
                || !std::isfinite(point.z) || !std::isfinite(point.time)) return false;
        }
    }
    if (!std::isfinite(state.guiViewAzDeg) || !std::isfinite(state.guiViewElDeg)
        || !std::isfinite(state.guiViewZoom)) return false;
    s3g::AmbiPathEncoder model;model.setPaths(state.paths);model.setParams(state.params);
    publishEncoderParams(*p,model.params());publishEncoderPaths(*p,model.paths());
#else
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(pt, s3g::clap_gui::cocoaRect(titleBand.loadButton))) {
        ...
        if (s3g::clap_gui::loadPluginStatePresetPreservingParam(
                &_plugin->plugin, @"Ambi Path Encoder", kOutputParamId, &name)) {
...
namespace {
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "../common/s3g_input_encoder_path_canvas.inc"
#endif
