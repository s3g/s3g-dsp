...
#if defined(__APPLE__)
@interface S3GMacroPitchView : NSView { void* _plugin; int _dragSlider; NSTimer* _timer; }
...
@implementation S3GMacroPitchView
- (id)initWithPlugin:(void*)plugin
{
...
- (void)drawRect:(NSRect)dirty
{
    ...
    s3g::clap_gui::drawPanelFrame(18, 166, 352, 226, style);
    s3g::clap_gui::drawPanelHeader(@"LANE PITCH REL", true, 18, 166, 352, 21, lab, style);
    ...
    [self drawSlider:@"PCH" value:[NSString stringWithFormat:@"%+.1f", prm.pitchSemitones] norm:(prm.pitchSemitones + 24.0f) / 48.0f y:78 attrs:small small:small];
    [self drawSlider:@"FNE" value:[NSString stringWithFormat:@"%+.0f", prm.fineCents] norm:(prm.fineCents + 100.0f) / 200.0f y:104 attrs:small small:small];
    [self drawSlider:@"WIN" value:[NSString stringWithFormat:@"%.0f", prm.windowMs] norm:(prm.windowMs - 20.0f) / 160.0f y:130 attrs:small small:small];
...
- (void)updateSlider:(NSPoint)point
{
    ...
    } else if (_dragSlider >= 4 && _dragSlider <= 10) {
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    const CGFloat engineRows[] = { 78, 104, 130 };
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool isFloating) { if (!guiIsApiSupported(plugin, api, isFloating)) return false; auto* p = self(plugin); if (p->guiView) return true; p->guiView = [[S3GMacroPitchView alloc] initWithPlugin:p]; return p->guiView != nullptr; }
void guiDestroy(const clap_plugin_t* plugin) { auto* p = self(plugin); if (p->guiView) { p->guiVisible = false; auto* v = static_cast<S3GMacroPitchView*>(p->guiView); [v stopRefreshTimer]; [v removeFromSuperview]; [v release]; p->guiView = nullptr; } }
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = true; [static_cast<NSView*>(p->guiView) setHidden:NO]; [static_cast<S3GMacroPitchView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GMacroPitchView*>(p->guiView) stopRefreshTimer]; [static_cast<NSView*>(p->guiView) setHidden:YES]; return true; }
