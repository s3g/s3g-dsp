...
{
    auto* p = &instance;
    if (s3g::kDrumKickFactoryPresetCount == 0u) return false;
...
- (void)applyFactoryPreset:(int)index
{
    ...
    index = std::clamp(index, 0, static_cast<int>(s3g::kDrumKickFactoryPresetCount - 1u));
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_kick_vstgui.inc"
