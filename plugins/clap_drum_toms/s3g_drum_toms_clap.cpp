...
constexpr clap_id kMidTuneParamId = 3u;
constexpr clap_id kHighTuneParamId = 4u;
constexpr clap_id kPitchDropParamId = 5u;
constexpr clap_id kPitchSweepTimeParamId = 6u;
constexpr clap_id kShellSpreadParamId = 7u;
constexpr clap_id kBodyParamId = 8u;
constexpr clap_id kRingParamId = 9u;
constexpr clap_id kBodyDecayParamId = 10u;
constexpr clap_id kDecaySpreadParamId = 11u;
constexpr clap_id kPunchParamId = 12u;
constexpr clap_id kRimLevelParamId = 13u;
constexpr clap_id kRimCharacterParamId = 14u;
constexpr clap_id kRimDecayParamId = 15u;
constexpr clap_id kStickToneParamId = 16u;
...
constexpr std::array<ParamDef, kParamCount> kParamDefs {{
    { kLowTuneParamId, "Low Tune", "Kit / Pitch", 40.0, 180.0, 82.0, false },
    { kNoteTrackingParamId, "Note Tracking", "Kit / MIDI", 0.0, 1.0, 0.75, false },
    { kMidTuneParamId, "Mid Tune", "Kit / Pitch", 55.0, 280.0, 127.0, false },
    { kHighTuneParamId, "High Tune", "Kit / Pitch", 70.0, 420.0, 182.0, false },
    { kPitchDropParamId, "Pitch Drop", "Kit / Pitch", -6.0, 30.0, 7.0, false },
    { kPitchSweepTimeParamId, "Sweep Time", "Kit / Pitch", 1.0, 250.0, 32.0, false },
    { kShellSpreadParamId, "Shell Spread", "Shape", 0.0, 1.0, 0.40, false },
    { kBodyParamId, "Body", "Shape", 0.0, 1.0, 0.62, false },
    { kRingParamId, "Ring", "Shape", 0.0, 1.0, 0.38, false },
    { kBodyDecayParamId, "Body Decay", "Shape", 0.03, 3.0, 0.65, false },
    { kDecaySpreadParamId, "Decay Spread", "Shape", -1.0, 1.0, 0.25, false },
    { kPunchParamId, "Punch", "Shape", 0.0, 1.0, 0.74, false },
    { kRimLevelParamId, "Rim Level", "Articulation", 0.0, 1.0, 0.52, false },
    { kRimCharacterParamId, "Rim Character", "Articulation", 0.0, 1.0, 0.52, false },
    { kRimDecayParamId, "Rim Decay", "Articulation", 0.02, 0.30, 0.065, false },
    { kStickToneParamId, "Stick Tone", "Articulation", 0.0, 1.0, 0.62, false },
...
struct Plugin {
    ...
    std::array<std::atomic<uint32_t>, 6u> pendingTriggers {};
...
uint64_t tailSamplesForParams(const s3g::DrumTomsParams& params,
    double sampleRate)
{
    const double seconds = s3g::drumTomsTailSeconds(params, sampleRate);
...
struct RoutedTomTrigger {
    s3g::DrumTomSlot slot = s3g::DrumTomSlot::Mid;
    s3g::DrumTomArticulation articulation =
        s3g::DrumTomArticulation::Head;
    int midiNote = 48;
};

RoutedTomTrigger triggerForCode(uint32_t code)
{
    switch (code) {
    case 1u: return { s3g::DrumTomSlot::Low,
        s3g::DrumTomArticulation::Head, 45 };
    case 2u: return { s3g::DrumTomSlot::Mid,
        s3g::DrumTomArticulation::Head, 48 };
    case 3u: return { s3g::DrumTomSlot::High,
        s3g::DrumTomArticulation::Head, 50 };
    case 4u: return { s3g::DrumTomSlot::Low,
        s3g::DrumTomArticulation::RimStick, 45 };
    case 5u: return { s3g::DrumTomSlot::Mid,
        s3g::DrumTomArticulation::RimStick, 48 };
    case 6u: return { s3g::DrumTomSlot::High,
        s3g::DrumTomArticulation::RimStick, 50 };
    default: return {};
    }
}

RoutedTomTrigger routeMidiNote(int midiNote)
{
    midiNote = std::clamp(midiNote, 0, 127);
    switch (midiNote) {
    case 37: return { s3g::DrumTomSlot::Mid,
        s3g::DrumTomArticulation::RimStick, 48 };
    case 57: return { s3g::DrumTomSlot::Low,
        s3g::DrumTomArticulation::RimStick, 45 };
    case 59: return { s3g::DrumTomSlot::Mid,
        s3g::DrumTomArticulation::RimStick, 47 };
    case 60: return { s3g::DrumTomSlot::Mid,
        s3g::DrumTomArticulation::RimStick, 48 };
    case 62: return { s3g::DrumTomSlot::High,
        s3g::DrumTomArticulation::RimStick, 50 };
    case 45: return { s3g::DrumTomSlot::Low,
        s3g::DrumTomArticulation::Head, midiNote };
    case 47:
    case 48: return { s3g::DrumTomSlot::Mid,
        s3g::DrumTomArticulation::Head, midiNote };
    case 50: return { s3g::DrumTomSlot::High,
        s3g::DrumTomArticulation::Head, midiNote };
    default: break;
    }
    const int lowDistance = std::abs(midiNote - 45);
    const int midDistance = std::abs(midiNote - 48);
    const int highDistance = std::abs(midiNote - 50);
    const auto slot = lowDistance <= midDistance && lowDistance <= highDistance
        ? s3g::DrumTomSlot::Low
        : (midDistance <= highDistance
            ? s3g::DrumTomSlot::Mid : s3g::DrumTomSlot::High);
    return { slot, s3g::DrumTomArticulation::Head, midiNote };
}
...
void clearPendingTriggers(Plugin& p)
{
    for (auto& count : p.pendingTriggers) {
    ...
    bool triggerImmediately = false)
{
    ...
    if (id == kTriggerParamId) {
        ...
        if (code > 0u && code != p.triggerCode) {
            if (triggerImmediately) {
                triggerToms(p, triggerForCode(code), 1.0f);
            } else {
                p.pendingTriggers[code - 1u].fetch_add(
    ...
    case kHighTuneParamId: p.params.highTuneHz = v; break;
    ...
    case kPunchParamId:
        ...
        p.params.punch = v;
        break;
    case kDecaySpreadParamId:
        tailChanged = std::fabs(
            p.params.decaySpread - v) > 1.0e-6f;
        p.params.decaySpread = v;
        break;
    case kRimLevelParamId:
        tailChanged = std::fabs(p.params.rimLevel - v) > 1.0e-6f;
        p.params.rimLevel = v;
        break;
    case kRimCharacterParamId:
        p.params.rimCharacter = v;
        break;
...
double rawParamValue(const Plugin& p, clap_id id)
{
    ...
    case kHighTuneParamId: return p.params.highTuneHz;
    ...
    case kDecaySpreadParamId: return p.params.decaySpread;
...
void assignSavedStateValues(s3g::DrumTomsParams& params,
    const std::array<double, kSavedParamCount>& values)
{
    const auto value = [&](clap_id id) {
        return static_cast<float>(values[id - kLowTuneParamId]);
    };
    ...
    params.highTuneHz = value(kHighTuneParamId);
    ...
    params.decaySpread = value(kDecaySpreadParamId);
...
bool consumePendingState(Plugin& p)
{
    ...
    s3g::DrumTomsParams next = p.params;
...
{
    if (code < 1u || code > 6u) return false;
    using Kind = s3g::clap_gui::ParamEventKind;
    ...
    const clap_process_t* process)
{
    ...
    for (uint32_t code = 1u; code <= p->pendingTriggers.size(); ++code) {
        uint32_t pending = p->pendingTriggers[code - 1u].exchange(
    ...
    for (uint32_t sample = 0u; sample < process->frames_count; ++sample) {
        ...
        float left = 0.0f;
        float right = 0.0f;
        p->toms.processFrame(left, right);
...
constexpr std::array<TomNoteName, 9u> kTomNoteNames {{
    { 37, "Mid Rim / Side Stick" },
    { 45, "Low Tom Head" },
    { 47, "Mid Tom Head" },
    { 48, "Mid Tom Head" },
    { 50, "High Tom Head" },
...
{
    return static_cast<uint32_t>(kTomNoteNames.size());
}

bool noteNameGet(const clap_plugin_t*, uint32_t index,
    clap_note_name_t* noteName)
{
    if (!noteName || index >= kTomNoteNames.size()) return false;
    *noteName = {};
    noteName->port = 0;
    noteName->key = kTomNoteNames[index].key;
    noteName->channel = -1;
    std::strncpy(noteName->name, kTomNoteNames[index].name,
    ...
    char* display, uint32_t size)
{
    ...
    if (id == kLowTuneParamId || id == kMidTuneParamId
        || id == kHighTuneParamId) {
        ...
        constexpr std::array<const char*, 7u> names {{
            ...
            "Low Rim", "Mid Rim", "High Rim",
        }};
        const auto code = static_cast<uint32_t>(std::clamp(
            std::llround(value), 0ll, 6ll));
        std::snprintf(display, size, "%s", names[code]);
    } else {
        std::snprintf(display, size, "%.0f%%", value * 100.0);
    }
    return true;
    ...
    const char* display, double* value)
{
    ...
    if (id == kTriggerParamId) {
        ...
        for (uint32_t code = 0u; code < names.size(); ++code) {
    ...
    const bool percentParam = id != kLowTuneParamId
        && id != kMidTuneParamId && id != kHighTuneParamId
...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream)
{
    ...
    if (p->preserveOutputOnNextStateLoad.exchange(
        ...
        const uint32_t outputIndex = kOutputParamId - kLowTuneParamId;
...
uint32_t tailGet(const clap_plugin_t* plugin)
{
    ...
    std::array<double, kSavedParamCount> values {};
    for (uint32_t index = 0u; index < values.size(); ++index) {
        values[index] = paramValue(*p, kParamDefs[index].id);
    }
    s3g::DrumTomsParams snapshot;
    assignSavedStateValues(snapshot, values);
    const double seconds = s3g::drumTomsTailSeconds(
        snapshot, p->sampleRate);
...
constexpr std::array<TomsUiRow, kSavedParamCount> kUiRows {{
    { kLowTuneParamId, "LOW TUNE", kLeftPanelX, kPanelWidth, 80.0 },
    { kNoteTrackingParamId, "NOTE TRACK", kLeftPanelX, kPanelWidth, 104.0 },
    { kMidTuneParamId, "MID TUNE", kLeftPanelX, kPanelWidth, 128.0 },
    { kHighTuneParamId, "HIGH TUNE", kLeftPanelX, kPanelWidth, 152.0 },
    { kPitchDropParamId, "PITCH DROP", kLeftPanelX, kPanelWidth, 176.0 },
    { kPitchSweepTimeParamId, "SWEEP TIME", kLeftPanelX, kPanelWidth, 200.0 },

    { kShellSpreadParamId, "SHELL SPREAD", kRightPanelX, kPanelWidth, 80.0 },
    { kBodyParamId, "BODY", kRightPanelX, kPanelWidth, 104.0 },
    { kRingParamId, "RING", kRightPanelX, kPanelWidth, 128.0 },
    { kBodyDecayParamId, "BODY DECAY", kRightPanelX, kPanelWidth, 152.0 },
    { kDecaySpreadParamId, "DECAY SPREAD", kRightPanelX, kPanelWidth, 176.0 },
    { kPunchParamId, "PUNCH", kRightPanelX, kPanelWidth, 200.0 },
    { kRimLevelParamId, "RIM LEVEL", kRightPanelX, kPanelWidth, 224.0 },
    { kRimCharacterParamId, "RIM CHARACTER", kRightPanelX, kPanelWidth, 248.0 },
    { kRimDecayParamId, "RIM DECAY", kRightPanelX, kPanelWidth, 272.0 },
    { kStickToneParamId, "STICK TONE", kRightPanelX, kPanelWidth, 296.0 },
...
{
    return id == kLowTuneParamId || id == kMidTuneParamId
        || id == kHighTuneParamId || id == kPitchSweepTimeParamId
        || id == kBodyDecayParamId || id == kRimDecayParamId;
}
...
s3g::DrumTomsParams publishedParamsSnapshot(const Plugin& p)
{
    s3g::DrumTomsParams params;
    ...
    params.highTuneHz = static_cast<float>(paramValue(p, kHighTuneParamId));
    ...
    params.shellSpread = static_cast<float>(paramValue(p, kShellSpreadParamId));
    ...
    params.decaySpread = static_cast<float>(
        paramValue(p, kDecaySpreadParamId));
...
double safeRandomParamValue(
    const s3g::DrumTomsParams& params, clap_id id)
{
    switch (id) {
    case kLowTuneParamId: return params.lowTuneHz;
    case kMidTuneParamId: return params.midTuneHz;
    case kHighTuneParamId: return params.highTuneHz;
    ...
    case kDecaySpreadParamId: return params.decaySpread;
...
bool queueGuiSafeRandomParamSet(Plugin& p,
    const s3g::DrumTomsParams& sourceParams)
{
    using Kind = s3g::clap_gui::ParamEventKind;
...
{
    auto* p = &instance;
    if (s3g::kDrumTomsFactoryPresetCount == 0u) return false;
    index = std::clamp(index, 0,
        static_cast<int>(s3g::kDrumTomsFactoryPresetCount - 1u));
    const auto preset = s3g::drumTomsFactoryPreset(
        static_cast<uint32_t>(index));
    std::array<double, kSavedParamCount> values {};
    const auto set = [&](clap_id id, double value) {
        values[id - kLowTuneParamId] = value;
    };
    set(kLowTuneParamId, preset.lowTuneHz);
    set(kNoteTrackingParamId, preset.noteTracking);
    set(kMidTuneParamId, preset.midTuneHz);
    set(kHighTuneParamId, preset.highTuneHz);
    ...
    set(kDecaySpreadParamId, preset.decaySpread);
...
#if defined(__APPLE__)
NSRect auditionPadRect(uint32_t code)
{
    if (code < 1u || code > 6u) return NSZeroRect;
    constexpr CGFloat gap = 7.0;
    constexpr CGFloat inset = 12.0;
    constexpr CGFloat height = 24.0;
    constexpr CGFloat width =
        (kPanelWidth - 2.0 * inset - 2.0 * gap) / 3.0;
...
@interface S3GDrumTomsView : NSView {
    void* _plugin;
    int _dragParam;
...
@implementation S3GDrumTomsView
...
- (id)initWithPlugin:(void*)plugin
{
    ...
    if (self) {
        ...
        _factoryPresetIndex = p
            ? s3g::drumTomsFactoryPresetIndex(publishedParamsSnapshot(*p))
            : 0;
...
- (void)applyFactoryPreset:(int)index
{
    ...
    std::snprintf(_presetName, sizeof(_presetName), "%s",
        s3g::drumTomsFactoryPresetInfo(static_cast<uint32_t>(index)).name);
    ...
    style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        items[index] = [NSString stringWithUTF8String:
            s3g::drumTomsFactoryPresetInfo(index).name];
    }
...
- (void)refresh:(NSTimer*)timer
{
    ...
    if (p) {
        ...
        if (revision != _observedParamRevision) {
            ...
            _factoryPresetIndex = s3g::drumTomsFactoryPresetIndex(
                publishedParamsSnapshot(*p));
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                _factoryPresetIndex >= 0
                    ? s3g::drumTomsFactoryPresetInfo(static_cast<uint32_t>(
...
- (void)drawRect:(NSRect)dirty
{
    ...
    drawPanel(@"KIT / PITCH", voicePanel);
    drawPanel(@"SHAPE / ARTICULATION", attackPanel);
    drawPanel(@"SHARED CHARACTER", characterPanel);
    ...
    constexpr std::array<const char*, 6u> padLabels {{
    ...
    [@"THREE SYNTHESIZED SHELLS  //  HEAD + RIM"
        drawAtPoint:NSMakePoint(kRightPanelX + 62.0, 646.0)
...
- (void)updateDraggedParam:(NSPoint)point
{
    ...
    const auto row = std::find_if(kUiRows.begin(), kUiRows.end(),
        [=](const TomsUiRow& candidate) { return candidate.id == id; });
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(point,
        ...
        _menuItemCount = std::min<uint32_t>(
            s3g::kDrumTomsFactoryPresetCount, 64u);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(point,
    ...
    if (NSPointInRect(point,
        ...
        const auto randomized = s3g::drumTomsSafeRandomParams(
    ...
    for (uint32_t code = 1u; code <= 6u; ++code) {
        if (NSPointInRect(point, auditionPadRect(code))) {
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    p->guiView = [[S3GDrumTomsView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = false;
    [static_cast<S3GDrumTomsView*>(p->guiView) stopRefreshTimer];
...
bool guiShow(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = true;
    [static_cast<S3GDrumTomsView*>(p->guiView) startRefreshTimer];
    return true;
}
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_toms_vstgui.inc"
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "A procedural mono-compatible stereo low/mid/high tom synthesizer with head and rim-stick articulations, inharmonic shells, and reusable drum character stages.",
    features
};
