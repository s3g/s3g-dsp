...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream)
{
    ...
    if (version == kStateVersion) {
        ...
        p->presetIndex = std::min<uint32_t>(state.presetIndex, s3g::kAmbiPyrosphereFactoryPresetCount - 1u);
        ...
        uint32_t presetIndex = 0u;
        char customPresetName[64] {};
        constexpr size_t legacyParamsSize = offsetof(s3g::AmbiPyrosphereParams, fieldListenMode);
        if (!readExact(stream, &params, legacyParamsSize)
            || !readExact(stream, &presetIndex, sizeof(presetIndex))
            || !readExact(stream, customPresetName, sizeof(customPresetName))) return false;
        p->params = params;
        p->presetIndex = std::min<uint32_t>(presetIndex, s3g::kAmbiPyrosphereFactoryPresetCount - 1u);
        std::snprintf(p->customPresetName, sizeof(p->customPresetName), "%s", customPresetName);
    } else if (version == 14u) {
        s3g::AmbiPyrosphereParams params {};
        uint32_t presetIndex = 0u;
        char customPresetName[64] {};
        constexpr size_t legacyParamsSize = offsetof(s3g::AmbiPyrosphereParams, environmentSize);
        if (!readExact(stream, &params, legacyParamsSize)
            || !readExact(stream, &presetIndex, sizeof(presetIndex))
            || !readExact(stream, customPresetName, sizeof(customPresetName))) return false;
        p->params = params;
        ...
        uint32_t presetIndex = 0u;
        char customPresetName[64] {};
        constexpr size_t legacyParamsSize = offsetof(s3g::AmbiPyrosphereParams, place);
        if (!readExact(stream, &params, legacyParamsSize)
            || !readExact(stream, &presetIndex, sizeof(presetIndex))
            || !readExact(stream, customPresetName, sizeof(customPresetName))) return false;
        p->params = params;
...
- (NSString*)presetDisplayName
{
    ...
    return [NSString stringWithUTF8String:s3g::ambiPyrosphereFactoryPresetInfo(_plugin->presetIndex).name];
}
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible = true; [static_cast<S3GAmbiPyrosphereEncoderView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GAmbiPyrosphereEncoderView*>(p->guiView) hideSurfacePopup]; [static_cast<S3GAmbiPyrosphereEncoderView*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
