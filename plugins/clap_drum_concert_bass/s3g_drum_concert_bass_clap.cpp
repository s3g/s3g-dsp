...
constexpr uint32_t kStateMagic = 0x42433353u; // "S3CB" in little endian.
constexpr uint32_t kStateVersion = 1u;
constexpr uint32_t kOutputChannels = 2u;
constexpr uint32_t kGuiWidth = 920u;
constexpr uint32_t kGuiHeight = 704u;

constexpr clap_id kTuneParamId = 1u;
constexpr clap_id kNoteTrackingParamId = 2u;
constexpr clap_id kSizeParamId = 3u;
constexpr clap_id kHeadTensionParamId = 4u;
constexpr clap_id kStrikePositionParamId = 5u;
constexpr clap_id kBeaterHardnessParamId = 6u;
constexpr clap_id kImpactParamId = 7u;
constexpr clap_id kBodyParamId = 8u;
constexpr clap_id kBodyDecayParamId = 9u;
constexpr clap_id kDampingParamId = 10u;
constexpr clap_id kBloomParamId = 11u;
constexpr clap_id kAirParamId = 12u;
constexpr clap_id kShellParamId = 13u;
constexpr clap_id kShellToneParamId = 14u;
constexpr clap_id kMutedDecayParamId = 15u;
constexpr clap_id kRimLevelParamId = 16u;
constexpr clap_id kRimToneParamId = 17u;
constexpr clap_id kRimDecayParamId = 18u;
constexpr clap_id kDriveParamId = 19u;
constexpr clap_id kBiasParamId = 20u;
constexpr clap_id kCompressionParamId = 21u;
constexpr clap_id kRateReductionParamId = 22u;
constexpr clap_id kBitDepthParamId = 23u;
constexpr clap_id kReconstructionParamId = 24u;
constexpr clap_id kCharacterToneParamId = 25u;
constexpr clap_id kStereoWidthParamId = 26u;
constexpr clap_id kVelocityParamId = 27u;
constexpr clap_id kOutputParamId = 28u;
constexpr clap_id kTriggerParamId = 29u;
constexpr clap_id kMidiReceiveParamId = 30u;
constexpr uint32_t kParamCount = 30u;
constexpr uint32_t kSavedParamCount = kParamCount - 1u;
constexpr uint32_t kLegacySavedParamCount = kSavedParamCount;
...
constexpr std::array<ParamDef, kParamCount> kParamDefs {{
    { kTuneParamId, "Tune", "Membrane", 24.0, 96.0, 44.0, false },
    { kNoteTrackingParamId, "Note Tracking", "Membrane / MIDI", 0.0, 1.0, 0.50, false },
    { kSizeParamId, "Form", "Membrane", 0.0, 1.0, 0.56, false },
    { kHeadTensionParamId, "Head Tension", "Membrane", 0.0, 1.0, 0.45, false },
    { kStrikePositionParamId, "Strike Position", "Exciter", 0.0, 1.0, 0.20, false },
    { kBeaterHardnessParamId, "Beater Hardness", "Exciter", 0.0, 1.0, 0.34, false },
    { kImpactParamId, "Impact", "Exciter", 0.0, 1.0, 0.58, false },
    { kBodyParamId, "Body", "Membrane", 0.0, 1.0, 0.88, false },
    { kBodyDecayParamId, "Body Decay", "Membrane", 0.15, 8.0, 3.20, false },
    { kDampingParamId, "Damping", "Membrane", 0.0, 1.0, 0.22, false },
    { kBloomParamId, "Bloom", "Air Coupling", 0.0, 1.0, 0.58, false },
    { kAirParamId, "Air", "Air Coupling", 0.0, 1.0, 0.62, false },
    { kShellParamId, "Boundary", "Membrane", 0.0, 1.0, 0.28, false },
    { kShellToneParamId, "Boundary Tone", "Membrane", 0.0, 1.0, 0.38, false },
    { kMutedDecayParamId, "Muted Decay", "Articulation", 0.04, 1.5, 0.32, false },
    { kRimLevelParamId, "Rim Level", "Articulation", 0.0, 1.0, 0.55, false },
    { kRimToneParamId, "Rim Tone", "Articulation", 0.0, 1.0, 0.35, false },
    { kRimDecayParamId, "Rim Decay", "Articulation", 0.015, 1.0, 0.14, false },
    ...
    { kStereoWidthParamId, "Stereo Width", "Output", 0.0, 1.0, 0.16, false },
...
struct Plugin {
    ...
    s3g::DrumConcertBassParams params {};
    s3g::DrumConcertBass concertBass;
    ...
    std::atomic<uint32_t> pendingCenterTriggers { 0u };
    std::atomic<uint32_t> pendingMutedTriggers { 0u };
...
s3g::DrumConcertBassParams publishedParamsSnapshot(const Plugin& p)
{
    s3g::DrumConcertBassParams params;
    ...
    params.size = static_cast<float>(paramValue(p, kSizeParamId));
    params.headTension = static_cast<float>(
        paramValue(p, kHeadTensionParamId));
    params.strikePosition = static_cast<float>(
        paramValue(p, kStrikePositionParamId));
    params.beaterHardness = static_cast<float>(
        paramValue(p, kBeaterHardnessParamId));
    params.impact = static_cast<float>(paramValue(p, kImpactParamId));
    ...
    params.bloom = static_cast<float>(paramValue(p, kBloomParamId));
    params.air = static_cast<float>(paramValue(p, kAirParamId));
    params.shell = static_cast<float>(paramValue(p, kShellParamId));
    params.shellTone = static_cast<float>(paramValue(p, kShellToneParamId));
    params.mutedDecaySeconds = static_cast<float>(
        paramValue(p, kMutedDecayParamId));
    params.rimLevel = static_cast<float>(paramValue(p, kRimLevelParamId));
    params.rimTone = static_cast<float>(paramValue(p, kRimToneParamId));
...
uint64_t tailSamplesForParams(const s3g::DrumConcertBassParams& params,
    double sampleRate)
{
    const double seconds = s3g::drumConcertBassTailSeconds(params, sampleRate);
...
void triggerConcertBass(Plugin& p,
    ...
    float velocity, int midiNote)
{
    ...
    p.concertBass.trigger(articulation, std::clamp(velocity, 0.0f, 1.0f),
    ...
    bool triggerImmediately = false)
{
    ...
    if (id == kTriggerParamId) {
        ...
        if (code > 0u && code != p.triggerCode) {
            ...
            if (triggerImmediately) {
                triggerConcertBass(p, articulation, 1.0f,
                    code == 2u ? 35 : (code == 3u ? 37 : 36));
            } else {
                auto& pending = code == 3u ? p.pendingRimTriggers
                    : (code == 2u ? p.pendingMutedTriggers
                                  : p.pendingCenterTriggers);
    ...
    case kSizeParamId: p.params.size = v; break;
    case kHeadTensionParamId: p.params.headTension = v; break;
    case kStrikePositionParamId: p.params.strikePosition = v; break;
    case kBeaterHardnessParamId: p.params.beaterHardness = v; break;
    case kImpactParamId: p.params.impact = v; break;
    ...
    case kDampingParamId:
        ...
        p.params.damping = v;
        break;
    case kBloomParamId: p.params.bloom = v; break;
    case kAirParamId: p.params.air = v; break;
    case kShellParamId: p.params.shell = v; break;
    case kShellToneParamId: p.params.shellTone = v; break;
    case kMutedDecayParamId:
        tailChanged = std::fabs(p.params.mutedDecaySeconds - v) > 1.0e-6f;
        p.params.mutedDecaySeconds = v;
        break;
    case kRimLevelParamId: p.params.rimLevel = v; break;
    case kRimToneParamId: p.params.rimTone = v; break;
    ...
    p.concertBass.setParams(p.params);
    p.params = p.concertBass.params();
...
double rawParamValue(const Plugin& p, clap_id id)
{
    ...
    case kSizeParamId: return p.params.size;
    case kHeadTensionParamId: return p.params.headTension;
    case kStrikePositionParamId: return p.params.strikePosition;
    case kBeaterHardnessParamId: return p.params.beaterHardness;
    case kImpactParamId: return p.params.impact;
    ...
    case kBloomParamId: return p.params.bloom;
    case kAirParamId: return p.params.air;
    case kShellParamId: return p.params.shell;
    case kShellToneParamId: return p.params.shellTone;
    case kMutedDecayParamId: return p.params.mutedDecaySeconds;
    case kRimLevelParamId: return p.params.rimLevel;
    case kRimToneParamId: return p.params.rimTone;
...
void assignSavedStateValues(s3g::DrumConcertBassParams& params,
    const std::array<double, kSavedParamCount>& values)
{
    ...
    params.size = value(kSizeParamId);
    params.headTension = value(kHeadTensionParamId);
    params.strikePosition = value(kStrikePositionParamId);
    params.beaterHardness = value(kBeaterHardnessParamId);
    params.impact = value(kImpactParamId);
    params.body = value(kBodyParamId);
    params.bodyDecaySeconds = value(kBodyDecayParamId);
    params.damping = value(kDampingParamId);
    params.bloom = value(kBloomParamId);
    params.air = value(kAirParamId);
    params.shell = value(kShellParamId);
    params.shellTone = value(kShellToneParamId);
    params.mutedDecaySeconds = value(kMutedDecayParamId);
    params.rimLevel = value(kRimLevelParamId);
    params.rimTone = value(kRimToneParamId);
...
bool consumePendingState(Plugin& p)
{
    ...
    s3g::DrumConcertBassParams next = p.params;
    assignSavedStateValues(next, values);
    p.concertBass.setParams(next);
    p.params = p.concertBass.params();
    p.midiReceive = values.back();
    p.pendingCenterTriggers.store(0u, std::memory_order_relaxed);
    p.pendingMutedTriggers.store(0u, std::memory_order_relaxed);
...
void applyEvent(Plugin& p, const clap_event_header_t* event)
{
    ...
    if (event->type == CLAP_EVENT_PARAM_VALUE
        ...
        if (note->velocity > 0.0
            ...
            const auto articulation = s3g::drumConcertBassArticulationForMidi(
                note->key);
            triggerConcertBass(p, articulation,
        ...
        if ((midi->data[0] & 0xf0u) == 0x90u && midi->data[2] > 0u
            ...
            const auto articulation = s3g::drumConcertBassArticulationForMidi(
                note);
            triggerConcertBass(p, articulation,
                static_cast<float>(midi->data[2]) / 127.0f, note);
        }
    }
}
    ...
    uint32_t, uint32_t)
{
    ...
    p->concertBass.prepare(p->sampleRate);
    p->concertBass.setParams(p->params);
    ...
    const clap_process_t* process)
{
    ...
    uint32_t pending = p->pendingCenterTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerConcertBass(*p,
        s3g::DrumConcertBassArticulation::Center, 1.0f, 36);
    pending = p->pendingMutedTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerConcertBass(*p,
        s3g::DrumConcertBassArticulation::Muted, 1.0f, 35);
    pending = p->pendingRimTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerConcertBass(*p,
        s3g::DrumConcertBassArticulation::Rim, 1.0f, 37);
    ...
    for (uint32_t sample = 0u; sample < process->frames_count; ++sample) {
        ...
        float left = 0.0f;
        float right = 0.0f;
        p->concertBass.processFrame(left, right);
...
struct ConcertBassNoteName {
    int16_t key;
    const char* name;
};

constexpr std::array<ConcertBassNoteName, 3u> kConcertBassNoteNames {{
    { 35, "Concert Bass Muted" },
    { 36, "Concert Bass Center" },
    { 37, "Concert Bass Rim" },
}};

uint32_t noteNameCount(const clap_plugin_t*)
{
    return static_cast<uint32_t>(kConcertBassNoteNames.size());
}

bool noteNameGet(const clap_plugin_t*, uint32_t index,
    clap_note_name_t* noteName)
{
    if (!noteName || index >= kConcertBassNoteNames.size()) return false;
    *noteName = {};
    noteName->port = 0;
    noteName->key = kConcertBassNoteNames[index].key;
    noteName->channel = -1;
    std::strncpy(noteName->name, kConcertBassNoteNames[index].name,
    ...
    char* display, uint32_t size)
{
    ...
    if (id == kTuneParamId) {
    ...
    } else if (id == kBodyDecayParamId || id == kMutedDecayParamId
        || id == kRimDecayParamId) {
        ...
        std::snprintf(display, size, "%s",
            code == 3u ? "Rim" : (code == 2u ? "Muted"
                : (code == 1u ? "Center" : "Ready")));
    } else {
        std::snprintf(display, size, "%.0f%%", value * 100.0);
    }
    return true;
    ...
    const char* display, double* value)
{
    ...
    if (id == kTriggerParamId) {
        if (std::strcmp(display, "Center") == 0) {
            *value = 1.0;
            return true;
        }
...
{
    const auto* p = self(plugin);
    const double seconds = s3g::drumConcertBassTailSeconds(
...
constexpr std::array<ConcertBassUiRow, kSavedParamCount> kUiRows {{
    { kTuneParamId, "TUNE", kLeftPanelX, kPanelWidth, 80.0 },
    { kNoteTrackingParamId, "NOTE TRACK", kLeftPanelX, kPanelWidth, 104.0 },
    { kSizeParamId, "FORM", kLeftPanelX, kPanelWidth, 128.0 },
    { kHeadTensionParamId, "HEAD TENSION", kLeftPanelX, kPanelWidth, 152.0 },
    { kBodyParamId, "BODY", kLeftPanelX, kPanelWidth, 176.0 },
    { kBodyDecayParamId, "BODY DECAY", kLeftPanelX, kPanelWidth, 200.0 },
    { kDampingParamId, "DAMPING", kLeftPanelX, kPanelWidth, 224.0 },
    { kBloomParamId, "BLOOM", kLeftPanelX, kPanelWidth, 248.0 },
    { kAirParamId, "AIR", kLeftPanelX, kPanelWidth, 272.0 },

    { kStrikePositionParamId, "STRIKE POSITION", kRightPanelX, kPanelWidth, 80.0 },
    { kBeaterHardnessParamId, "BEATER HARDNESS", kRightPanelX, kPanelWidth, 104.0 },
    { kImpactParamId, "IMPACT", kRightPanelX, kPanelWidth, 128.0 },
    { kShellParamId, "BOUNDARY", kRightPanelX, kPanelWidth, 152.0 },
    { kShellToneParamId, "BOUNDARY TONE", kRightPanelX, kPanelWidth, 176.0 },
    { kMutedDecayParamId, "MUTED DECAY", kRightPanelX, kPanelWidth, 200.0 },
    { kRimLevelParamId, "RIM LEVEL", kRightPanelX, kPanelWidth, 224.0 },
    { kRimToneParamId, "RIM TONE", kRightPanelX, kPanelWidth, 248.0 },
    { kRimDecayParamId, "RIM DECAY", kRightPanelX, kPanelWidth, 272.0 },

    { kDriveParamId, "DRIVE", kLeftPanelX, kPanelWidth, 398.0 },
    { kBiasParamId, "BIAS", kLeftPanelX, kPanelWidth, 422.0 },
    { kCompressionParamId, "COMPRESSION", kLeftPanelX, kPanelWidth, 446.0 },
    { kRateReductionParamId, "RATE REDUCTION", kLeftPanelX, kPanelWidth, 470.0 },
    { kBitDepthParamId, "BIT REDUCTION", kLeftPanelX, kPanelWidth, 494.0 },
    { kReconstructionParamId, "RECONSTRUCTION", kLeftPanelX, kPanelWidth, 518.0 },
    { kCharacterToneParamId, "TONE", kLeftPanelX, kPanelWidth, 542.0 },

    { kStereoWidthParamId, "STEREO WIDTH", kRightPanelX, kPanelWidth, 398.0 },
    { kVelocityParamId, "VELOCITY", kRightPanelX, kPanelWidth, 422.0 },
    { kOutputParamId, "OUTPUT", kRightPanelX, kPanelWidth, 446.0 },
    { kMidiReceiveParamId, "MIDI RECEIVE", kRightPanelX, kPanelWidth, 470.0 },
}};

bool isLogUiParam(clap_id id)
{
    return id == kTuneParamId || id == kBodyDecayParamId
        || id == kMutedDecayParamId || id == kRimDecayParamId;
}
...
constexpr std::array<clap_id, 25u> kSafeRandomParamIds {{
    kTuneParamId,
    kSizeParamId,
    kHeadTensionParamId,
    kStrikePositionParamId,
    kBeaterHardnessParamId,
    kImpactParamId,
    kBodyParamId,
    kBodyDecayParamId,
    kDampingParamId,
...
double safeRandomParamValue(
    const s3g::DrumConcertBassParams& params, clap_id id)
{
    switch (id) {
    case kTuneParamId: return params.tuneHz;
    case kSizeParamId: return params.size;
    case kHeadTensionParamId: return params.headTension;
    case kStrikePositionParamId: return params.strikePosition;
    case kBeaterHardnessParamId: return params.beaterHardness;
    case kImpactParamId: return params.impact;
    ...
    case kBloomParamId: return params.bloom;
    case kAirParamId: return params.air;
    case kShellParamId: return params.shell;
    case kShellToneParamId: return params.shellTone;
    case kMutedDecayParamId: return params.mutedDecaySeconds;
    case kRimLevelParamId: return params.rimLevel;
    case kRimToneParamId: return params.rimTone;
...
bool queueGuiSafeRandomParamSet(Plugin& p,
    const s3g::DrumConcertBassParams& sourceParams)
{
    using Kind = s3g::clap_gui::ParamEventKind;
...
{
    auto* p = &instance;
    if (s3g::kDrumConcertBassFactoryPresetCount == 0u) return false;
    index = std::clamp(index, 0,
        static_cast<int>(s3g::kDrumConcertBassFactoryPresetCount - 1u));
    const auto preset = s3g::drumConcertBassFactoryPreset(
    ...
    set(kHeadTensionParamId, preset.headTension);
    set(kStrikePositionParamId, preset.strikePosition);
    set(kBeaterHardnessParamId, preset.beaterHardness);
    set(kImpactParamId, preset.impact);
    set(kBodyParamId, preset.body);
    set(kBodyDecayParamId, preset.bodyDecaySeconds);
    set(kDampingParamId, preset.damping);
    set(kBloomParamId, preset.bloom);
    set(kAirParamId, preset.air);
    set(kShellParamId, preset.shell);
    set(kShellToneParamId, preset.shellTone);
    set(kMutedDecayParamId, preset.mutedDecaySeconds);
    set(kRimLevelParamId, preset.rimLevel);
    set(kRimToneParamId, preset.rimTone);
...
@interface S3GDrumConcertBassView : NSView {
    void* _plugin;
    int _dragParam;
...
@implementation S3GDrumConcertBassView
...
- (id)initWithPlugin:(void*)plugin
{
    ...
    if (self) {
        ...
        _factoryPresetIndex = p
            ? s3g::drumConcertBassFactoryPresetIndex(publishedParamsSnapshot(*p))
            : 0;
        ...
        if (_factoryPresetIndex >= 0) {
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                s3g::drumConcertBassFactoryPresetInfo(
...
- (void)applyFactoryPreset:(int)index
{
    ...
    std::snprintf(_presetName, sizeof(_presetName), "%s",
        s3g::drumConcertBassFactoryPresetInfo(static_cast<uint32_t>(index)).name);
...
- (NSRect)openMenuRect
{
    ...
    if (_openMenu == kFactoryPresetMenuId) {
        ...
        const auto row = std::find_if(kUiRows.begin(), kUiRows.end(),
            [](const ConcertBassUiRow& candidate) {
    ...
    style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        items[index] = [NSString stringWithUTF8String:
            s3g::drumConcertBassFactoryPresetInfo(index).name];
    }
...
- (void)refresh:(NSTimer*)timer
{
    ...
    if (p) {
        ...
        if (revision != _observedParamRevision) {
            ...
            _factoryPresetIndex = s3g::drumConcertBassFactoryPresetIndex(
                publishedParamsSnapshot(*p));
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                _factoryPresetIndex >= 0
                    ? s3g::drumConcertBassFactoryPresetInfo(static_cast<uint32_t>(
...
- (void)drawRect:(NSRect)dirty
{
    ...
    const NSRect attackPanel = NSMakeRect(kRightPanelX, 50.0,
        kPanelWidth, 286.0);
    const NSRect characterPanel = NSMakeRect(kLeftPanelX, 368.0,
        kPanelWidth, 218.0);
    const NSRect outputPanel = NSMakeRect(kRightPanelX, 368.0,
        kPanelWidth, 320.0);
    ...
    drawPanel(@"PRESSURE / BODY", voicePanel);
    drawPanel(@"EXCITER / ARTICULATIONS", attackPanel);
    ...
    const NSRect centerPad = NSMakeRect(kRightPanelX + 12.0, 510.0,
        padWidth, 24.0);
    const NSRect mutedPad = NSMakeRect(NSMaxX(centerPad) + 6.0, 510.0,
        padWidth, 24.0);
    const NSRect rimPad = NSMakeRect(NSMaxX(mutedPad) + 6.0, 510.0,
        padWidth, 24.0);
    s3g::clap_gui::drawHeaderButton(centerPad, outputPanel, @"CENTER",
        visualTrigger == 1u && activity > 0.02f, valueAttrs, style);
    s3g::clap_gui::drawHeaderButton(mutedPad, outputPanel, @"MUTED",
    ...
    const NSPoint center = NSMakePoint(687.0, 608.0);
    ...
    [@"SPECTRAL PRESSURE  //  KETTLE  //  TAIKO"
        drawAtPoint:NSMakePoint(kRightPanelX + 64.0, 678.0)
...
- (void)updateDraggedParam:(NSPoint)point
{
    ...
    const auto row = std::find_if(kUiRows.begin(), kUiRows.end(),
        [=](const ConcertBassUiRow& candidate) { return candidate.id == id; });
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(point,
        ...
        _menuItemCount = std::min<uint32_t>(
            s3g::kDrumConcertBassFactoryPresetCount, 64u);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(point,
        ...
        const bool loaded = s3g::clap_gui::loadPluginStatePreset(
            &p->plugin, @"Drum Concert Bass", &name);
    ...
    const NSRect rimPad = NSMakeRect(NSMaxX(mutedPad) + 6.0, 510.0,
        padWidth, 24.0);
    if (NSPointInRect(point, centerPad) || NSPointInRect(point, mutedPad)
        || NSPointInRect(point, rimPad)) {
        const uint32_t code = NSPointInRect(point, rimPad) ? 3u
            : (NSPointInRect(point, mutedPad) ? 2u : 1u);
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    p->guiView = [[S3GDrumConcertBassView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = false;
    [static_cast<S3GDrumConcertBassView*>(p->guiView) stopRefreshTimer];
...
bool guiShow(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = true;
    [static_cast<S3GDrumConcertBassView*>(p->guiView) startRefreshTimer];
    return true;
}
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_concert_bass_vstgui.inc"
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "An original mono-compatible stereo concert bass drum synthesizer with normalized stochastic pressure bands, tonal anchors, muted, rim, and reusable drum character stages.",
    features
};
