...
constexpr const char* portableTitle = "s3g AMBI ENCODER WIND";
constexpr const char* portablePresetDirectory = "Ambi Wind Encoder";
constexpr const char* portablePresetExtension = "s3gwind";
#define S3G_ENVIRONMENT_KIND 0
struct Plugin;
...
void publishInactiveTelemetry(Plugin& instance)
{
    ...
    const uint32_t voices =
        std::min<uint32_t>(p->engine.processingVoiceCount(), s3g::kAmbiWindMaxVoices);
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "../common/s3g_environment_encoder_wind_canvas.inc"
#endif
