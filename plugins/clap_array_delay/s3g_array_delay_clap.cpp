...
#define S3G_ARRAY_DELAY_VIEW_CLASS \
    S3G_OBJC_CLASS_JOIN(S3GArrayDelayView, S3G_ARRAY_DELAY_CHANNELS)
#define S3G_ARRAY_DELAY_TEXT_CLASS \
    S3G_OBJC_CLASS_JOIN(S3GArrayDelayNumberTextField, S3G_ARRAY_DELAY_CHANNELS)
...
@interface S3G_ARRAY_DELAY_TEXT_CLASS : NSTextField
@end

@implementation S3G_ARRAY_DELAY_TEXT_CLASS
...
@interface S3G_ARRAY_DELAY_VIEW_CLASS : NSView {
@private
    Plugin* _plugin;
    int _dragControl;
    int _dragChannel;
...
@implementation S3G_ARRAY_DELAY_VIEW_CLASS
- (instancetype)initWithPlugin:(Plugin*)plugin
{
    ...
    if (self) {
        ...
        for (uint32_t row = 0; row < kRowsPerPage; ++row) {
            NSTextField* field = [[S3G_ARRAY_DELAY_TEXT_CLASS alloc] initWithFrame:NSZeroRect];
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    s3g::clap_gui::drawArrayTitleBand(
        [NSString stringWithFormat:@"s3g ARRAY DELAY %uCH", kChannelCount],
    ...
    [self clampPage];
    [self drawDelayRows:s3g::clap_gui::cocoaRect(kArrayLayout.editor.frame)
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (s3g::clap_gui::handleProcessorTitleClick(
            pt, &_plugin->plugin, @"Array Delay", titleBand,
    ...
    for (uint32_t row = 0; row < n; ++row) {
        ...
        if (NSPointInRect(pt, NSMakeRect(
            ...
            const clap_id param = kDelayParamBaseId + ch;
            double defaultValue = 0.0;
            if (s3g::clap_gui::sliderDoubleClickDefault(
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool isFloating) { if (!guiIsApiSupported(plugin, api, isFloating)) return false; auto* p = self(plugin); if (p->guiView) return true; p->guiView = [[S3G_ARRAY_DELAY_VIEW_CLASS alloc] initWithPlugin:p]; if (!p->guiView) return false; if (!s3g::clap_gui::createResponsiveViewport(p->guiViewport, static_cast<NSView*>(p->guiView), kGuiWidth, kGuiHeight)) { [static_cast<NSView*>(p->guiView) release]; p->guiView = nullptr; return false; } return true; }
void guiDestroy(const clap_plugin_t* plugin) { auto* p = self(plugin); if (p && p->guiView) { p->guiVisible.store(false, std::memory_order_relaxed); [static_cast<S3G_ARRAY_DELAY_VIEW_CLASS*>(p->guiView) stopRefreshTimer]; s3g::clap_gui::destroyResponsiveViewport(p->guiViewport, p->guiView); } }
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible.store(true, std::memory_order_relaxed); [static_cast<S3G_ARRAY_DELAY_VIEW_CLASS*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible.store(false, std::memory_order_relaxed); [static_cast<S3G_ARRAY_DELAY_VIEW_CLASS*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
