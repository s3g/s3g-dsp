...
#if defined(S3G_USE_LEGACY_COCOA_MACRO_SHRED_GUI) && defined(__APPLE__)
...
#elif defined(S3G_USE_LEGACY_COCOA_MACRO_SHRED_GUI) && defined(__APPLE__)
...
#if defined(S3G_ENABLE_VSTGUI_MACRO_EFFECT_GUI) || (defined(S3G_USE_LEGACY_COCOA_MACRO_SHRED_GUI) && defined(__APPLE__))
...
{
    return static_cast<Plugin*>(context)->loopActivity.load(
        std::memory_order_relaxed);
}

float guiReadReduction(void* context)
{
    return static_cast<Plugin*>(context)->governorReduction.load(
        std::memory_order_relaxed);
}

float guiReadFrequency(void* context)
{
    return static_cast<Plugin*>(context)->loopFrequencyHz.load(
        std::memory_order_relaxed);
}
...
bool guiLoadPreset(void* context, const char* path)
{
    ...
    if (!read || !closed || state.version != kStateVersion) return false;
    auto& plugin = *static_cast<Plugin*>(context);
    const float output = plugin.params.outputGainDb;
    plugin.params = state.params;
    plugin.params.outputGainDb = output;
    plugin.shred.setParams(plugin.params);
    plugin.params = plugin.shred.params();
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    return s3g::clap_gui::portable::create(
        p->guiEditor, api, floating, [p]() {
            ...
            config.callbacks.getSecondaryActivity = guiReadReduction;
            config.callbacks.getFrequencyHz = guiReadFrequency;
            ...
            config.kind = s3g::portable_gui::MacroEffectKind::Shred;
            config.pluginName = S3G_MACRO_SHRED_PLUGIN_NAME;
