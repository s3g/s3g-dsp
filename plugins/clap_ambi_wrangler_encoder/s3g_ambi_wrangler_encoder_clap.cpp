...
constexpr clap_id kSpreadParamId = 5;
constexpr clap_id kDetuneParamId = 6;
constexpr clap_id kChaosParamId = 7;
constexpr clap_id kCrossParamId = 8;
constexpr clap_id kRungParamId = 9;
constexpr clap_id kRungSizeParamId = 10;
constexpr clap_id kThresholdParamId = 11;
constexpr clap_id kColorParamId = 12;
constexpr clap_id kFilterParamId = 13;
constexpr clap_id kResonanceParamId = 14;
constexpr clap_id kSaturationParamId = 15;
constexpr clap_id kTopologyShapeParamId = 16;
constexpr clap_id kTopologyMotionParamId = 17;
constexpr clap_id kTopologyRateParamId = 18;
constexpr clap_id kTopologyAmountParamId = 19;
constexpr clap_id kTopologyDepthParamId = 20;
constexpr clap_id kTopologyScaleParamId = 21;
constexpr clap_id kTopologyCollapseParamId = 22;
constexpr clap_id kAzimuthParamId = 23;
constexpr clap_id kElevationParamId = 24;
constexpr clap_id kDistanceParamId = 25;
constexpr clap_id kSpatialFollowParamId = 26;
constexpr clap_id kOutputParamId = 27;
...
bool assignParam(s3g::AmbiWranglerParams& params, clap_id id, double value)
{
    ...
    case kBaseNoteParamId: params.baseNote = static_cast<float>(value); return true;
    case kSpreadParamId: params.spreadSemitones = static_cast<float>(value); return true;
    case kDetuneParamId: params.detuneCents = static_cast<float>(value); return true;
    case kChaosParamId: params.chaos = static_cast<float>(value); return true;
    case kCrossParamId: params.cross = static_cast<float>(value); return true;
    case kRungParamId: params.rung = static_cast<float>(value); return true;
...
constexpr ParamDef kParams[] {
    ...
    { kBaseNoteParamId, "Base", 0.0, 96.0, 34.0, false },
    { kSpreadParamId, "Spread", 0.0, 60.0, 24.0, false },
    { kDetuneParamId, "Detune", 0.0, 100.0, 12.0, false },
    { kChaosParamId, "Chaos", 0.0, 1.0, 0.58, false },
    { kCrossParamId, "Cross", 0.0, 1.0, 0.28, false },
    { kRungParamId, "Rung", 0.0, 1.0, 0.72, false },
    ...
    { kOutputParamId, "Output", -60.0, 12.0, -24.0, false },
};
...
bool paramsGetValue(const clap_plugin_t* plugin, clap_id id, double* value)
{
    ...
    case kSpreadParamId: *value = params.spreadSemitones; return true;
    case kDetuneParamId: *value = params.detuneCents; return true;
    case kChaosParamId: *value = params.chaos; return true;
    case kCrossParamId: *value = params.cross; return true;
    case kRungParamId: *value = params.rung; return true;
...
bool paramsValueToText(const clap_plugin_t*, clap_id id, double value, char* display, uint32_t size)
{
    ...
    } else if (id == kBaseNoteParamId || id == kSpreadParamId) {
    ...
    } else if (id == kChaosParamId || id == kCrossParamId || id == kRungParamId || id == kThresholdParamId
        || id == kColorParamId || id == kFilterParamId || id == kResonanceParamId || id == kSaturationParamId
...
bool paramsTextToValue(const clap_plugin_t*, clap_id id, const char* display, double* value)
{
    ...
    *value = std::atof(display);
    if (id == kChaosParamId || id == kCrossParamId || id == kRungParamId || id == kThresholdParamId
        || id == kColorParamId || id == kFilterParamId || id == kResonanceParamId || id == kSaturationParamId
        || id == kTopologyAmountParamId || id == kTopologyDepthParamId || id == kTopologyCollapseParamId
        || id == kSpatialFollowParamId) {
        *value *= 0.01;
...
bool stateSave(const clap_plugin_t* plugin, const clap_ostream_t* stream)
{
    ...
    auto* p = self(plugin);
    const SavedState state { kStateVersion, p->params, p->presetIndex };
...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream)
{
    ...
    SavedState state {};
    if (!readExact(stream, &state.version, sizeof(state.version))) return false;
...
constexpr GuiSliderSpec kGuiSliders[] {
    ...
    { kBaseNoteParamId, 630, 156, 0.0, 96.0, false },
    { kSpreadParamId, 630, 182, 0.0, 60.0, false },
    { kDetuneParamId, 630, 208, 0.0, 100.0, false },
    { kChaosParamId, 630, 282, 0.0, 1.0, false },
    { kCrossParamId, 630, 308, 0.0, 1.0, false },
    { kRungParamId, 630, 334, 0.0, 1.0, false },
    { kRungSizeParamId, 630, 360, 2.0, 8.0, false },
    { kThresholdParamId, 630, 386, 0.0, 1.0, false },
    { kColorParamId, 630, 460, 0.0, 1.0, false },
    { kFilterParamId, 630, 486, 0.0, 1.0, false },
    { kResonanceParamId, 630, 512, 0.0, 1.0, false },
    { kSaturationParamId, 630, 538, 0.0, 1.0, false },
    { kOutputParamId, 630, 612, -60.0, 12.0, false },
...
- (void)drawField:(NSDictionary*)attrs valueAttrs:(NSDictionary*)valueAttrs style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t voice = 0; voice < voices; ++voice) {
        ...
        const CGFloat size = (selected ? base + 5.0 : base) + std::clamp(std::sqrt(std::max(0.0f, energy)) * 34.0f, 0.0f, 8.0f);
        const NSRect marker = NSMakeRect(projected[voice].x - size * 0.5, projected[voice].y - size * 0.5, size, size);
        [[self voiceColor:voice selected:selected] setFill];
        NSRectFill(marker);
        [s3g::clap_gui::color(selected ? 0xe6e6e6 : 0x4f4f4f) setStroke];
        NSFrameRect(marker);
    ...
    NSString* readout = [NSString stringWithFormat:@"V%02u  AZ%+.1f  EL%+.1f  D%.2f  E%.3f", _selectedVoice + 1u, az, el, dist, energy];
...
- (void)drawPanels:(NSDictionary*)attrs valueAttrs:(NSDictionary*)valueAttrs style:(const s3g::clap_gui::Style&)style
{
    ...
    [self drawSlider:@"BASE" param:kBaseNoteParamId value:p.baseNote attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"SPREAD" param:kSpreadParamId value:p.spreadSemitones attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"DETUNE" param:kDetuneParamId value:p.detuneCents attrs:attrs valueAttrs:valueAttrs style:style];

    s3g::clap_gui::drawPanelFrame(630, 256, 250, 166, style);
    s3g::clap_gui::drawPanelHeader(@"RUNGLER", true, 630, 256, 250, 21, attrs, style);
    [self drawSlider:@"CHAOS" param:kChaosParamId value:p.chaos attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"CROSS" param:kCrossParamId value:p.cross attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"RUNG" param:kRungParamId value:p.rung attrs:attrs valueAttrs:valueAttrs style:style];
    ...
    s3g::clap_gui::drawPanelFrame(630, 434, 250, 134, style);
    s3g::clap_gui::drawPanelHeader(@"MATERIAL", true, 630, 434, 250, 21, attrs, style);
    [self drawSlider:@"COLOR" param:kColorParamId value:p.color attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"FILTER" param:kFilterParamId value:p.filter attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"RES" param:kResonanceParamId value:p.resonance attrs:attrs valueAttrs:valueAttrs style:style];
    [self drawSlider:@"SAT" param:kSaturationParamId value:p.saturation attrs:attrs valueAttrs:valueAttrs style:style];

    s3g::clap_gui::drawPanelFrame(630, 582, 250, 72, style);
    s3g::clap_gui::drawPanelHeader(@"OUTPUT", true, 630, 582, 250, 21, attrs, style);
...
- (uint32_t)menuCount:(int)menu
{
    ...
    case 2: return 7u;
    case 3: return s3g::kTopologyShapeCount;
    case 4: return s3g::kTopologyMotionModeCount;
    default: return 0u;
    }
}
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (_openMenu > 0) {
        ...
        if (hit >= 0) {
            ...
            else if (_openMenu == 3) applyParam(*_plugin, kTopologyShapeParamId, hit);
            else if (_openMenu == 4) applyParam(*_plugin, kTopologyMotionParamId, hit);
        }
        _openMenu = 0;
        _hoverMenuItem = -1;
        [self setNeedsDisplay:YES];
    ...
    if (NSPointInRect(point, NSMakeRect(1004, 77, 124, 15))) { [self openMenu:3]; return; }
    if (NSPointInRect(point, NSMakeRect(1004, 103, 124, 15))) { [self openMenu:4]; return; }
    const NSRect panel = [self fieldPanelRect];
