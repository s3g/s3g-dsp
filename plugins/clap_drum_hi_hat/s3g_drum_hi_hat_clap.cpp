...
constexpr clap_id kAlloyParamId = 3u;
constexpr clap_id kSpreadParamId = 4u;
constexpr clap_id kDensityParamId = 5u;
constexpr clap_id kToneParamId = 6u;
constexpr clap_id kAirParamId = 7u;
constexpr clap_id kAttackParamId = 8u;
constexpr clap_id kClosedDecayParamId = 9u;
constexpr clap_id kOpenDecayParamId = 10u;
constexpr clap_id kPedalDecayParamId = 11u;
constexpr clap_id kWashParamId = 12u;
constexpr clap_id kChickParamId = 13u;
constexpr clap_id kChickToneParamId = 14u;
constexpr clap_id kSizzleParamId = 15u;
constexpr clap_id kChokeTimeParamId = 16u;
...
constexpr std::array<ParamDef, kParamCount> kParamDefs {{
    { kTuneParamId, "Tune", "Metal", 320.0, 3200.0, 1180.0, false },
    { kNoteTrackingParamId, "Note Tracking", "Metal / MIDI", 0.0, 1.0, 0.35, false },
    { kAlloyParamId, "Alloy", "Metal", 0.0, 1.0, 0.62, false },
    { kSpreadParamId, "Spread", "Metal", 0.0, 1.0, 0.58, false },
    { kDensityParamId, "Density", "Metal", 0.0, 1.0, 0.72, false },
    { kToneParamId, "Tone", "Wash", 0.0, 1.0, 0.62, false },
    { kAirParamId, "Air", "Wash", 0.0, 1.0, 0.55, false },
    { kAttackParamId, "Attack", "Contact", 0.0, 1.0, 0.66, false },
    { kClosedDecayParamId, "Closed Decay", "Articulation", 0.03, 1.0, 0.18, false },
    { kOpenDecayParamId, "Open Decay", "Articulation", 0.12, 6.0, 1.35, false },
    { kPedalDecayParamId, "Pedal Decay", "Articulation", 0.03, 1.5, 0.30, false },
    { kWashParamId, "Wash", "Articulation", 0.0, 1.0, 0.58, false },
    { kChickParamId, "Chick", "Articulation", 0.0, 1.0, 0.52, false },
    { kChickToneParamId, "Chick Tone", "Articulation", 0.0, 1.0, 0.56, false },
    { kSizzleParamId, "Sizzle", "Articulation", 0.0, 1.0, 0.28, false },
    { kChokeTimeParamId, "Choke Time", "Articulation", 0.3, 80.0, 7.0, false },
...
struct Plugin {
    ...
    std::atomic<uint32_t> pendingClosedTriggers { 0u };
    std::atomic<uint32_t> pendingOpenTriggers { 0u };
    std::atomic<uint32_t> pendingPedalTriggers { 0u };
...
s3g::DrumHiHatParams publishedParamsSnapshot(const Plugin& p)
{
    s3g::DrumHiHatParams params;
    ...
    params.tone = static_cast<float>(paramValue(p, kToneParamId));
    params.air = static_cast<float>(paramValue(p, kAirParamId));
    params.attack = static_cast<float>(paramValue(p, kAttackParamId));
    params.closedDecaySeconds = static_cast<float>(
        paramValue(p, kClosedDecayParamId));
    params.openDecaySeconds = static_cast<float>(
        paramValue(p, kOpenDecayParamId));
    params.pedalDecaySeconds = static_cast<float>(
        paramValue(p, kPedalDecayParamId));
    params.wash = static_cast<float>(paramValue(p, kWashParamId));
    params.chick = static_cast<float>(paramValue(p, kChickParamId));
    params.chickTone = static_cast<float>(paramValue(p, kChickToneParamId));
    params.sizzle = static_cast<float>(paramValue(p, kSizzleParamId));
    params.chokeTimeMs = static_cast<float>(
        paramValue(p, kChokeTimeParamId));
...
uint64_t tailSamplesForParams(const s3g::DrumHiHatParams& params,
    double sampleRate)
{
    const double seconds = std::max({
        s3g::drumHiHatTailSeconds(params,
            s3g::DrumHiHatArticulation::Closed, sampleRate),
        s3g::drumHiHatTailSeconds(params,
            s3g::DrumHiHatArticulation::Open, sampleRate),
        s3g::drumHiHatTailSeconds(params,
            s3g::DrumHiHatArticulation::Pedal, sampleRate),
    });
    ...
    bool triggerImmediately = false)
{
    ...
    if (id == kTriggerParamId) {
        ...
        if (code > 0u && code != p.triggerCode) {
            const auto articulation = code == 3u
                ...
                : code == 2u ? s3g::DrumHiHatArticulation::Open
                    : s3g::DrumHiHatArticulation::Closed;
            if (triggerImmediately) {
                triggerHiHat(p, articulation, 1.0f,
                    s3g::drumHiHatCanonicalMidiNote(articulation));
            } else {
                auto& pending = code == 3u ? p.pendingPedalTriggers
                    : code == 2u ? p.pendingOpenTriggers
                        : p.pendingClosedTriggers;
    ...
    case kAlloyParamId: p.params.alloy = v; break;
    case kSpreadParamId: p.params.spread = v; break;
    case kDensityParamId:
        tailChanged = std::fabs(p.params.density - v) > 1.0e-6f;
        p.params.density = v;
        break;
    case kToneParamId: p.params.tone = v; break;
    case kAirParamId: p.params.air = v; break;
    case kAttackParamId: p.params.attack = v; break;
    case kClosedDecayParamId:
        tailChanged = std::fabs(
            p.params.closedDecaySeconds - v) > 1.0e-6f;
        p.params.closedDecaySeconds = v;
        break;
    case kOpenDecayParamId:
        tailChanged = std::fabs(
            p.params.openDecaySeconds - v) > 1.0e-6f;
        p.params.openDecaySeconds = v;
        break;
    case kPedalDecayParamId:
        tailChanged = std::fabs(
            p.params.pedalDecaySeconds - v) > 1.0e-6f;
        p.params.pedalDecaySeconds = v;
        break;
    case kWashParamId:
        tailChanged = std::fabs(p.params.wash - v) > 1.0e-6f;
        p.params.wash = v;
        break;
    case kChickParamId:
        tailChanged = std::fabs(p.params.chick - v) > 1.0e-6f;
        p.params.chick = v;
        break;
    case kChickToneParamId:
        tailChanged = std::fabs(p.params.chickTone - v) > 1.0e-6f;
        p.params.chickTone = v;
        break;
    case kSizzleParamId:
        tailChanged = std::fabs(p.params.sizzle - v) > 1.0e-6f;
        p.params.sizzle = v;
        break;
    case kChokeTimeParamId: p.params.chokeTimeMs = v; break;
    case kDriveParamId:
        tailChanged = std::fabs(
...
double rawParamValue(const Plugin& p, clap_id id)
{
    ...
    case kClosedDecayParamId: return p.params.closedDecaySeconds;
    case kOpenDecayParamId: return p.params.openDecaySeconds;
    case kPedalDecayParamId: return p.params.pedalDecaySeconds;
    case kWashParamId: return p.params.wash;
    case kChickParamId: return p.params.chick;
    case kChickToneParamId: return p.params.chickTone;
    case kSizzleParamId: return p.params.sizzle;
...
void assignSavedStateValues(s3g::DrumHiHatParams& params,
    const std::array<double, kSavedParamCount>& values)
{
    ...
    params.tone = value(kToneParamId);
    params.air = value(kAirParamId);
    params.attack = value(kAttackParamId);
    params.closedDecaySeconds = value(kClosedDecayParamId);
    params.openDecaySeconds = value(kOpenDecayParamId);
    params.pedalDecaySeconds = value(kPedalDecayParamId);
    params.wash = value(kWashParamId);
    params.chick = value(kChickParamId);
    params.chickTone = value(kChickToneParamId);
    params.sizzle = value(kSizzleParamId);
...
bool consumePendingState(Plugin& p)
{
    ...
    s3g::DrumHiHatParams next = p.params;
    ...
    if (event->type == CLAP_EVENT_PARAM_VALUE
        ...
        if ((midi->data[0] & 0xf0u) == 0x90u && midi->data[2] > 0u
            ...
            const auto articulation = note == 46
                ? s3g::DrumHiHatArticulation::Open
                : note == 44 ? s3g::DrumHiHatArticulation::Pedal
                    : s3g::DrumHiHatArticulation::Closed;
            triggerHiHat(p, articulation,
    ...
    const clap_process_t* process)
{
    ...
    uint32_t pending = p->pendingClosedTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerHiHat(*p,
        s3g::DrumHiHatArticulation::Closed, 1.0f, 42);
    pending = p->pendingOpenTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerHiHat(*p,
        s3g::DrumHiHatArticulation::Open, 1.0f, 46);
    pending = p->pendingPedalTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerHiHat(*p,
        s3g::DrumHiHatArticulation::Pedal, 1.0f, 44);
    ...
    for (uint32_t sample = 0u; sample < process->frames_count; ++sample) {
        ...
        float left = 0.0f;
        float right = 0.0f;
        p->hiHat.processFrame(left, right);
...
struct HiHatNoteName {
    int16_t key;
    const char* name;
};

constexpr std::array<HiHatNoteName, 3u> kHiHatNoteNames {{
    { 42, "Closed Hi-Hat" },
    { 44, "Pedal Hi-Hat" },
    { 46, "Open Hi-Hat" },
}};
...
{
    return static_cast<uint32_t>(kHiHatNoteNames.size());
}

bool noteNameGet(const clap_plugin_t*, uint32_t index,
    clap_note_name_t* noteName)
{
    if (!noteName || index >= kHiHatNoteNames.size()) return false;
    *noteName = {};
    noteName->port = 0;
    noteName->key = kHiHatNoteNames[index].key;
    noteName->channel = -1;
    std::strncpy(noteName->name, kHiHatNoteNames[index].name,
    ...
    char* display, uint32_t size)
{
    ...
    if (id == kTuneParamId) {
    ...
    } else if (id == kClosedDecayParamId || id == kOpenDecayParamId
        ...
        std::snprintf(display, size, "%s",
            code == 3u ? "Pedal" : (code == 2u ? "Open"
                : (code == 1u ? "Closed" : "Ready")));
    } else {
        std::snprintf(display, size, "%.0f%%", value * 100.0);
    }
    return true;
...
constexpr std::array<HiHatUiRow, kSavedParamCount> kUiRows {{
    { kTuneParamId, "TUNE", kLeftPanelX, kPanelWidth, 80.0 },
    { kNoteTrackingParamId, "NOTE TRACK", kLeftPanelX, kPanelWidth, 104.0 },
    { kAlloyParamId, "ALLOY", kLeftPanelX, kPanelWidth, 128.0 },
    { kSpreadParamId, "SPREAD", kLeftPanelX, kPanelWidth, 152.0 },
    { kDensityParamId, "DENSITY", kLeftPanelX, kPanelWidth, 176.0 },
    { kToneParamId, "TONE", kLeftPanelX, kPanelWidth, 200.0 },
    { kAirParamId, "AIR", kLeftPanelX, kPanelWidth, 224.0 },
    { kAttackParamId, "ATTACK", kLeftPanelX, kPanelWidth, 248.0 },

    { kClosedDecayParamId, "CLOSED DECAY", kRightPanelX, kPanelWidth, 80.0 },
    { kOpenDecayParamId, "OPEN DECAY", kRightPanelX, kPanelWidth, 104.0 },
    { kPedalDecayParamId, "PEDAL DECAY", kRightPanelX, kPanelWidth, 128.0 },
    { kWashParamId, "WASH", kRightPanelX, kPanelWidth, 152.0 },
    { kChickParamId, "CHICK", kRightPanelX, kPanelWidth, 176.0 },
    { kChickToneParamId, "CHICK TONE", kRightPanelX, kPanelWidth, 200.0 },
    { kSizzleParamId, "SIZZLE", kRightPanelX, kPanelWidth, 224.0 },
    { kChokeTimeParamId, "CHOKE TIME", kRightPanelX, kPanelWidth, 248.0 },
...
{
    return id == kTuneParamId || id == kClosedDecayParamId
        || id == kOpenDecayParamId || id == kPedalDecayParamId
        || id == kChokeTimeParamId;
}
...
double safeRandomParamValue(
    const s3g::DrumHiHatParams& params, clap_id id)
{
    switch (id) {
    ...
    case kToneParamId: return params.tone;
    case kAirParamId: return params.air;
    case kAttackParamId: return params.attack;
    case kClosedDecayParamId: return params.closedDecaySeconds;
    case kOpenDecayParamId: return params.openDecaySeconds;
    case kPedalDecayParamId: return params.pedalDecaySeconds;
    case kWashParamId: return params.wash;
    case kChickParamId: return params.chick;
    case kChickToneParamId: return params.chickTone;
    case kSizzleParamId: return params.sizzle;
...
bool queueGuiSafeRandomParamSet(Plugin& p,
    const s3g::DrumHiHatParams& sourceParams)
{
    using Kind = s3g::clap_gui::ParamEventKind;
...
{
    auto* p = &instance;
    if (s3g::kDrumHiHatFactoryPresetCount == 0u) return false;
    index = std::clamp(index, 0,
        static_cast<int>(s3g::kDrumHiHatFactoryPresetCount - 1u));
    const auto preset = s3g::drumHiHatFactoryPreset(
    ...
    set(kToneParamId, preset.tone);
    set(kAirParamId, preset.air);
    set(kAttackParamId, preset.attack);
    set(kClosedDecayParamId, preset.closedDecaySeconds);
    set(kOpenDecayParamId, preset.openDecaySeconds);
    set(kPedalDecayParamId, preset.pedalDecaySeconds);
    set(kWashParamId, preset.wash);
    set(kChickParamId, preset.chick);
    set(kChickToneParamId, preset.chickTone);
    set(kSizzleParamId, preset.sizzle);
...
@interface S3GDrumHiHatView : NSView {
    void* _plugin;
    int _dragParam;
...
@implementation S3GDrumHiHatView
...
- (id)initWithPlugin:(void*)plugin
{
    ...
    if (self) {
        ...
        _factoryPresetIndex = p
            ? s3g::drumHiHatFactoryPresetIndex(publishedParamsSnapshot(*p))
            : 0;
        ...
        if (_factoryPresetIndex >= 0) {
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                s3g::drumHiHatFactoryPresetInfo(
...
- (void)applyFactoryPreset:(int)index
{
    ...
    index = std::clamp(index, 0, static_cast<int>(s3g::kDrumHiHatFactoryPresetCount - 1u));
    ...
    std::snprintf(_presetName, sizeof(_presetName), "%s",
        s3g::drumHiHatFactoryPresetInfo(static_cast<uint32_t>(index)).name);
    ...
    style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        items[index] = [NSString stringWithUTF8String:
            s3g::drumHiHatFactoryPresetInfo(index).name];
    }
...
- (void)refresh:(NSTimer*)timer
{
    ...
    if (p) {
        ...
        if (revision != _observedParamRevision) {
            ...
            _factoryPresetIndex = s3g::drumHiHatFactoryPresetIndex(
                publishedParamsSnapshot(*p));
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                _factoryPresetIndex >= 0
                    ? s3g::drumHiHatFactoryPresetInfo(static_cast<uint32_t>(
...
- (void)drawRect:(NSRect)dirty
{
    ...
    drawPanel(@"METAL / WASH", voicePanel);
    drawPanel(@"ARTICULATION", attackPanel);
    ...
    for (uint32_t cymbal = 0u; cymbal < 2u; ++cymbal) {
        const CGFloat width = 116.0 + activity * (18.0 - cymbal * 4.0);
        const CGFloat height = 22.0 + activity * 7.0;
        const CGFloat offset = cymbal == 0u ? -12.0 - activity * 5.0
                                            : 12.0 + activity * 5.0;
        ...
        NSBezierPath* path = [NSBezierPath bezierPathWithOvalInRect:NSMakeRect(
            center.x - width * 0.5, center.y + offset - height * 0.5,
            width, height)];
    ...
    [stand moveToPoint:NSMakePoint(center.x, center.y - 55.0)];
    [stand lineToPoint:NSMakePoint(center.x, center.y + 62.0)];
    [stand setLineWidth:1.2];
    [stand stroke];
    for (uint32_t ray = 0u; ray < 8u; ++ray) {
        const CGFloat angle = static_cast<CGFloat>(ray) * s3g::kPi / 4.0;
        const CGFloat radius = 65.0 + activity * 13.0;
        NSBezierPath* path = [NSBezierPath bezierPath];
        [path moveToPoint:center];
        [path lineToPoint:NSMakePoint(
            center.x + std::cos(angle) * radius,
            center.y + std::sin(angle) * radius * 0.35)];
    ...
    [@"SYNTHESIZED CLOSED  //  OPEN  //  PEDAL"
        drawAtPoint:NSMakePoint(kRightPanelX + 64.0, 646.0)
...
- (void)updateDraggedParam:(NSPoint)point
{
    ...
    const auto row = std::find_if(kUiRows.begin(), kUiRows.end(),
        [=](const HiHatUiRow& candidate) { return candidate.id == id; });
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(point,
        ...
        _menuItemCount = std::min<uint32_t>(
            s3g::kDrumHiHatFactoryPresetCount, 64u);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(point,
    ...
    if (NSPointInRect(point, closedPad) || NSPointInRect(point, openPad)
        ...
        const uint32_t code = NSPointInRect(point, pedalPad) ? 3u
            : (NSPointInRect(point, openPad) ? 2u : 1u);
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    p->guiView = [[S3GDrumHiHatView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = false;
    [static_cast<S3GDrumHiHatView*>(p->guiView) stopRefreshTimer];
...
bool guiShow(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = true;
    [static_cast<S3GDrumHiHatView*>(p->guiView) startRefreshTimer];
    return true;
}
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_hi_hat_vstgui.inc"
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "A procedural mono-compatible stereo hi-hat synthesizer with closed, open, pedal and smooth choke articulations.",
    features
};
