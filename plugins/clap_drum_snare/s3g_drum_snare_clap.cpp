...
constexpr uint32_t kStateMagic = 0x53473353u; // "S3GS" in little endian.
...
constexpr clap_id kWiresParamId = 10u;
constexpr clap_id kWireToneParamId = 11u;
constexpr clap_id kWireTensionParamId = 12u;
constexpr clap_id kWireDecayParamId = 13u;
constexpr clap_id kRattleParamId = 14u;
constexpr clap_id kClickParamId = 15u;
constexpr clap_id kClickToneParamId = 16u;
...
constexpr std::array<ParamDef, kParamCount> kParamDefs {{
    { kTuneParamId, "Tune", "Shell", 70.0, 420.0, 180.0, false },
    { kNoteTrackingParamId, "Note Tracking", "Shell / MIDI", 0.0, 1.0, 0.75, false },
    { kPitchDropParamId, "Pitch Drop", "Shell / Pitch", -12.0, 36.0, 7.0, false },
    { kPitchSweepTimeParamId, "Sweep Time", "Shell / Pitch", 1.0, 250.0, 22.0, false },
    { kShellSpreadParamId, "Shell Spread", "Shell", 0.0, 1.0, 0.40, false },
    { kBodyParamId, "Body", "Shell", 0.0, 1.0, 0.58, false },
    { kRingParamId, "Ring", "Shell", 0.0, 1.0, 0.32, false },
    { kBodyDecayParamId, "Body Decay", "Shell", 0.02, 3.0, 0.42, false },
    { kPunchParamId, "Punch", "Attack", 0.0, 1.0, 0.78, false },
    { kWiresParamId, "Wires", "Wires", 0.0, 1.0, 0.70, false },
    { kWireToneParamId, "Wire Tone", "Wires", 0.0, 1.0, 0.56, false },
    { kWireTensionParamId, "Wire Tension", "Wires", 0.0, 1.0, 0.48, false },
    { kWireDecayParamId, "Wire Decay", "Wires", 0.01, 4.0, 0.34, false },
    { kRattleParamId, "Rattle", "Wires", 0.0, 1.0, 0.18, false },
    { kClickParamId, "Click", "Attack", 0.0, 1.0, 0.10, false },
    { kClickToneParamId, "Click Tone", "Attack", 0.0, 1.0, 0.60, false },
...
double boundedTailSeconds(double pitchSweepMs, double body, double ring,
    double bodyDecay, double punch, double wires, double wireDecay,
    double rattle, double click, double clickTone, double drive)
{
    const double longestMode = bodyDecay * std::max({
        0.72 + (1.28 - 0.72) * body,
        0.50 + (1.55 - 0.50) * ring,
        0.34 + (1.95 - 0.34) * ring,
        0.24 + (1.55 - 0.24) * ring,
    });
    const double pitchSeconds = pitchSweepMs * 0.001;
    const double impactSeconds = std::max(pitchSeconds * 0.75,
        0.032 + (0.005 - 0.032) * punch);
    const double clickSeconds = 0.010
        + (0.0015 - 0.010) * clickTone;
    return std::min(40.0, std::max({
        0.10,
        longestMode * 2.15,
        impactSeconds * 2.15,
        wires > 1.0e-6 ? wireDecay * 2.15 : 0.0,
        wires > 1.0e-6 && rattle > 1.0e-6
            ? wireDecay * (1.05 + (2.80 - 1.05) * rattle) * 2.15 : 0.0,
        click > 1.0e-6 ? clickSeconds * 2.15 : 0.0,
        drive > 1.0e-6 ? 0.50 : 0.0,
    }));
}

uint64_t tailSamplesForParams(const s3g::DrumSnareParams& params,
    double sampleRate)
{
    const double seconds = boundedTailSeconds(params.pitchSweepMs,
        params.body, params.ring, params.bodyDecaySeconds, params.punch,
        params.wires, params.wireDecaySeconds, params.rattle, params.click,
        params.clickTone, params.character.drive);
...
void triggerSnare(Plugin& p, float velocity, int midiNote)
{
    p.snare.trigger(std::clamp(velocity, 0.0f, 1.0f),
    ...
    bool triggerImmediately = false)
{
    ...
    case kPunchParamId:
        ...
        break;
    case kWiresParamId:
        tailChanged = std::fabs(p.params.wires - v) > 1.0e-6f;
        p.params.wires = v;
        break;
    case kWireToneParamId: p.params.wireTone = v; break;
    case kWireTensionParamId: p.params.wireTension = v; break;
    case kWireDecayParamId:
        tailChanged = std::fabs(
            p.params.wireDecaySeconds - v) > 1.0e-6f;
        p.params.wireDecaySeconds = v;
        break;
    case kRattleParamId:
        tailChanged = std::fabs(p.params.rattle - v) > 1.0e-6f;
        p.params.rattle = v;
        break;
    case kClickParamId:
        tailChanged = std::fabs(p.params.click - v) > 1.0e-6f;
        p.params.click = v;
        break;
    case kClickToneParamId:
        tailChanged = std::fabs(p.params.clickTone - v) > 1.0e-6f;
        p.params.clickTone = v;
        break;
    case kDriveParamId:
        tailChanged = std::fabs(
    ...
    p.snare.setParams(p.params);
    p.params = p.snare.params();
...
double rawParamValue(const Plugin& p, clap_id id)
{
    ...
    case kWiresParamId: return p.params.wires;
    case kWireToneParamId: return p.params.wireTone;
    case kWireTensionParamId: return p.params.wireTension;
    case kWireDecayParamId: return p.params.wireDecaySeconds;
...
void assignSavedStateValues(s3g::DrumSnareParams& params,
    const std::array<double, kSavedParamCount>& values)
{
    ...
    params.wires = value(kWiresParamId);
    params.wireTone = value(kWireToneParamId);
    params.wireTension = value(kWireTensionParamId);
    params.wireDecaySeconds = value(kWireDecayParamId);
    params.rattle = value(kRattleParamId);
    params.click = value(kClickParamId);
    params.clickTone = value(kClickToneParamId);
...
bool consumePendingState(Plugin& p)
{
    ...
    s3g::DrumSnareParams next = p.params;
    assignSavedStateValues(next, values);
    p.snare.setParams(next);
    p.params = p.snare.params();
...
void applyEvent(Plugin& p, const clap_event_header_t* event)
{
    ...
    if (event->type == CLAP_EVENT_PARAM_VALUE
        ...
        if (note->velocity > 0.0
            ...
            triggerSnare(p, static_cast<float>(note->velocity), note->key);
        }
        ...
        if ((midi->data[0] & 0xf0u) == 0x90u && midi->data[2] > 0u
            ...
            triggerSnare(p, static_cast<float>(midi->data[2]) / 127.0f,
                midi->data[1]);
        }
    }
}
    ...
    const clap_process_t* process)
{
    ...
    while (pending-- > 0u) triggerSnare(*p, 1.0f, 38);
    ...
    for (uint32_t sample = 0u; sample < process->frames_count; ++sample) {
        ...
        float left = 0.0f;
        float right = 0.0f;
        p->snare.processFrame(left, right);
    ...
    char* display, uint32_t size)
{
    ...
    } else if (id == kBodyDecayParamId || id == kWireDecayParamId) {
    ...
    const char* display, double* value)
{
    ...
    const bool percentParam = id != kTuneParamId
        ...
        && id != kBodyDecayParamId && id != kWireDecayParamId
        && id != kOutputParamId
    ...
    const char* expectedSuffix = id == kTuneParamId ? "Hz"
        ...
        : ((id == kBodyDecayParamId || id == kWireDecayParamId) ? "s"
...
{
    const auto* p = self(plugin);
    const double seconds = boundedTailSeconds(
        paramValue(*p, kPitchSweepTimeParamId),
        paramValue(*p, kBodyParamId), paramValue(*p, kRingParamId),
        paramValue(*p, kBodyDecayParamId), paramValue(*p, kPunchParamId),
        paramValue(*p, kWiresParamId), paramValue(*p, kWireDecayParamId),
        paramValue(*p, kRattleParamId), paramValue(*p, kClickParamId),
        paramValue(*p, kClickToneParamId), paramValue(*p, kDriveParamId));
...
constexpr std::array<SnareUiRow, kSavedParamCount> kUiRows {{
    ...
    { kWiresParamId, "WIRES", kRightPanelX, kPanelWidth, 104.0 },
    { kWireToneParamId, "WIRE TONE", kRightPanelX, kPanelWidth, 128.0 },
    { kWireTensionParamId, "WIRE TENSION", kRightPanelX, kPanelWidth, 152.0 },
    { kWireDecayParamId, "WIRE DECAY", kRightPanelX, kPanelWidth, 176.0 },
    { kRattleParamId, "RATTLE", kRightPanelX, kPanelWidth, 200.0 },
    { kClickParamId, "CLICK", kRightPanelX, kPanelWidth, 224.0 },
    { kClickToneParamId, "CLICK TONE", kRightPanelX, kPanelWidth, 248.0 },
...
s3g::DrumSnareParams publishedParamsSnapshot(const Plugin& p)
{
    s3g::DrumSnareParams params;
    ...
    params.wires = static_cast<float>(paramValue(p, kWiresParamId));
    params.wireTone = static_cast<float>(paramValue(p, kWireToneParamId));
    params.wireTension = static_cast<float>(
        paramValue(p, kWireTensionParamId));
    params.wireDecaySeconds = static_cast<float>(
        paramValue(p, kWireDecayParamId));
    params.rattle = static_cast<float>(paramValue(p, kRattleParamId));
...
double safeRandomParamValue(
    const s3g::DrumSnareParams& params, clap_id id)
{
    switch (id) {
    ...
    case kWiresParamId: return params.wires;
    case kWireToneParamId: return params.wireTone;
    case kWireTensionParamId: return params.wireTension;
    case kWireDecayParamId: return params.wireDecaySeconds;
    case kRattleParamId: return params.rattle;
    case kClickParamId: return params.click;
    case kClickToneParamId: return params.clickTone;
...
bool queueGuiSafeRandomParamSet(Plugin& p,
    const s3g::DrumSnareParams& sourceParams)
{
    using Kind = s3g::clap_gui::ParamEventKind;
...
{
    auto* p = &instance;
    if (s3g::kDrumSnareFactoryPresetCount == 0u) return false;
    index = std::clamp(index, 0,
        static_cast<int>(s3g::kDrumSnareFactoryPresetCount - 1u));
    const auto preset = s3g::drumSnareFactoryPreset(
    ...
    set(kWiresParamId, preset.wires);
    set(kWireToneParamId, preset.wireTone);
    set(kWireTensionParamId, preset.wireTension);
    set(kWireDecayParamId, preset.wireDecaySeconds);
    set(kRattleParamId, preset.rattle);
...
@interface S3GDrumSnareView : NSView {
    void* _plugin;
    int _dragParam;
...
@implementation S3GDrumSnareView
...
- (id)initWithPlugin:(void*)plugin
{
    ...
    if (self) {
        ...
        _factoryPresetIndex = p
            ? s3g::drumSnareFactoryPresetIndex(publishedParamsSnapshot(*p))
            : 0;
        ...
        if (_factoryPresetIndex >= 0) {
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                s3g::drumSnareFactoryPresetInfo(
...
- (void)applyFactoryPreset:(int)index
{
    ...
    std::snprintf(_presetName, sizeof(_presetName), "%s",
        s3g::drumSnareFactoryPresetInfo(static_cast<uint32_t>(index)).name);
    ...
    style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        items[index] = [NSString stringWithUTF8String:
            s3g::drumSnareFactoryPresetInfo(index).name];
    }
...
- (void)refresh:(NSTimer*)timer
{
    ...
    if (p) {
        ...
        if (revision != _observedParamRevision) {
            ...
            _factoryPresetIndex = s3g::drumSnareFactoryPresetIndex(
                publishedParamsSnapshot(*p));
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                _factoryPresetIndex >= 0
                    ? s3g::drumSnareFactoryPresetInfo(static_cast<uint32_t>(
...
- (void)drawRect:(NSRect)dirty
{
    ...
    drawPanel(@"SHELL / PITCH", voicePanel);
    drawPanel(@"ATTACK / WIRES", attackPanel);
    ...
    [@"SYNTHESIZED SHELL  //  STOCHASTIC WIRES"
...
- (void)updateDraggedParam:(NSPoint)point
{
    ...
    const auto row = std::find_if(kUiRows.begin(), kUiRows.end(),
        [=](const SnareUiRow& candidate) { return candidate.id == id; });
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(point,
        ...
        _menuItemCount = std::min<uint32_t>(
            s3g::kDrumSnareFactoryPresetCount, 64u);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(point,
    ...
    if (NSPointInRect(point,
        ...
        const auto randomized = s3g::drumSnareSafeRandomParams(
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    p->guiView = [[S3GDrumSnareView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = false;
    [static_cast<S3GDrumSnareView*>(p->guiView) stopRefreshTimer];
...
bool guiShow(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = true;
    [static_cast<S3GDrumSnareView*>(p->guiView) startRefreshTimer];
    return true;
}
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_snare_vstgui.inc"
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "A procedural mono-compatible stereo snare synthesizer with inharmonic shell modes, stochastic wires, impact, and reusable drum character stages.",
    features
};
