...
constexpr clap_id kParamGroupSize = 1010;
constexpr clap_id kParamShape = 1011;
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    double canonical=0.;
    if(id<p.routingValues.size()&&rawParamsGetValue(&p.plugin,id,&canonical))
        p.routingValues[id].store(canonical,std::memory_order_release);
#endif
}
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    p->routingPhase.store(p->matrix.previewPhase(),std::memory_order_relaxed);
#endif
    return CLAP_PROCESS_CONTINUE;
}
...
uint32_t paramsCount(const clap_plugin_t*) { return kCrosspointCount + 12; }
...
clap_id paramIdForIndex(uint32_t index)
{
    ...
    case 11: return kParamGroupSize;
...
bool paramsGetInfo(const clap_plugin_t*, uint32_t index, clap_param_info_t* info)
{
    ...
    case kParamGroupSize:
        std::strncpy(info->name, "Group Size", sizeof(info->name));
        info->min_value = 0.0; info->max_value = 2.0; break;
    default:
        return false;
    }
...
#endif
{
    ...
    case kParamGroupSize: *value = static_cast<double>(static_cast<uint32_t>(p.groupSize)); return true;
...
bool paramsTextToValue(const clap_plugin_t*, clap_id paramId, const char* display, double* value)
{
    ...
    if (paramId == kParamGroupSize) {
        for (uint32_t index = 0u; index <= 2u; ++index) {
            if (std::strcmp(display, s3g::groupMatrixSizeName(static_cast<s3g::GroupMatrixSize>(index))) == 0) {
                *value = static_cast<double>(index);
                return true;
            }
        }
...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream)
{
    ...
    for(float v:state.params.crosspointDb)if(!std::isfinite(v))return false;
    for(float v:{state.params.flow,state.params.spread,state.params.vortex,state.params.motion,state.params.rate,state.params.divisionBeats,state.params.phaseOffset,state.params.smoothingMs,state.params.outputGainDb})if(!std::isfinite(v))return false;
    auto* p = self(plugin);
...
- (NSRect)groupMenuBoxRect;
- (NSRect)groupDropdownRect;
...
- (NSRect)modeMenuBoxRect
{
    return NSMakeRect(
        s3g::gui_layout::processorControlX(
            kMatrixLayout.groupPattern.frame.x),
        s3g::gui_layout::rowY(kMatrixLayout.groupPattern, 2u) - 1.0,
        s3g::gui_layout::processorMenuWidth(
            kMatrixLayout.groupPattern.frame.width),
        17.0);
}
- (NSRect)modeDropdownRect
{
...
- (NSRect)shapeMenuBoxRect
{
    return NSMakeRect(
        s3g::gui_layout::processorControlX(
            kMatrixLayout.groupPattern.frame.x),
        s3g::gui_layout::rowY(kMatrixLayout.groupPattern, 1u) - 1.0,
        s3g::gui_layout::processorMenuWidth(
            kMatrixLayout.groupPattern.frame.width),
        17.0);
}
...
- (NSRect)groupMenuBoxRect
{
    return NSMakeRect(
        s3g::gui_layout::processorControlX(
            kMatrixLayout.groupPattern.frame.x),
        s3g::gui_layout::rowY(kMatrixLayout.groupPattern, 0u) - 1.0,
        s3g::gui_layout::processorMenuWidth(
            kMatrixLayout.groupPattern.frame.width),
        17.0);
}
- (NSRect)groupDropdownRect
{
    NSRect box = [self groupMenuBoxRect];
    return NSMakeRect(box.origin.x, NSMaxY(box) + 1.0,
        box.size.width, 60.0);
}
- (NSRect)randomDevSliderRect
...
- (void)randomizeMatrix
{
    ...
    for (uint32_t src = 0; src < groups; ++src) {
        for (uint32_t dst = 0; dst < groups; ++dst) {
            ...
            applyParam(*p, kCrosspointBase + s3g::groupMatrixIndex(src, dst), db);
        }
    }
    [self setNeedsDisplay:YES];
}
- (void)updateRandomDev:(NSPoint)pt
{
    _randomDev = std::clamp((pt.x - [self randomDevSliderRect].origin.x) / [self randomDevSliderRect].size.width, 0.0, 1.0);
    [self setNeedsDisplay:YES];
}
- (void)drawOpenMenu:(NSDictionary*)attrs style:(const s3g::clap_gui::Style&)style
{
...
- (void)updateMenuHover:(NSPoint)pt
{
    ...
    const NSRect rect = _openMenu == 1 ? [self groupDropdownRect] : (_openMenu == 2 ? [self modeDropdownRect] : [self shapeDropdownRect]);
    const uint32_t count = _openMenu == 1 ? 3u : (_openMenu == 2 ? 2u : 6u);
...
- (void)drawFlowPreview:(NSRect)rect attrs:(NSDictionary*)attrs style:(const s3g::clap_gui::Style&)style
{
    ...
    const CGFloat rowGap = groups > 1u ? (rect.size.height - 60.0) / static_cast<CGFloat>(groups - 1u) : 0.0;
    ...
    for (uint32_t i = 0; i < groups; ++i) {
        ...
        [style.strip setFill];
        const CGFloat box = groups > 8u ? 12.0 : 20.0;
        NSRectFill(NSMakeRect(srcPt.x - box * 0.5, srcPt.y - box * 0.5, box, box));
        NSRectFill(NSMakeRect(dstPt.x - box * 0.5, dstPt.y - box * 0.5, box, box));
        [style.grid setStroke];
        NSFrameRect(NSMakeRect(srcPt.x - box * 0.5, srcPt.y - box * 0.5, box, box));
        NSFrameRect(NSMakeRect(dstPt.x - box * 0.5, dstPt.y - box * 0.5, box, box));
        if (groups <= 8u) {
...
- (void)drawRect:(NSRect)dirty
{
    ...
    drawPanel(@"PATTERN", kMatrixLayout.groupPattern);
    ...
    for (uint32_t src = 0; src < groups; ++src) {
        for (uint32_t dst = 0; dst < groups; ++dst) {
            ...
            if (manualNorm > 0.01) {
                const CGFloat inset = groups > 8u ? 3.0 : 7.0;
            ...
            if (groups <= 8u) {
                [label drawAtPoint:NSMakePoint(r.origin.x + 11.0, r.origin.y + r.size.height * 0.42) withAttributes:text];
            }
        }
    }
    ...
    const auto menu = [&](NSString* label, NSString* value,
        ...
        s3g::clap_gui::drawProcessorMenu(
            ...
            s3g::gui_layout::rowY(kMatrixLayout.groupPattern, row),
            kMatrixLayout.groupPattern.frame.x,
            kMatrixLayout.groupPattern.frame.width,
            small, small, style);
    };
    ...
    s3g::clap_gui::drawProcessorSlider(
        ...
        small, small, style);
    menu(@"GROUP", [NSString stringWithUTF8String:
        s3g::groupMatrixSizeName(prm.groupSize)], 0u);
    menu(@"SHAPE", [NSString stringWithUTF8String:
        s3g::matrixFlowShapeName(prm.shape)], 1u);
    menu(@"MODE", prm.mode == s3g::GroupMatrixFlowMode::Sync
        ? @"SYNC" : @"FREE", 2u);
    slider(@"DEPTH", [NSString stringWithFormat:@"%.0f%%",
        static_cast<double>(prm.flow * 100.0f)], prm.flow, 3u);
    slider(@"SPREAD", [NSString stringWithFormat:@"%.0f%%",
        static_cast<double>(prm.spread * 100.0f)], prm.spread, 4u);
    slider(@"VORTEX", [NSString stringWithFormat:@"%+.2f",
        static_cast<double>(prm.vortex)],
        (prm.vortex + 1.0f) * 0.5f, 5u);
    slider(@"MOTION", [NSString stringWithFormat:@"%.0f%%",
        static_cast<double>(prm.motion * 100.0f)], prm.motion, 6u);
    slider(@"RATE", [NSString stringWithFormat:@"%.0f%%",
        static_cast<double>(prm.rate * 100.0f)], prm.rate, 7u);
    slider(@"DIVISION", [NSString stringWithFormat:@"%.2g",
        static_cast<double>(prm.divisionBeats)],
        (prm.divisionBeats - 0.25f) / 63.75f, 8u);
    ...
    slider(@"SMOOTHING", [NSString stringWithFormat:@"%.0f ms",
        ...
        (prm.smoothingMs - 1.0f) / 499.0f, 10u);
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (_openMenu > 0) {
        const NSRect menuRect = _openMenu == 1 ? [self groupDropdownRect] : (_openMenu == 2 ? [self modeDropdownRect] : [self shapeDropdownRect]);
        const uint32_t count = _openMenu == 1 ? 3u : (_openMenu == 2 ? 2u : 6u);
        if (NSPointInRect(pt, menuRect)) {
            const int hit = s3g::clap_gui::dropdownHitIndex(pt, menuRect, 20.0, count);
            if (hit >= 0) {
                if (_openMenu == 1) {
                    applyParam(*p, kParamGroupSize, static_cast<double>(hit));
                } else if (_openMenu == 2) {
    ...
    if (NSPointInRect(pt, [self groupMenuBoxRect])) {
        _openMenu = 1;
        _hoverMenuItem = -1;
        [self setNeedsDisplay:YES];
        return;
    ...
    const SliderHit hits[] {
        ...
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 3u), 0, kParamFlow },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 4u), 1, kParamSpread },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 5u), 2, kParamVortex },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 6u), 3, kParamMotion },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 7u), 4, kParamRate },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 8u), 5, kParamDivision },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 9u), 6, kParamPhase },
        { s3g::gui_layout::sliderHitRect(
              kMatrixLayout.groupPattern, 10u), 7, kParamSmoothing },
    };
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool isFloating)
{
    ...
    if (!s3g::clap_gui::createResponsiveViewport(p->guiViewport, static_cast<NSView*>(p->guiView), kGuiWidth, kGuiHeight)) { [static_cast<NSView*>(p->guiView) release]; p->guiView = nullptr; return false; }
    return true;
}
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    if (p->guiView) {
        p->guiVisible = false;
        [static_cast<S3GGroupMatrixView*>(p->guiView) stopRefreshTimer];
        s3g::clap_gui::destroyResponsiveViewport(p->guiViewport, p->guiView);
    }
}
...
bool guiSetParent(const clap_plugin_t* plugin, const clap_window_t* win)
{
    ...
    auto* p = self(plugin);
    return s3g::clap_gui::setResponsiveViewportParent(p->guiViewport, static_cast<NSView*>(win->cocoa), p->host);
}
bool guiSetTransient(const clap_plugin_t*, const clap_window_t*) { return false; }
void guiSuggestTitle(const clap_plugin_t*, const char*) {}
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible = true; [static_cast<S3GGroupMatrixView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GGroupMatrixView*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
const clap_plugin_gui_t guiExt { guiIsApiSupported, guiGetPreferredApi, guiCreate, guiDestroy, guiSetScale, guiGetSize, guiCanResize, guiGetResizeHints, guiAdjustSize, guiSetSize, guiSetParent, guiSetTransient, guiSuggestTitle, guiShow, guiHide };
#endif

#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "../common/s3g_group_matrix_canvas.inc"
