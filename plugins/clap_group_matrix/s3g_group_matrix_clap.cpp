...
clap_id paramIdForIndex(uint32_t index)
{
    ...
    case 10: return kParamGroupSize;
...
- (void)updateMenuHover:(NSPoint)pt
{
    ...
    const NSRect rect = _openMenu == 1 ? [self groupDropdownRect] : [self modeDropdownRect];
...
- (void)drawRect:(NSRect)dirty
{
    ...
    s3g::clap_gui::drawMenu(@"MODE", prm.mode == s3g::GroupMatrixFlowMode::Sync ? @"SYNC" : @"FREE", 212, small, small, style, 752, 838, 118);
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    auto* p = static_cast<Plugin*>(_plugin);
    if (_openMenu > 0) {
        const NSRect menuRect = _openMenu == 1 ? [self groupDropdownRect] : [self modeDropdownRect];
        const uint32_t count = _openMenu == 1 ? 3u : 2u;
    ...
    const CGFloat ys[] = { 108, 134, 160, 186, 238, 264, 290, 316, 386 };
    for (int i = 0; i < 9; ++i) {
