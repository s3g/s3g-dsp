...
constexpr clap_id kOutputFormatParamId = 20;
constexpr clap_id kOutputRotationParamId = 21;

constexpr double kGuiW = 920.0;
constexpr double kGuiH = 726.0;
...
bool loadSamplesFromURLs(Plugin& p, NSArray<NSURL*>* urls,
                         bool resetControls)
{
    std::array<std::shared_ptr<const s3g::LoopProcessorSample>,
        kMaxSources> sources {};
    std::array<std::string, kMaxSources> samplePaths {};
    uint32_t loaded = 0u;
    for (NSURL* url in urls) {
        if (loaded >= kMaxSources) break;
        const char* path = url && [url isFileURL]
            ? [[url path] fileSystemRepresentation] : nullptr;
        if (!path) continue;
        auto sample = readSampleFromPath(path);
        if (!sample) continue;
        sources[loaded] = sample;
        samplePaths[loaded] = path;
        ++loaded;
    }
    if (loaded == 0u) {
        return false;
    ...
    p.samplePaths = std::move(samplePaths);
    rebuildCompositeSample(p, resetControls);
    return true;
}
...
void chooseSampleFromFinder(Plugin& p)
{
    ...
    if ([panel runModal] == NSModalResponseOK) {
        (void)loadSamplesFromURLs(p, [panel URLs], true);
    }
}
...
bool paramsGetInfo(const clap_plugin_t*, uint32_t index, clap_param_info_t* info)
{
    ...
    if (def.id == kLaneMaskParamId || def.id == kRuleParamId || def.id == kMidiModeParamId || def.id == kMidiRootParamId || def.id == kOutputFormatParamId) {
    ...
    std::strncpy(info->module,
        def.id == kOutputFormatParamId || def.id == kOutputRotationParamId
            ? "Output" : "Processor Multi Loop", sizeof(info->module));
...
bool paramsTextToValue(const clap_plugin_t*, clap_id id, const char* display, double* value)
{
    ...
    if (id == kXfadeParamId || id == kLoopStartParamId || id == kLoopLengthParamId) {
        v *= 0.01;
    } else if (id == kSourceRateParamId || id == kSourceBlendParamId) {
        v *= 0.01;
    } else if (id == kSpreadParamId) {
        v *= 0.01;
    } else if (id == kRuleParamId) {
        for (uint32_t index = 0u; index <= 3u; ++index) {
            if (std::strcmp(display, ruleName(index)) == 0) {
                v = static_cast<double>(index);
                break;
            }
        }
...
bool stateLoad(const clap_plugin_t* plugin, const clap_istream_t* stream)
{
    ...
    if (version == kStateVersion) {
        ...
        s.sourceRateSpread = legacy.sourceRateSpread;
        s.sourceBlend = legacy.sourceBlend;
        s.midiMode = legacy.midiMode;
        s.midiRoot = legacy.midiRoot;
        s.launchMode = legacy.launchMode;
        s.laneMask = legacy.laneMask;
        s.rule = legacy.rule;
        s.playing = legacy.playing;
        std::memcpy(s.samplePaths, legacy.samplePaths,
            sizeof(s.samplePaths));
    } else {
        return false;
    }
    auto* p = self(plugin);
...
#if defined(__APPLE__)
@interface S3GMultiLoopProcessorView : NSView <NSDraggingDestination> {
    void* _plugin;
    int _dragSlider;
    NSTimer* _timer;
    int _openMenu;
...
- (void)drawSlider:(NSString*)name value:(NSString*)val norm:(CGFloat)n y:(CGFloat)y attrs:(NSDictionary*)attrs small:(NSDictionary*)small
{
    ...
    s3g::clap_gui::drawProcessorSlider(
        name, val, n, y, kToolboxX, kToolboxW, attrs, small, style);
}
- (void)drawMenuControl:(NSString*)name value:(NSString*)value y:(CGFloat)y attrs:(NSDictionary*)attrs small:(NSDictionary*)small
{
    s3g::clap_gui::Style style;
...
- (void)drawRect:(NSRect)dirty
{
    ...
    s3g::clap_gui::drawProcessorTitleBand(
        ...
        titleAttrs, lab, small, titleStyle);
    NSRect samplePanel = NSMakeRect(18,42,560,624);
    ...
    [self drawWaveform:sample rect:NSMakeRect(28,362,540,284) attrs:small];
    ...
    const CGFloat engineH =
        s3g::gui_layout::toolboxHeightForRows(10);
    panelFrame(y, engineH);
    [self drawSectionHeader:@"ENGINE" y:y attrs:section];
    [self drawMenuControl:@"RULE" value:[NSString stringWithUTF8String:ruleName(p->targets.rule.load(std::memory_order_acquire))] y:y + 36 attrs:small small:small];
    [self drawSlider:@"SRAT" value:[NSString stringWithFormat:@"%.0f%%", p->targets.sourceRateSpread.load(std::memory_order_acquire) * 100.0f] norm:p->targets.sourceRateSpread.load(std::memory_order_acquire) y:y + 62 attrs:small small:small];
    [self drawSlider:@"XBLD" value:[NSString stringWithFormat:@"%.0f%%", p->targets.sourceBlend.load(std::memory_order_acquire) * 100.0f] norm:p->targets.sourceBlend.load(std::memory_order_acquire) y:y + 88 attrs:small small:small];
    [self drawSlider:@"RATE" value:[NSString stringWithFormat:@"%.3f", params.baseRate] norm:(params.baseRate - 0.125) / (4.0 - 0.125) y:y + 114 attrs:small small:small];
    [self drawSlider:@"XFD" value:[NSString stringWithFormat:@"%.0f%%", params.xfadePct * 100.0f] norm:params.xfadePct / 0.3f y:y + 140 attrs:small small:small];
    [self drawSlider:@"DUCK" value:[NSString stringWithFormat:@"%.2f", params.seamDuck] norm:params.seamDuck / 0.75f y:y + 166 attrs:small small:small];
    [self drawSlider:@"STRT" value:[NSString stringWithFormat:@"%.0f%%", params.loopStart * 100.0f] norm:params.loopStart / 0.999f y:y + 192 attrs:small small:small];
    [self drawSlider:@"LEN" value:[NSString stringWithFormat:@"%.0f%%", params.loopLength * 100.0f] norm:(params.loopLength - 0.01f) / 0.99f y:y + 218 attrs:small small:small];
    ...
    [self drawSlider:@"ROOT" value:[NSString stringWithFormat:@"%.0f", p->targets.midiRoot.load(std::memory_order_acquire)] norm:p->targets.midiRoot.load(std::memory_order_acquire) / 127.0f y:y + 270 attrs:small small:small];
    y += engineH + gap;
    const CGFloat relH = 254.0;
    panelFrame(y, relH);
    ...
    [self drawRelationshipPreview:NSMakeRect(
        ...
        panelW - 2.0 * s3g::gui_layout::kStandardMetrics.labelInset, 96) attrs:small];
    if (_openMenu == 1) {
        ...
        s3g::clap_gui::drawDropdownMenu(NSMakeRect(
            ...
            s3g::gui_layout::processorMenuWidth(kToolboxW), 18.0 * 4.0),
            18.0, items, 4, selected, _hoverMenuItem, small, style);
    } else if (_openMenu == 2) {
        ...
        s3g::clap_gui::drawDropdownMenu(NSMakeRect(
            _menuOrigin.x, _menuOrigin.y,
            s3g::gui_layout::processorMenuWidth(kToolboxW), 18.0 * 3.0),
            18.0, items, 3, selected, _hoverMenuItem, small, style);
    } else if (_openMenu == 3) {
        ...
        s3g::clap_gui::drawDropdownMenu(NSMakeRect(
            ...
            18.0, items, 3, static_cast<int>(outputFormat),
            _hoverMenuItem, small, style);
    }
}
- (void)updateMenuHover:(NSPoint)point
{
    if (!(_openMenu == 1 || _openMenu == 2 || _openMenu == 3)) return;
...
- (void)updateSlider:(NSPoint)pt
{
    ...
    case 14: setParam(*p, kOutputRotationParamId, -180.0 + n * 360.0); break;
    default: break;
    }
    [self setNeedsDisplay:YES];
}
...
{
    auto* p = static_cast<Plugin*>(_plugin);
    if (!loadSamplesFromURLs(*p, supportedAudioFileURLs(
            [sender draggingPasteboard]), true)) {
        return NO;
    }
    _waveZoom = 1.0;
    _waveScroll = 0.0;
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (_openMenu == 1 || _openMenu == 2 || _openMenu == 3) {
        const uint32_t menuCount = _openMenu == 1 ? 4u : 3u;
        ...
        if (hit >= 0) {
            if (_openMenu == 1) {
                ...
                setParam(*p, kOutputFormatParamId, hit);
            }
        }
        _openMenu = 0;
        _hoverMenuItem = -1;
    ...
    const NSRect waveRect = NSMakeRect(28,362,540,284);
    const CGFloat zoom = std::clamp<CGFloat>(_waveZoom, 1.0, 16.0);
    ...
    if (NSPointInRect(pt, NSMakeRect(
        ...
        if (s3g::clap_gui::sliderDoubleClickDefault(
                event, &p->plugin, kGainParamId, &defaultValue)) {
            setParam(*p, kGainParamId, defaultValue);
            _dragSlider = -1;
        } else {
            _dragSlider = 4;
    ...
    if (NSPointInRect(pt, NSMakeRect(kToolboxX, engineY + 36 - 8, kToolboxW, 24))) {
        _openMenu = 1;
        _hoverMenuItem = -1;
        _menuItemCount = 4;
        _menuOrigin = NSMakePoint(kSliderTrackX, engineY + 53);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(pt, NSMakeRect(kToolboxX, engineY + 244 - 8, kToolboxW, 24))) {
        _openMenu = 2;
        _hoverMenuItem = -1;
        _menuItemCount = 3;
    ...
    auto paramForSlider = [](int slider) -> clap_id {
        switch (slider) {
        case 1: return kRateParamId;
        case 2: return kXfadeParamId;
        case 3: return kDuckParamId;
        case 5: return kSpreadParamId;
        case 6: return kDriftParamId;
        case 7: return kCenterParamId;
        case 8: return kGlideParamId;
        case 9: return kLoopStartParamId;
        case 10: return kLoopLengthParamId;
        case 11: return kSourceRateParamId;
        case 12: return kSourceBlendParamId;
        case 13: return kMidiRootParamId;
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool isFloating) { if (!guiIsApiSupported(plugin, api, isFloating)) return false; auto* p = self(plugin); if (p->guiView) return true; p->guiView = [[S3GMultiLoopProcessorView alloc] initWithPlugin:p]; if (!p->guiView) return false; if (!s3g::clap_gui::createResponsiveViewport(p->guiViewport, static_cast<NSView*>(p->guiView), kGuiW, kGuiH)) { [static_cast<NSView*>(p->guiView) release]; p->guiView = nullptr; return false; } return true; }
void guiDestroy(const clap_plugin_t* plugin) { auto* p = self(plugin); if (p->guiView) { p->guiVisible.store(false); [static_cast<S3GMultiLoopProcessorView*>(p->guiView) stopRefreshTimer]; s3g::clap_gui::destroyResponsiveViewport(p->guiViewport, p->guiView); } }
bool guiSetScale(const clap_plugin_t*, double) { return true; }
bool guiGetSize(const clap_plugin_t* plugin, uint32_t* w, uint32_t* h) { return s3g::clap_gui::getResponsiveViewportSize(self(plugin)->guiViewport, kGuiW, kGuiH, w, h); }
...
bool guiAdjustSize(const clap_plugin_t* plugin, uint32_t* w, uint32_t* h) { return s3g::clap_gui::adjustResponsiveViewportSize(self(plugin)->guiViewport, kGuiW, kGuiH, w, h); }
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible.store(true); [static_cast<S3GMultiLoopProcessorView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible.store(false); [static_cast<S3GMultiLoopProcessorView*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
