...
- (void)drawRect:(NSRect)dirty
{
    ...
    menu(@"MODE", prm.mode == s3g::AmbiGroupMatrix128FlowMode::Sync
        ? @"SYNC" : @"FREE", 1u);
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (s3g::clap_gui::handleProcessorTitleClick(
            pt, &p->plugin, @"Ambi Matrix Group 128",
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    if (p->guiView) {
        p->guiVisible = false;
        [static_cast<S3GAmbiGroupMatrix128View*>(p->guiView) stopRefreshTimer];
        s3g::clap_gui::destroyResponsiveViewport(p->guiViewport, p->guiView);
    }
}
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible = true; [static_cast<S3GAmbiGroupMatrix128View*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GAmbiGroupMatrix128View*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
const clap_plugin_gui_t guiExt { guiIsApiSupported, guiGetPreferredApi, guiCreate, guiDestroy, guiSetScale, guiGetSize, guiCanResize, guiGetResizeHints, guiAdjustSize, guiSetSize, guiSetParent, guiSetTransient, guiSuggestTitle, guiShow, guiHide };
#endif

#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#define S3G_AMBI_GROUP_MATRIX_CHANNELS 128
