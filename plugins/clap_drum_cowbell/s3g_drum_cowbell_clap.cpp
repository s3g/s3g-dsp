...
constexpr uint32_t kStateMagic = 0x57473353u; // "S3GW" in little endian.
...
constexpr clap_id kIntervalParamId = 3u;
constexpr clap_id kDetuneParamId = 4u;
constexpr clap_id kShapeParamId = 5u;
constexpr clap_id kAttackParamId = 6u;
constexpr clap_id kDecayParamId = 7u;
constexpr clap_id kDampingParamId = 8u;
constexpr clap_id kBodyParamId = 9u;
constexpr clap_id kBodyDecayParamId = 10u;
constexpr clap_id kBrightnessParamId = 11u;
constexpr clap_id kNoiseParamId = 12u;
constexpr clap_id kNoiseDecayParamId = 13u;
constexpr clap_id kBendParamId = 14u;
constexpr clap_id kBendDecayParamId = 15u;
constexpr clap_id kStrikeToneParamId = 16u;
...
constexpr std::array<ParamDef, kParamCount> kParamDefs {{
    { kTuneParamId, "Tune", "Oscillators", 180.0, 2400.0, 560.0, false },
    { kNoteTrackingParamId, "Note Tracking", "Oscillators / MIDI", 0.0, 1.0, 0.65, false },
    { kIntervalParamId, "Interval", "Oscillators", 1.08, 2.60, 1.52, false },
    { kDetuneParamId, "Detune", "Oscillators", 0.0, 1.0, 0.08, false },
    { kShapeParamId, "Shape", "Oscillators", 0.0, 1.0, 0.68, false },
    { kAttackParamId, "Attack", "Strike", 0.0, 1.0, 0.62, false },
    { kDecayParamId, "Decay", "Oscillators", 0.025, 2.0, 0.24, false },
    { kDampingParamId, "Damping", "Shell", 0.0, 1.0, 0.46, false },
    { kBodyParamId, "Body", "Shell", 0.0, 1.0, 0.62, false },
    { kBodyDecayParamId, "Body Decay", "Shell", 0.02, 1.5, 0.18, false },
    { kBrightnessParamId, "Brightness", "Shell", 0.0, 1.0, 0.58, false },
    { kNoiseParamId, "Noise", "Strike", 0.0, 1.0, 0.16, false },
    { kNoiseDecayParamId, "Noise Decay", "Strike", 0.004, 0.40, 0.035, false },
    { kBendParamId, "Bend", "Motion", -24.0, 24.0, 0.0, false },
    { kBendDecayParamId, "Bend Decay", "Motion", 0.003, 0.50, 0.030, false },
    { kStrikeToneParamId, "Strike Tone", "Strike", 0.0, 1.0, 0.56, false },
...
struct Plugin {
    ...
    std::atomic<uint32_t> pendingCowbellTriggers { 0u };
    std::atomic<uint32_t> pendingMutedTriggers { 0u };
    std::atomic<uint32_t> pendingHighTriggers { 0u };
...
s3g::DrumCowbellParams publishedParamsSnapshot(const Plugin& p)
{
    s3g::DrumCowbellParams params;
    ...
    params.intervalRatio = static_cast<float>(paramValue(p, kIntervalParamId));
    params.detune = static_cast<float>(paramValue(p, kDetuneParamId));
    params.shape = static_cast<float>(paramValue(p, kShapeParamId));
    ...
    params.body = static_cast<float>(
        paramValue(p, kBodyParamId));
    params.bodyDecaySeconds = static_cast<float>(
        paramValue(p, kBodyDecayParamId));
    params.brightness = static_cast<float>(
        paramValue(p, kBrightnessParamId));
    params.noise = static_cast<float>(paramValue(p, kNoiseParamId));
    params.noiseDecaySeconds = static_cast<float>(paramValue(p, kNoiseDecayParamId));
    params.bendSemitones = static_cast<float>(paramValue(p, kBendParamId));
    params.bendDecaySeconds = static_cast<float>(paramValue(p, kBendDecayParamId));
    params.strikeTone = static_cast<float>(
        paramValue(p, kStrikeToneParamId));
...
uint64_t tailSamplesForParams(const s3g::DrumCowbellParams& params,
    double sampleRate)
{
    const double seconds = std::max({
        s3g::drumCowbellTailSeconds(params,
            s3g::DrumCowbellArticulation::Cowbell, sampleRate),
        s3g::drumCowbellTailSeconds(params,
            s3g::DrumCowbellArticulation::Muted, sampleRate),
        s3g::drumCowbellTailSeconds(params,
            s3g::DrumCowbellArticulation::High, sampleRate),
    });
...
void triggerCowbell(Plugin& p, s3g::DrumCowbellArticulation articulation,
    float velocity, int midiNote)
{
    p.visualTriggerCode.store(
        articulation == s3g::DrumCowbellArticulation::High ? 3u
            : articulation == s3g::DrumCowbellArticulation::Muted ? 2u : 1u,
        std::memory_order_relaxed);
    p.cowbell.trigger(articulation, std::clamp(velocity, 0.0f, 1.0f),
    ...
    bool triggerImmediately = false)
{
    ...
    if (id == kTriggerParamId) {
        ...
        if (code > 0u && code != p.triggerCode) {
            const auto articulation = code == 3u
                ? s3g::DrumCowbellArticulation::High
                : code == 2u ? s3g::DrumCowbellArticulation::Muted
                    : s3g::DrumCowbellArticulation::Cowbell;
            if (triggerImmediately) {
                triggerCowbell(p, articulation, 1.0f,
                    s3g::drumCowbellCanonicalMidiNote(articulation));
            } else {
                auto& pending = code == 3u ? p.pendingHighTriggers
                    : code == 2u ? p.pendingMutedTriggers
                        : p.pendingCowbellTriggers;
    ...
    case kIntervalParamId: p.params.intervalRatio = v; break;
    case kDetuneParamId:
        tailChanged = std::fabs(p.params.detune - v) > 1.0e-6f;
        p.params.detune = v;
        break;
    case kShapeParamId:
        tailChanged = std::fabs(p.params.shape - v) > 1.0e-6f;
        p.params.shape = v;
        break;
    ...
    case kBrightnessParamId:
        tailChanged = std::fabs(
            p.params.brightness - v) > 1.0e-6f;
        p.params.brightness = v;
        break;
    case kNoiseParamId:
        tailChanged = std::fabs(p.params.noise - v) > 1.0e-6f;
        p.params.noise = v;
        break;
    case kNoiseDecayParamId:
        tailChanged = std::fabs(p.params.noiseDecaySeconds - v) > 1.0e-6f;
        p.params.noiseDecaySeconds = v;
        break;
    case kBendParamId:
        tailChanged = std::fabs(p.params.bendSemitones - v) > 1.0e-6f;
        p.params.bendSemitones = v;
        break;
    case kBendDecayParamId:
        tailChanged = std::fabs(p.params.bendDecaySeconds - v) > 1.0e-6f;
        p.params.bendDecaySeconds = v;
        break;
    case kStrikeToneParamId: p.params.strikeTone = v; break;
    case kDriveParamId:
        tailChanged = std::fabs(
    ...
    p.cowbell.setParams(p.params);
    p.params = p.cowbell.params();
    publishParam(p, id, rawParamValue(p, id));
    p.parameterRevision.fetch_add(1u, std::memory_order_release);
    if (tailChanged) {
        // Some character state is live rather than voice-latched. Lengthening
        // a brightness-affecting control while a hit is active must therefore extend
...
double rawParamValue(const Plugin& p, clap_id id)
{
    ...
    case kIntervalParamId: return p.params.intervalRatio;
    case kDetuneParamId: return p.params.detune;
    case kShapeParamId: return p.params.shape;
    ...
    case kBrightnessParamId: return p.params.brightness;
    case kNoiseParamId: return p.params.noise;
    case kNoiseDecayParamId: return p.params.noiseDecaySeconds;
    case kBendParamId: return p.params.bendSemitones;
    case kBendDecayParamId: return p.params.bendDecaySeconds;
    case kStrikeToneParamId: return p.params.strikeTone;
...
void assignSavedStateValues(s3g::DrumCowbellParams& params,
    const std::array<double, kSavedParamCount>& values)
{
    ...
    params.intervalRatio = value(kIntervalParamId);
    params.detune = value(kDetuneParamId);
    params.shape = value(kShapeParamId);
    params.attack = value(kAttackParamId);
    params.decaySeconds = value(kDecayParamId);
    params.damping = value(kDampingParamId);
    params.body = value(kBodyParamId);
    params.bodyDecaySeconds = value(kBodyDecayParamId);
    params.brightness = value(kBrightnessParamId);
    params.noise = value(kNoiseParamId);
    params.noiseDecaySeconds = value(kNoiseDecayParamId);
    params.bendSemitones = value(kBendParamId);
    params.bendDecaySeconds = value(kBendDecayParamId);
    params.strikeTone = value(kStrikeToneParamId);
...
bool consumePendingState(Plugin& p)
{
    ...
    s3g::DrumCowbellParams next = p.params;
    assignSavedStateValues(next, values);
    p.cowbell.setParams(next);
    p.params = p.cowbell.params();
    p.midiReceive = values.back();
    p.pendingCowbellTriggers.store(0u, std::memory_order_relaxed);
    p.pendingMutedTriggers.store(0u, std::memory_order_relaxed);
    p.pendingHighTriggers.store(0u, std::memory_order_relaxed);
    p.triggerCode = 0u;
    publishParam(p, kTriggerParamId, 0.0);
    if (p.active) {
        extendActiveTail(p, tailSamplesForParams(p.params, p.sampleRate));
...
void applyEvent(Plugin& p, const clap_event_header_t* event)
{
    ...
    if (event->type == CLAP_EVENT_PARAM_VALUE
        ...
        if (note->velocity > 0.0
            ...
            const auto articulation = note->key == 57
                ? s3g::DrumCowbellArticulation::Muted
                : note->key == 58 ? s3g::DrumCowbellArticulation::High
                    : s3g::DrumCowbellArticulation::Cowbell;
            triggerCowbell(p, articulation,
        ...
        if ((midi->data[0] & 0xf0u) == 0x90u && midi->data[2] > 0u
            ...
            const auto articulation = note == 57
                ? s3g::DrumCowbellArticulation::Muted
                : note == 58 ? s3g::DrumCowbellArticulation::High
                    : s3g::DrumCowbellArticulation::Cowbell;
            triggerCowbell(p, articulation,
    ...
    uint32_t, uint32_t)
{
    ...
    p->cowbell.prepare(p->sampleRate);
    p->cowbell.setParams(p->params);
    ...
    const clap_process_t* process)
{
    ...
    uint32_t pending = p->pendingCowbellTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerCowbell(*p,
        s3g::DrumCowbellArticulation::Cowbell, 1.0f, 56);
    pending = p->pendingMutedTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerCowbell(*p,
        s3g::DrumCowbellArticulation::Muted, 1.0f, 57);
    pending = p->pendingHighTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerCowbell(*p,
        s3g::DrumCowbellArticulation::High, 1.0f, 58);
    ...
    for (uint32_t sample = 0u; sample < process->frames_count; ++sample) {
        ...
        float left = 0.0f;
        float right = 0.0f;
        p->cowbell.processFrame(left, right);
    ...
    char* display, uint32_t size)
{
    ...
    if (id == kTuneParamId) {
    ...
    } else if (id == kDecayParamId || id == kBodyDecayParamId
        || id == kNoiseDecayParamId || id == kBendDecayParamId) {
        std::snprintf(display, size, "%.3g s", value);
    } else if (id == kIntervalParamId) {
        std::snprintf(display, size, "%.3g x", value);
    } else if (id == kBendParamId) {
        std::snprintf(display, size, "%+.2g st", value);
        ...
        std::snprintf(display, size, "%s",
            code == 3u ? "High" : (code == 2u ? "Muted"
                : (code == 1u ? "Cowbell" : "Ready")));
    } else {
        std::snprintf(display, size, "%.0f%%", value * 100.0);
    }
    return true;
    ...
    const char* display, double* value)
{
    ...
    if (id == kTriggerParamId) {
        if (std::strcmp(display, "Cowbell") == 0) {
            *value = 1.0;
            return true;
        }
    ...
    const bool percentParam = id != kTuneParamId
        && id != kIntervalParamId && id != kDecayParamId
        && id != kBodyDecayParamId && id != kNoiseDecayParamId
        && id != kBendParamId && id != kBendDecayParamId
        && id != kOutputParamId
        && id != kMidiReceiveParamId && id != kTriggerParamId;
    const char* expectedSuffix = id == kTuneParamId ? "Hz"
        : ((id == kDecayParamId || id == kBodyDecayParamId
              || id == kNoiseDecayParamId || id == kBendDecayParamId) ? "s"
        : (id == kIntervalParamId ? "x"
        : (id == kBendParamId ? "st"
...
constexpr std::array<ClapUiRow, kSavedParamCount> kUiRows {{
    ...
    { kIntervalParamId, "INTERVAL", kLeftPanelX, kPanelWidth, 128.0 },
    { kDetuneParamId, "DETUNE", kLeftPanelX, kPanelWidth, 152.0 },
    { kShapeParamId, "SHAPE", kLeftPanelX, kPanelWidth, 176.0 },
    { kAttackParamId, "ATTACK", kLeftPanelX, kPanelWidth, 200.0 },
    { kDecayParamId, "DECAY", kLeftPanelX, kPanelWidth, 224.0 },
    { kDampingParamId, "DAMPING", kLeftPanelX, kPanelWidth, 248.0 },

    { kBodyParamId, "BODY", kRightPanelX, kPanelWidth, 80.0 },
    { kBodyDecayParamId, "BODY DECAY", kRightPanelX, kPanelWidth, 104.0 },
    { kBrightnessParamId, "BRIGHTNESS", kRightPanelX, kPanelWidth, 128.0 },
    { kNoiseParamId, "NOISE", kRightPanelX, kPanelWidth, 152.0 },
    { kNoiseDecayParamId, "NOISE DECAY", kRightPanelX, kPanelWidth, 176.0 },
    { kBendParamId, "BEND", kRightPanelX, kPanelWidth, 200.0 },
    { kBendDecayParamId, "BEND DECAY", kRightPanelX, kPanelWidth, 224.0 },
    { kStrikeToneParamId, "STRIKE TONE", kRightPanelX, kPanelWidth, 248.0 },
...
{
    return id == kTuneParamId || id == kDecayParamId
        || id == kBodyDecayParamId || id == kNoiseDecayParamId
        || id == kBendDecayParamId;
}
    ...
    const s3g::DrumCowbellParams& params, clap_id id)
{
    ...
    case kBrightnessParamId: return params.brightness;
    case kNoiseParamId: return params.noise;
    case kNoiseDecayParamId: return params.noiseDecaySeconds;
    case kBendParamId: return params.bendSemitones;
    case kBendDecayParamId: return params.bendDecaySeconds;
    case kStrikeToneParamId: return params.strikeTone;
...
bool queueGuiSafeRandomParamSet(Plugin& p,
    const s3g::DrumCowbellParams& sourceParams)
{
    using Kind = s3g::clap_gui::ParamEventKind;
...
{
    auto* p = &instance;
    if (s3g::kDrumCowbellFactoryPresetCount == 0u) return false;
    index = std::clamp(index, 0,
        static_cast<int>(s3g::kDrumCowbellFactoryPresetCount - 1u));
    const auto preset = s3g::drumCowbellFactoryPreset(
    ...
    set(kIntervalParamId, preset.intervalRatio);
    set(kDetuneParamId, preset.detune);
    set(kShapeParamId, preset.shape);
    ...
    set(kNoiseParamId, preset.noise);
    set(kNoiseDecayParamId, preset.noiseDecaySeconds);
    set(kBendParamId, preset.bendSemitones);
    set(kBendDecayParamId, preset.bendDecaySeconds);
    set(kStrikeToneParamId, preset.strikeTone);
...
@interface S3GDrumCowbellView : NSView {
    void* _plugin;
    int _dragParam;
...
@implementation S3GDrumCowbellView
...
- (id)initWithPlugin:(void*)plugin
{
    ...
    if (self) {
        ...
        _factoryPresetIndex = p
            ? s3g::drumCowbellFactoryPresetIndex(publishedParamsSnapshot(*p))
            : 0;
        ...
        if (_factoryPresetIndex >= 0) {
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                s3g::drumCowbellFactoryPresetInfo(
...
- (void)applyFactoryPreset:(int)index
{
    ...
    std::snprintf(_presetName, sizeof(_presetName), "%s",
        s3g::drumCowbellFactoryPresetInfo(static_cast<uint32_t>(index)).name);
    ...
    style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        items[index] = [NSString stringWithUTF8String:
            s3g::drumCowbellFactoryPresetInfo(index).name];
    }
...
- (void)refresh:(NSTimer*)timer
{
    ...
    if (p) {
        ...
        if (revision != _observedParamRevision) {
            ...
            _factoryPresetIndex = s3g::drumCowbellFactoryPresetIndex(
                publishedParamsSnapshot(*p));
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                _factoryPresetIndex >= 0
                    ? s3g::drumCowbellFactoryPresetInfo(static_cast<uint32_t>(
...
- (void)drawRect:(NSRect)dirty
{
    ...
    drawPanel(@"OSCILLATORS / STRIKE", voicePanel);
    drawPanel(@"SHELL / MOTION", attackPanel);
    ...
    const NSRect clapPad = NSMakeRect(kRightPanelX + 12.0, 480.0,
        padWidth, 24.0);
    const NSRect flamPad = NSMakeRect(NSMaxX(clapPad) + 6.0, 480.0,
        padWidth, 24.0);
    const NSRect tightPad = NSMakeRect(NSMaxX(flamPad) + 6.0, 480.0,
        padWidth, 24.0);
    s3g::clap_gui::drawHeaderButton(clapPad, outputPanel, @"COW",
        visualTrigger == 1u && activity > 0.02f, valueAttrs, style);
    s3g::clap_gui::drawHeaderButton(flamPad, outputPanel, @"MUTED",
        visualTrigger == 2u && activity > 0.02f, valueAttrs, style);
    s3g::clap_gui::drawHeaderButton(tightPad, outputPanel, @"HIGH",
    ...
    for (uint32_t shell = 0u; shell < 3u; ++shell) {
        const CGFloat inset = static_cast<CGFloat>(shell) * 9.0;
        [[NSColor colorWithCalibratedRed:0.30 + activity * 0.38
            green:0.62 + activity * 0.24 blue:0.66 + activity * 0.20
            alpha:0.30 + activity * 0.52] setStroke];
        NSBezierPath* path = [NSBezierPath bezierPath];
        [path moveToPoint:NSMakePoint(
            center.x - 52.0 + inset, center.y + 31.0 - inset * 0.28)];
        [path lineToPoint:NSMakePoint(
            center.x + 52.0 - inset, center.y + 31.0 - inset * 0.28)];
        [path lineToPoint:NSMakePoint(
            center.x + 34.0 - inset * 0.58, center.y - 36.0 + inset)];
        [path lineToPoint:NSMakePoint(
            center.x - 34.0 + inset * 0.58, center.y - 36.0 + inset)];
        [path closePath];
        [path setLineWidth:1.0 + activity * 1.8];
        [path stroke];
    }
    [[NSColor colorWithCalibratedRed:0.72 green:0.86 blue:0.82
        alpha:0.45 + activity * 0.45] setStroke];
    for (uint32_t ray = 0u; ray < 7u; ++ray) {
        const CGFloat angle = -0.72 + static_cast<CGFloat>(ray) * 0.24;
        const CGFloat radius = 52.0 + activity * 13.0;
        NSBezierPath* path = [NSBezierPath bezierPath];
        [path moveToPoint:NSMakePoint(center.x, center.y + 32.0)];
        [path lineToPoint:NSMakePoint(
    ...
    const CGFloat clutchRadius = 4.0 + activity * 4.0;
    [[NSBezierPath bezierPathWithOvalInRect:NSMakeRect(
        center.x - clutchRadius, center.y - 44.0 - clutchRadius,
        clutchRadius * 2.0, clutchRadius * 2.0)] fill];
    [@"SYNTHESIZED COWBELL  //  MUTED  //  HIGH"
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(point,
        ...
        _menuItemCount = std::min<uint32_t>(
            s3g::kDrumCowbellFactoryPresetCount, 64u);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(point,
    ...
    if (NSPointInRect(point,
        ...
        const auto randomized = s3g::drumCowbellSafeRandomParams(
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    p->guiView = [[S3GDrumCowbellView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = false;
    [static_cast<S3GDrumCowbellView*>(p->guiView) stopRefreshTimer];
...
bool guiShow(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = true;
    [static_cast<S3GDrumCowbellView*>(p->guiView) startRefreshTimer];
    return true;
}
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_cowbell_vstgui.inc"
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "A procedural mono-compatible stereo cowbell synthesizer with open, muted and high articulations.",
    features
};
