    ...
    float velocity, int midiNote)
{
    p.visualTriggerCode.store(
        articulation == s3g::DrumTomArticulation::RimStick ? 2u : 1u,
...
{
    auto* p = &instance;
    if (s3g::kDrumFloorTomFactoryPresetCount == 0u) return false;
...
- (void)drawRect:(NSRect)dirty
{
    ...
    const NSRect headPad = NSMakeRect(kRightPanelX + 12.0, 480.0,
        padWidth, 24.0);
    const NSRect rimPad = NSMakeRect(NSMaxX(headPad) + 6.0, 480.0,
        padWidth, 24.0);
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_floor_tom_vstgui.inc"
