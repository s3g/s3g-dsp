...
constexpr uint32_t kStateMagic = 0x52473353u; // "S3GR" in little endian.
...
constexpr clap_id kAlloyParamId = 4u;
constexpr clap_id kSpreadParamId = 5u;
constexpr clap_id kDensityParamId = 6u;
constexpr clap_id kBrightnessParamId = 7u;
constexpr clap_id kAttackParamId = 8u;
constexpr clap_id kDecayParamId = 9u;
constexpr clap_id kDampingParamId = 10u;
constexpr clap_id kWashParamId = 11u;
constexpr clap_id kWashDecayParamId = 12u;
constexpr clap_id kBellParamId = 13u;
constexpr clap_id kBellTuneParamId = 14u;
constexpr clap_id kChokeTimeParamId = 15u;
...
constexpr std::array<ParamDef, kParamCount> kParamDefs {{
    { kTuneParamId, "Tune", "Metal Field", 180.0, 2400.0, 720.0, false },
    { kNoteTrackingParamId, "Note Tracking", "Metal Field / MIDI", 0.0, 1.0, 0.25, false },
    { kSizeParamId, "Size", "Metal Field", 0.0, 1.0, 0.62, false },
    { kAlloyParamId, "Alloy", "Metal Field", 0.0, 1.0, 0.58, false },
    { kSpreadParamId, "Spread", "Metal Field", 0.0, 1.0, 0.66, false },
    { kDensityParamId, "Density", "Metal Field", 4.0, 16.0, 12.0, true },
    { kBrightnessParamId, "Brightness", "Metal Field", 0.0, 1.0, 0.68, false },
    { kAttackParamId, "Attack", "Strike", 0.0, 1.0, 0.72, false },
    { kDecayParamId, "Decay", "Metal Field", 0.08, 8.0, 1.30, false },
    { kDampingParamId, "Damping", "Metal Field", 0.0, 1.0, 0.44, false },
    { kWashParamId, "Wash", "Wash", 0.0, 1.0, 0.74, false },
    { kWashDecayParamId, "Wash Decay", "Wash", 0.08, 8.0, 1.45, false },
    { kBellParamId, "Bell", "Bell", 0.0, 1.0, 0.20, false },
    { kBellTuneParamId, "Bell Tune", "Bell", 350.0, 6000.0, 1680.0, false },
    { kChokeTimeParamId, "Choke Time", "Articulation", 0.5, 150.0, 14.0, false },
    { kTextureParamId, "Texture", "Wash", 0.0, 1.0, 0.58, false },
    ...
    { kStereoWidthParamId, "Stereo Width", "Output", 0.0, 1.0, 0.68, false },
    { kVelocityParamId, "Velocity Sensitivity", "MIDI", 0.0, 1.0, 0.90, false },
    { kOutputParamId, "Output Gain", "Output", -36.0, 12.0, -11.0, false },
...
struct Plugin {
    ...
    std::atomic<uint32_t> pendingCrashTriggers { 0u };
    std::atomic<uint32_t> pendingChokeTriggers { 0u };
    std::atomic<uint32_t> pendingBellTriggers { 0u };
    ...
    std::atomic<double> preservedOutputValue { -11.0 };
...
s3g::DrumCrashParams publishedParamsSnapshot(const Plugin& p)
{
    s3g::DrumCrashParams params;
    ...
    params.alloy = static_cast<float>(paramValue(p, kAlloyParamId));
    params.spread = static_cast<float>(paramValue(p, kSpreadParamId));
    params.density = static_cast<float>(paramValue(p, kDensityParamId));
    params.brightness = static_cast<float>(paramValue(p, kBrightnessParamId));
    params.attack = static_cast<float>(paramValue(p, kAttackParamId));
    params.decaySeconds = static_cast<float>(
        paramValue(p, kDecayParamId));
    params.damping = static_cast<float>(
        paramValue(p, kDampingParamId));
    params.wash = static_cast<float>(
        paramValue(p, kWashParamId));
    params.washDecaySeconds = static_cast<float>(paramValue(p, kWashDecayParamId));
    params.bell = static_cast<float>(paramValue(p, kBellParamId));
    params.bellTuneHz = static_cast<float>(paramValue(p, kBellTuneParamId));
    params.chokeTimeMs = static_cast<float>(paramValue(p, kChokeTimeParamId));
...
uint64_t tailSamplesForParams(const s3g::DrumCrashParams& params,
    double sampleRate)
{
    const double seconds = std::max({
        s3g::drumCrashTailSeconds(params,
            s3g::DrumCrashArticulation::Crash, sampleRate),
        s3g::drumCrashTailSeconds(params,
            s3g::DrumCrashArticulation::Choke, sampleRate),
        s3g::drumCrashTailSeconds(params,
            s3g::DrumCrashArticulation::Bell, sampleRate),
    });
...
void triggerCrash(Plugin& p, s3g::DrumCrashArticulation articulation,
    float velocity, int midiNote)
{
    p.visualTriggerCode.store(
        articulation == s3g::DrumCrashArticulation::Bell ? 3u
            : articulation == s3g::DrumCrashArticulation::Choke ? 2u : 1u,
        std::memory_order_relaxed);
    p.crash.trigger(articulation, std::clamp(velocity, 0.0f, 1.0f),
    ...
    bool triggerImmediately = false)
{
    ...
    if (id == kTriggerParamId) {
        ...
        if (code > 0u && code != p.triggerCode) {
            const auto articulation = code == 3u
                ? s3g::DrumCrashArticulation::Bell
                : code == 2u ? s3g::DrumCrashArticulation::Choke
                    : s3g::DrumCrashArticulation::Crash;
            if (triggerImmediately) {
                triggerCrash(p, articulation, 1.0f,
                    s3g::drumCrashCanonicalMidiNote(articulation));
            } else {
                auto& pending = code == 3u ? p.pendingBellTriggers
                    : code == 2u ? p.pendingChokeTriggers
                        : p.pendingCrashTriggers;
    ...
    case kAlloyParamId:
        tailChanged = std::fabs(p.params.alloy - v) > 1.0e-6f;
        p.params.alloy = v;
        break;
    case kSpreadParamId:
        tailChanged = std::fabs(p.params.spread - v) > 1.0e-6f;
        p.params.spread = v;
        break;
    case kDensityParamId: p.params.density = v; break;
    case kBrightnessParamId: p.params.brightness = v; break;
    case kAttackParamId: p.params.attack = v; break;
    case kDecayParamId:
        tailChanged = std::fabs(
            p.params.decaySeconds - v) > 1.0e-6f;
        p.params.decaySeconds = v;
        break;
    case kDampingParamId:
        tailChanged = std::fabs(
            p.params.damping - v) > 1.0e-6f;
        p.params.damping = v;
        break;
    case kWashParamId:
        tailChanged = std::fabs(
            p.params.wash - v) > 1.0e-6f;
        p.params.wash = v;
        break;
    case kWashDecayParamId:
        tailChanged = std::fabs(p.params.washDecaySeconds - v) > 1.0e-6f;
        p.params.washDecaySeconds = v;
        break;
    case kBellParamId:
        tailChanged = std::fabs(p.params.bell - v) > 1.0e-6f;
        p.params.bell = v;
        break;
    case kBellTuneParamId:
        tailChanged = std::fabs(p.params.bellTuneHz - v) > 1.0e-6f;
        p.params.bellTuneHz = v;
        break;
    case kChokeTimeParamId:
        tailChanged = std::fabs(p.params.chokeTimeMs - v) > 1.0e-6f;
        p.params.chokeTimeMs = v;
        break;
    ...
    p.crash.setParams(p.params);
    p.params = p.crash.params();
    publishParam(p, id, rawParamValue(p, id));
    p.parameterRevision.fetch_add(1u, std::memory_order_release);
    if (tailChanged) {
        // Some character state is live rather than voice-latched. Lengthening
        // a wash-affecting control while a hit is active must therefore extend
...
double rawParamValue(const Plugin& p, clap_id id)
{
    ...
    case kAlloyParamId: return p.params.alloy;
    case kSpreadParamId: return p.params.spread;
    ...
    case kWashParamId: return p.params.wash;
    case kWashDecayParamId: return p.params.washDecaySeconds;
    case kBellParamId: return p.params.bell;
    case kBellTuneParamId: return p.params.bellTuneHz;
    case kChokeTimeParamId: return p.params.chokeTimeMs;
...
void assignSavedStateValues(s3g::DrumCrashParams& params,
    const std::array<double, kSavedParamCount>& values)
{
    ...
    params.alloy = value(kAlloyParamId);
    params.spread = value(kSpreadParamId);
    params.density = value(kDensityParamId);
    ...
    params.wash = value(kWashParamId);
    params.washDecaySeconds = value(kWashDecayParamId);
    params.bell = value(kBellParamId);
    params.bellTuneHz = value(kBellTuneParamId);
    params.chokeTimeMs = value(kChokeTimeParamId);
...
bool consumePendingState(Plugin& p)
{
    ...
    s3g::DrumCrashParams next = p.params;
    assignSavedStateValues(next, values);
    p.crash.setParams(next);
    p.params = p.crash.params();
    p.midiReceive = values.back();
    p.pendingCrashTriggers.store(0u, std::memory_order_relaxed);
    p.pendingChokeTriggers.store(0u, std::memory_order_relaxed);
    p.pendingBellTriggers.store(0u, std::memory_order_relaxed);
    p.triggerCode = 0u;
    publishParam(p, kTriggerParamId, 0.0);
    if (p.active) {
        extendActiveTail(p, tailSamplesForParams(p.params, p.sampleRate));
...
void applyEvent(Plugin& p, const clap_event_header_t* event)
{
    ...
    if (event->type == CLAP_EVENT_PARAM_VALUE
        ...
        if (note->velocity > 0.0
            ...
            const auto articulation = note->key == 50
                ? s3g::DrumCrashArticulation::Choke
                : note->key == 53 ? s3g::DrumCrashArticulation::Bell
                    : s3g::DrumCrashArticulation::Crash;
            triggerCrash(p, articulation,
        ...
        if ((midi->data[0] & 0xf0u) == 0x90u && midi->data[2] > 0u
            ...
            const auto articulation = note == 50
                ? s3g::DrumCrashArticulation::Choke
                : note == 53 ? s3g::DrumCrashArticulation::Bell
                    : s3g::DrumCrashArticulation::Crash;
            triggerCrash(p, articulation,
    ...
    const clap_process_t* process)
{
    ...
    uint32_t pending = p->pendingCrashTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerCrash(*p,
        s3g::DrumCrashArticulation::Crash, 1.0f, 49);
    pending = p->pendingChokeTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerCrash(*p,
        s3g::DrumCrashArticulation::Choke, 1.0f, 50);
    pending = p->pendingBellTriggers.exchange(
        0u, std::memory_order_relaxed);
    while (pending-- > 0u) triggerCrash(*p,
        s3g::DrumCrashArticulation::Bell, 1.0f, 53);
    ...
    for (uint32_t sample = 0u; sample < process->frames_count; ++sample) {
        ...
        float left = 0.0f;
        float right = 0.0f;
        p->crash.processFrame(left, right);
    ...
    char* display, uint32_t size)
{
    ...
    if (id == kTuneParamId || id == kBellTuneParamId) {
        std::snprintf(display, size, "%.1f Hz", value);
    } else if (id == kDecayParamId || id == kWashDecayParamId) {
        std::snprintf(display, size, "%.3g s", value);
    } else if (id == kChokeTimeParamId) {
        std::snprintf(display, size, "%.1f ms", value);
    } else if (id == kSizeParamId) {
        std::snprintf(display, size, "%.0f%%", value * 100.0);
    } else if (id == kDensityParamId) {
        std::snprintf(display, size, "%.0f modes", value);
    ...
    const char* display, double* value)
{
    ...
    if (id == kTriggerParamId) {
        if (std::strcmp(display, "Crash") == 0) {
            *value = 1.0;
            return true;
        }
        if (std::strcmp(display, "Choke") == 0) {
            *value = 2.0;
            return true;
        }
        if (std::strcmp(display, "Bell") == 0) {
            *value = 3.0;
            return true;
        }
    ...
    const bool percentParam = id != kTuneParamId
        && id != kBellTuneParamId && id != kDensityParamId
        && id != kChokeTimeParamId && id != kDecayParamId
        && id != kWashDecayParamId
        && id != kOutputParamId
        && id != kMidiReceiveParamId && id != kTriggerParamId;
    const char* expectedSuffix = (id == kTuneParamId
            || id == kBellTuneParamId) ? "Hz"
        : ((id == kDecayParamId || id == kWashDecayParamId) ? "s"
        : (id == kChokeTimeParamId ? "ms"
        : (id == kDensityParamId ? "modes"
...
constexpr std::array<ClapUiRow, kSavedParamCount> kUiRows {{
    ...
    { kSizeParamId, "SIZE", kLeftPanelX, kPanelWidth, 128.0 },
    { kAlloyParamId, "ALLOY", kLeftPanelX, kPanelWidth, 152.0 },
    { kSpreadParamId, "SPREAD", kLeftPanelX, kPanelWidth, 176.0 },
    { kDensityParamId, "DENSITY", kLeftPanelX, kPanelWidth, 200.0 },
    { kBrightnessParamId, "BRIGHTNESS", kLeftPanelX, kPanelWidth, 224.0 },
    { kAttackParamId, "ATTACK", kLeftPanelX, kPanelWidth, 248.0 },

    { kDecayParamId, "DECAY", kRightPanelX, kPanelWidth, 80.0 },
    { kDampingParamId, "DAMPING", kRightPanelX, kPanelWidth, 104.0 },
    { kWashParamId, "WASH", kRightPanelX, kPanelWidth, 128.0 },
    { kWashDecayParamId, "WASH DECAY", kRightPanelX, kPanelWidth, 152.0 },
    { kBellParamId, "BELL", kRightPanelX, kPanelWidth, 176.0 },
    { kBellTuneParamId, "BELL TUNE", kRightPanelX, kPanelWidth, 200.0 },
    { kChokeTimeParamId, "CHOKE TIME", kRightPanelX, kPanelWidth, 224.0 },
...
bool isLogUiParam(clap_id id)
{
    return id == kTuneParamId || id == kDecayParamId
        || id == kWashDecayParamId || id == kBellTuneParamId
...
double safeRandomParamValue(
    const s3g::DrumCrashParams& params, clap_id id)
{
    switch (id) {
    case kTuneParamId: return params.tuneHz;
    case kSizeParamId: return params.size;
    case kAlloyParamId: return params.alloy;
    case kSpreadParamId: return params.spread;
    case kDensityParamId: return params.density;
    ...
    case kWashParamId: return params.wash;
    case kWashDecayParamId: return params.washDecaySeconds;
    case kBellParamId: return params.bell;
    case kBellTuneParamId: return params.bellTuneHz;
    case kChokeTimeParamId: return params.chokeTimeMs;
...
bool queueGuiSafeRandomParamSet(Plugin& p,
    const s3g::DrumCrashParams& sourceParams)
{
    using Kind = s3g::clap_gui::ParamEventKind;
...
@implementation S3GDrumCrashView
...
- (id)initWithPlugin:(void*)plugin
{
    ...
    if (self) {
        ...
        _factoryPresetIndex = p
            ? s3g::drumCrashFactoryPresetIndex(publishedParamsSnapshot(*p))
            : 0;
        ...
        if (_factoryPresetIndex >= 0) {
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                s3g::drumCrashFactoryPresetInfo(
...
- (void)applyFactoryPreset:(int)index
{
    ...
    std::snprintf(_presetName, sizeof(_presetName), "%s",
        s3g::drumCrashFactoryPresetInfo(static_cast<uint32_t>(index)).name);
    ...
    style:(const s3g::clap_gui::Style&)style
{
    ...
    for (uint32_t index = 0u; index < count; ++index) {
        items[index] = [NSString stringWithUTF8String:
            s3g::drumCrashFactoryPresetInfo(index).name];
    }
...
- (void)refresh:(NSTimer*)timer
{
    ...
    if (p) {
        ...
        if (revision != _observedParamRevision) {
            ...
            _factoryPresetIndex = s3g::drumCrashFactoryPresetIndex(
                publishedParamsSnapshot(*p));
            std::snprintf(_presetName, sizeof(_presetName), "%s",
                _factoryPresetIndex >= 0
                    ? s3g::drumCrashFactoryPresetInfo(static_cast<uint32_t>(
...
- (void)drawRect:(NSRect)dirty
{
    ...
    drawPanel(@"METAL FIELD", voicePanel);
    drawPanel(@"WASH / BELL", attackPanel);
    ...
    const NSRect tightPad = NSMakeRect(NSMaxX(flamPad) + 6.0, 480.0,
        padWidth, 24.0);
    s3g::clap_gui::drawHeaderButton(clapPad, outputPanel, @"CRASH",
        visualTrigger == 1u && activity > 0.02f, valueAttrs, style);
    s3g::clap_gui::drawHeaderButton(flamPad, outputPanel, @"CHOKE",
        visualTrigger == 2u && activity > 0.02f, valueAttrs, style);
    s3g::clap_gui::drawHeaderButton(tightPad, outputPanel, @"BELL",
        visualTrigger == 3u && activity > 0.02f, valueAttrs, style);

    const NSPoint center = NSMakePoint(687.0, 568.0);
    for (uint32_t ring = 0u; ring < 5u; ++ring) {
        const CGFloat inset = static_cast<CGFloat>(ring) * 13.0;
        const CGFloat width = 132.0 + activity * 14.0 - inset;
        const CGFloat height = 54.0 + activity * 8.0 - inset * 0.30;
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    if (NSPointInRect(point,
        ...
        _menuItemCount = std::min<uint32_t>(
            s3g::kDrumCrashFactoryPresetCount, 64u);
        [self setNeedsDisplay:YES];
        return;
    }
    if (NSPointInRect(point,
    ...
    if (NSPointInRect(point,
        ...
        const auto randomized = s3g::drumCrashSafeRandomParams(
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool floating)
{
    ...
    p->guiView = [[S3GDrumCrashView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = false;
    [static_cast<S3GDrumCrashView*>(p->guiView) stopRefreshTimer];
...
bool guiShow(const clap_plugin_t* plugin)
{
    ...
    p->guiVisible = true;
    [static_cast<S3GDrumCrashView*>(p->guiView) startRefreshTimer];
    return true;
}
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "A procedural mono-compatible stereo crash synthesizer with crash, choke and bell articulations.",
    features
};
