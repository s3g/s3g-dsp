...
constexpr uint32_t kCrosspointCount = s3g::kAmbiGroupMatrixGroups * s3g::kAmbiGroupMatrixGroups;
...
struct SavedState {
    ...
    s3g::AmbiGroupMatrixParams params {};
};
...
struct Plugin {
    ...
    s3g::AmbiGroupMatrixParams params = s3g::makeDefaultAmbiGroupMatrixParams();
    s3g::AmbiGroupMatrix matrix;
    std::atomic<float> outputPeak { 0.0f };
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "../common/s3g_routing_parameter_members.inc"
    std::atomic<bool> routingActive{false};
#endif
#if defined(__APPLE__)
...
double defaultValueForParam(clap_id id)
{
    if (isCrosspointParam(id)) {
        ...
        const uint32_t dst = idx % s3g::kAmbiGroupMatrixGroups;
...
void applyRawParam(Plugin& p, clap_id id, double value)
#else
void applyParam(Plugin& p, clap_id id, double value)
#endif
{
    ...
    if (isCrosspointParam(id)) {
        ...
        case kParamMode: p.params.mode = value >= 0.5 ? s3g::AmbiGroupMatrixFlowMode::Sync : s3g::AmbiGroupMatrixFlowMode::Free; break;
...
{
    if (p.params.mode != s3g::AmbiGroupMatrixFlowMode::Sync) {
...
bool activate(const clap_plugin_t* plugin, double sampleRate, uint32_t, uint32_t maxFrames)
{
    ...
    p->routingActive.store(true, std::memory_order_release);
#endif
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
    self(plugin)->routingActive.store(false, std::memory_order_release);
#endif
}
...
clap_process_status process(const clap_plugin_t* plugin, const clap_process_t* process)
{
    ...
    if (input.data32 && output.data32) {
        ...
        s3g::clearAudioBufferFromChannel(output, s3g::kAmbiGroupMatrixChannels, frames);
        p->outputPeak.store(peakForBuffer(output.data32, std::min<uint32_t>(outChannels, s3g::kAmbiGroupMatrixChannels), frames),
            std::memory_order_relaxed);
    } else if (input.data64 && output.data64) {
        p->matrix.process(input.data64, input.channel_count, output.data64, outChannels, frames);
        s3g::clearAudioBufferFromChannel(output, s3g::kAmbiGroupMatrixChannels, frames);
        p->outputPeak.store(peakForBuffer(output.data64, std::min<uint32_t>(outChannels, s3g::kAmbiGroupMatrixChannels), frames),
            std::memory_order_relaxed);
    }
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
...
bool audioPortsGet(const clap_plugin_t*, uint32_t index, bool isInput, clap_audio_port_info_t* info)
{
    ...
    info->channel_count = s3g::kAmbiGroupMatrixChannels;
...
bool paramsGetInfo(const clap_plugin_t*, uint32_t index, clap_param_info_t* info)
{
    ...
    if (isCrosspointParam(id)) {
        const uint32_t idx = crosspointIndexFromParam(id);
        const uint32_t src = idx / s3g::kAmbiGroupMatrixGroups + 1u;
        const uint32_t dst = idx % s3g::kAmbiGroupMatrixGroups + 1u;
...
#endif
{
    ...
    case kParamMode: *value = p.mode == s3g::AmbiGroupMatrixFlowMode::Sync ? 1.0 : 0.0; return true;
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "../common/s3g_ambi_bus_parameter_bridge.inc"
#endif
...
void paramsFlush(const clap_plugin_t* plugin, const clap_input_events_t* in, const clap_output_events_t* out) {
    ...
    syncRoutingParameters(*self(plugin));
#endif
...
#undef stateSave
#undef stateLoad
#include "../common/s3g_ambi_bus_state_bridge.inc"
#endif
...
@implementation S3GAmbiGroupMatrixView
- (instancetype)initWithPlugin:(void*)plugin
{
...
- (void)randomizeMatrix
{
    ...
    for (uint32_t src = 0; src < s3g::kAmbiGroupMatrixGroups; ++src) {
        for (uint32_t dst = 0; dst < s3g::kAmbiGroupMatrixGroups; ++dst) {
            ...
            applyParam(*p, kCrosspointBase + s3g::ambiGroupMatrixIndex(src, dst), db);
        }
    }
    [self setNeedsDisplay:YES];
}
...
- (void)drawOpenMenu:(NSDictionary*)attrs style:(const s3g::clap_gui::Style&)style
{
    ...
    if (_openMenu == 1) {
        const int selected = p->params.mode == s3g::AmbiGroupMatrixFlowMode::Sync ? 1 : 0;
...
- (void)drawFlowPreview:(NSRect)rect attrs:(NSDictionary*)attrs style:(const s3g::clap_gui::Style&)style
{
    ...
    const CGFloat topY = rect.origin.y + 44.0;
    const CGFloat rowGap = (rect.size.height - 88.0) / 3.0;
    ...
    for (uint32_t src = 0; src < s3g::kAmbiGroupMatrixGroups; ++src) {
        ...
        for (uint32_t dst = 0; dst < s3g::kAmbiGroupMatrixGroups; ++dst) {
            const uint32_t idx = s3g::ambiGroupMatrixIndex(src, dst);
            const float manual = s3g::ambiGroupMatrixDbToGain(p->params.crosspointDb[idx]);
            if (manual <= 0.000001f) {
                continue;
            }
    ...
    for (uint32_t i = 0; i < s3g::kAmbiGroupMatrixGroups; ++i) {
...
- (void)drawRect:(NSRect)dirty
{
    ...
    for (uint32_t src = 0; src < s3g::kAmbiGroupMatrixGroups; ++src) {
        for (uint32_t dst = 0; dst < s3g::kAmbiGroupMatrixGroups; ++dst) {
            ...
            if (manualNorm > 0.01) {
                NSRect fill = NSInsetRect(r, 8.0, 8.0);
                fill.size.height *= manualNorm;
                fill.origin.y = r.origin.y + r.size.height - 8.0 - fill.size.height;
                ...
                NSRect liveFill = NSInsetRect(r, 8.0, 8.0);
                liveFill.size.height *= liveNorm;
                liveFill.origin.y = r.origin.y + r.size.height - 8.0 - liveFill.size.height;
                const CGFloat greenMix = static_cast<CGFloat>(std::clamp(p->params.motion * flowWeight, 0.0f, 1.0f));
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool isFloating)
{
    ...
    p->guiView = [[S3GAmbiGroupMatrixView alloc] initWithPlugin:p];
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    if (p->guiView) {
        p->guiVisible = false;
        [static_cast<S3GAmbiGroupMatrixView*>(p->guiView) stopRefreshTimer];
        s3g::clap_gui::destroyResponsiveViewport(p->guiViewport, p->guiView);
    }
}
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView || !s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, false)) return false; p->guiVisible = true; [static_cast<S3GAmbiGroupMatrixView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GAmbiGroupMatrixView*>(p->guiView) stopRefreshTimer]; return s3g::clap_gui::setResponsiveViewportHidden(p->guiViewport, true); }
const clap_plugin_gui_t guiExt { guiIsApiSupported, guiGetPreferredApi, guiCreate, guiDestroy, guiSetScale, guiGetSize, guiCanResize, guiGetResizeHints, guiAdjustSize, guiSetSize, guiSetParent, guiSetTransient, guiSuggestTitle, guiShow, guiHide };
#endif

#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#define S3G_AMBI_GROUP_MATRIX_CHANNELS 64
...
const clap_plugin_descriptor_t descriptor {
    ...
    "",
    "0.1.0",
    "64-channel lane-locked 4x3OA group matrix mixer with automatable bus crosspoints.",
    features
};
