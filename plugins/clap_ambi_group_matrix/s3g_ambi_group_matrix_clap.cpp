...
- (void)drawRect:(NSRect)dirty
{
    ...
    const CGFloat left = 92.0;
    const CGFloat top = 108.0;
    const CGFloat cell = 70.0;
    const float livePhase = p->matrix.previewPhase();
    const auto liveFlow = p->matrix.generatedFlowPreview(livePhase);
    for (uint32_t i = 0; i < s3g::kAmbiGroupMatrixGroups; ++i) {
        [[NSString stringWithFormat:@"D%u", i + 1u] drawAtPoint:NSMakePoint(left + i * cell + 30.0, top - 28.0) withAttributes:small];
        [[NSString stringWithFormat:@"S%u", i + 1u] drawAtPoint:NSMakePoint(left - 48.0, top + i * cell + 29.0) withAttributes:small];
    }
    ...
    for (uint32_t src = 0; src < s3g::kAmbiGroupMatrixGroups; ++src) {
        for (uint32_t dst = 0; dst < s3g::kAmbiGroupMatrixGroups; ++dst) {
            ...
            NSRect r = NSMakeRect(left + dst * cell, top + src * cell, cell - 8.0, cell - 8.0);
            [style.strip setFill];
            NSRectFill(r);
            [style.grid setStroke];
            NSFrameRect(r);
            ...
            [label drawAtPoint:NSMakePoint(r.origin.x + 16.0, r.origin.y + 27.0) withAttributes:text];
        }
    }
    ...
    s3g::clap_gui::drawMenu(@"MODE", prm.mode == s3g::AmbiGroupMatrixFlowMode::Sync ? @"SYNC" : @"FREE", 186, small, small, style, 752, 838, 118);
...
- (void)updateCell:(NSPoint)pt
{
    ...
    const CGFloat top = 108.0 + static_cast<CGFloat>(idx / s3g::kAmbiGroupMatrixGroups) * 70.0;
    const double n = std::clamp(1.0 - (pt.y - top) / 62.0, 0.0, 1.0);
...
void guiDestroy(const clap_plugin_t* plugin)
{
    ...
    if (p->guiView) {
        p->guiVisible = false;
        auto* v = static_cast<S3GAmbiGroupMatrixView*>(p->guiView);
        [v stopRefreshTimer];
        [v removeFromSuperview];
        [v release];
        p->guiView = nullptr;
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = true; [static_cast<NSView*>(p->guiView) setHidden:NO]; [static_cast<S3GAmbiGroupMatrixView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GAmbiGroupMatrixView*>(p->guiView) stopRefreshTimer]; [static_cast<NSView*>(p->guiView) setHidden:YES]; return true; }
