...
#if defined(__APPLE__)
@interface S3GOrbitDelayView : NSView { void* _plugin; int _dragSlider; NSTimer* _timer; }
...
- (void)drawRect:(NSRect)dirty
{
    ...
    [@"s3g ORBIT DELAY" drawAtPoint:NSMakePoint(18,14) withAttributes:lab];
    ...
    s3g::clap_gui::drawPanelHeader(@"ORBIT", true, 18, 42, 354, 21, lab, style);
    s3g::clap_gui::drawPanelFrame(388, 42, 354, 286, style);
    s3g::clap_gui::drawPanelHeader(@"DELAY / OUTPUT", true, 388, 42, 354, 21, lab, style);
    ...
    [@"circulating 16ch echo cloud" drawAtPoint:NSMakePoint(406, 334) withAttributes:small];
}
- (void)updateSlider:(NSPoint)point
{
    auto* p = static_cast<Plugin*>(_plugin);
...
bool guiCreate(const clap_plugin_t* plugin, const char* api, bool isFloating) { if (!guiIsApiSupported(plugin, api, isFloating)) return false; auto* p = self(plugin); if (p->guiView) return true; p->guiView = [[S3GOrbitDelayView alloc] initWithPlugin:p]; return p->guiView != nullptr; }
void guiDestroy(const clap_plugin_t* plugin) { auto* p = self(plugin); if (p->guiView) { p->guiVisible = false; auto* v = static_cast<S3GOrbitDelayView*>(p->guiView); [v stopRefreshTimer]; [v removeFromSuperview]; [v release]; p->guiView = nullptr; } }
...
bool guiShow(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = true; [static_cast<NSView*>(p->guiView) setHidden:NO]; [static_cast<S3GOrbitDelayView*>(p->guiView) startRefreshTimer]; return true; }
bool guiHide(const clap_plugin_t* plugin) { auto* p = self(plugin); if (!p->guiView) return false; p->guiVisible = false; [static_cast<S3GOrbitDelayView*>(p->guiView) stopRefreshTimer]; [static_cast<NSView*>(p->guiView) setHidden:YES]; return true; }
