...
{
    if (id < kToneParamId || id > kMidiReceiveParamId) return;
...
{
    if (id < kToneParamId || id > kMidiReceiveParamId) return 0.0;
    ...
    float velocity, int midiNote)
{
    p.visualTriggerCode.store(
        articulation == s3g::DrumClapArticulation::Tight ? 3u
            : articulation == s3g::DrumClapArticulation::Flam ? 2u : 1u,
...
{
    auto* p = &instance;
    if (s3g::kDrumClapFactoryPresetCount == 0u) return false;
...
#if defined(S3G_ENABLE_VSTGUI_CANVAS_GUI)
#include "s3g_drum_clap_vstgui.inc"
