...
constexpr const char* kPluginId = "org.s3g.s3g-dsp.drum-hi-hat";
constexpr const char* kPluginName = "s3g Drum Hi-Hat 2";
constexpr double kSampleRate = 48000.0;
constexpr uint32_t kFrames = 256u;
constexpr uint32_t kChannels = 2u;
constexpr int16_t kBaseMidiNote = 42;
...
constexpr std::array<ParamSpec, 28u> kParamSpecs {{
    { kTune, "Tune", 320.0, 3200.0, 1180.0, 1490.0, false },
    { kNoteTracking, "Note Tracking", 0.0, 1.0, 0.35, 0.37, false },
    { kAlloy, "Alloy", 0.0, 1.0, 0.62, 0.51, false },
    { kSpread, "Spread", 0.0, 1.0, 0.58, 0.63, false },
    { kDensity, "Density", 0.0, 1.0, 0.72, 0.41, false },
    { kTone, "Tone", 0.0, 1.0, 0.62, 0.63, false },
    { kAir, "Air", 0.0, 1.0, 0.55, 0.29, false },
    { kAttack, "Attack", 0.0, 1.0, 0.66, 0.82, false },
    { kClosedDecay, "Closed Decay", 0.03, 1.0, 0.18, 0.21, false },
    { kOpenDecay, "Open Decay", 0.12, 6.0, 1.35, 1.84, false },
    { kPedalDecay, "Pedal Decay", 0.03, 1.5, 0.30, 0.38, false },
    { kWash, "Wash", 0.0, 1.0, 0.58, 0.61, false },
    { kChick, "Chick", 0.0, 1.0, 0.52, 0.47, false },
    { kChickTone, "Chick Tone", 0.0, 1.0, 0.56, 0.57, false },
    { kSizzle, "Sizzle", 0.0, 1.0, 0.28, 0.72, false },
    { kChokeTime, "Choke Time", 0.3, 80.0, 7.0, 8.5, false },
    ...
    { kStereoWidth, "Stereo Width", 0.0, 1.0, 0.18, 0.67, false },
    { kVelocitySensitivity, "Velocity Sensitivity", 0.0, 1.0, 0.90, 0.83, false },
    { kOutputGain, "Output Gain", -36.0, 12.0, -8.0, -4.5, false },
    { kMidiReceive, "MIDI Receive", 0.0, 16.0, 0.0, 7.0, true },
    { kTrigger, "Trigger", 0.0, 3.0, 0.0, 3.0, true },
}};
    ...
    int16_t key, double velocity)
{
    ...
    for (size_t index = 1u; index < crossings.size(); ++index) {
        ...
        if (period >= kSampleRate / 7000.0
            && period <= kSampleRate / 300.0) {
...
{
    if (argc != 3) {
        std::cerr << "usage: s3g_drum_hi_hat_clap_smoke "
                  << "<bundle-or-binary> <plugin-id>\n";
        return 2;
    }
    if (std::strcmp(argv[2], kPluginId) != 0) {
        std::cerr << "unexpected drum hi-hat plugin id: " << argv[2]
                  << "\n";
        return 1;
    }
    ...
    if (binary.empty()) {
        std::cerr << "could not resolve drum hi-hat binary\n";
        return 1;
    }
    void* library = dlopen(binary.c_str(), RTLD_LOCAL | RTLD_NOW);
    if (!library) {
        std::cerr << "could not load drum hi-hat: " << dlerror() << "\n";
        return 1;
    }
    ...
    if (!entry || !entry->init(binary.c_str())) {
        std::cerr << "invalid drum hi-hat CLAP entry\n";
        dlclose(library);
        return 1;
    }
    ...
    if (!descriptor || !descriptor->id || !descriptor->name
        ...
        || std::strcmp(descriptor->version, "0.1.0") != 0) {
        std::cerr << "unexpected drum hi-hat factory or descriptor\n";
        entry->deinit();
        dlclose(library);
        return 1;
    }
    ...
    context.host.name = "s3g drum hi-hat smoke";
    ...
    if (!plugin || !plugin->init(plugin)) {
        std::cerr << "could not create drum hi-hat plugin\n";
    ...
    if (!ok) {
        std::cerr << "drum hi-hat CLAP port/parameter contract failed\n";
    ...
    if (!processing) {
        std::cerr << "could not activate drum hi-hat plugin\n";
    ...
    resetOnAudioThread(plugin);
    EventList delayedOpen;
    constexpr uint32_t kOpenTime = 151u;
    const bool addedOpen = delayedOpen.addNote(46, 1.0, kOpenTime);
    double openBefore = 0.0;
    double openAfter = 0.0;
    const bool openTiming = addedOpen
        && exactTimedStart(plugin, audio, &delayedOpen.input, kOpenTime,
            openBefore, openAfter);
    if (!openTiming) {
        std::cerr << "sample-accurate open-hat start failed: "
                  << openBefore << " / " << openAfter << "\n";
    }
    ok = ok && openTiming;
    ...
    const bool monoConfigured = flushParamsOnAudioThread(plugin, params, {
        { kTone, 0.8 }, { kAir, 0.7 }, { kOpenDecay, 0.2 },
        { kPedalDecay, 0.9 }, { kWash, 0.8 },
        { kChickTone, 0.8 }, { kStereoWidth, 0.0 },
    ...
    const bool wideConfigured = flushParamsOnAudioThread(plugin, params, {
        { kClosedDecay, 1.0 }, { kPedalDecay, 1.0 },
        { kWash, 0.8 }, { kChick, 0.12 },
        { kChickTone, 1.0 }, { kSizzle, 0.8 },
        { kChokeTime, 8.0 },
        { kStereoWidth, 1.0 },
    });
    resetOnAudioThread(plugin);
    ...
    // GM closed, pedal and open notes are three distinct articulations, and
    ...
    const bool articulationConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kTune, 1180.0 }, { kNoteTracking, 0.35 },
            { kAlloy, 0.62 }, { kSpread, 0.58 },
            { kTone, 0.8 }, { kAir, 0.5 }, { kAttack, 0.7 },
            { kClosedDecay, 0.12 }, { kOpenDecay, 1.4 },
            { kPedalDecay, 0.28 }, { kWash, 0.65 },
            { kChick, 0.64 }, { kChickTone, 0.8 },
            { kSizzle, 0.7 }, { kChokeTime, 6.0 },
            { kDrive, 0.0 }, { kStereoWidth, 0.0 },
        });
    const auto renderFirstBlock = [&](int16_t note, AudioBlock& result) {
        resetOnAudioThread(plugin);
        EventList strike;
    ...
    const auto renderFirstMidiBlock = [&](uint8_t note, AudioBlock& result) {
        resetOnAudioThread(plugin);
        EventList strike;
        if (!strike.addMidi(note, 127u)) return false;
        result.clear();
        const auto status = runBlock(plugin, result, &strike.input);
        return status != CLAP_PROCESS_ERROR && finiteBlock(result)
            && blockEnergy(result) > 1.0e-9;
    };
    AudioBlock closedBlock;
    AudioBlock pedalBlock;
    AudioBlock openBlock;
    const bool articulationRouting = articulationConfigured
        && renderFirstBlock(42, closedBlock)
        && renderFirstBlock(44, pedalBlock)
        && renderFirstBlock(46, openBlock)
        && !blocksMatchExactly(closedBlock, pedalBlock)
        && !blocksMatchExactly(closedBlock, openBlock)
        && !blocksMatchExactly(pedalBlock, openBlock);
    if (!articulationRouting) {
        std::cerr << "closed/open/pedal articulation routing failed\n";
    }
    ok = ok && articulationRouting;

    AudioBlock midiClosedBlock;
    AudioBlock midiPedalBlock;
    AudioBlock midiOpenBlock;
    const bool rawMidiRouting = renderFirstMidiBlock(42u, midiClosedBlock)
        && renderFirstMidiBlock(44u, midiPedalBlock)
        && renderFirstMidiBlock(46u, midiOpenBlock)
        && blocksMatchExactly(closedBlock, midiClosedBlock)
        && blocksMatchExactly(pedalBlock, midiPedalBlock)
        && blocksMatchExactly(openBlock, midiOpenBlock);
    if (!rawMidiRouting) {
        std::cerr << "raw MIDI hat routing diverged from CLAP notes\n";
    }
    ok = ok && rawMidiRouting;

    // Use two chromatic closed-hat notes outside the dedicated 44/46 keys.
    ...
    const bool noteResponse = lowNote.finite && highNote.finite
        && softNote.finite
        && lowNote.frequency >= 500.0 && lowNote.frequency <= 2400.0
        && octaveRatio >= 1.65 && octaveRatio <= 2.35
    ...
    const bool stateConfigured = flushParamsOnAudioThread(plugin, params, {
        ...
        { kAlloy, 0.22 }, { kSpread, 0.61 },
        { kDensity, 0.0 }, { kTone, 0.0 }, { kAir, 0.0 },
        { kAttack, 0.85 }, { kClosedDecay, 0.28 },
        { kOpenDecay, 2.2 }, { kPedalDecay, 0.44 },
        { kWash, 0.48 }, { kChick, 0.64 },
        { kChickTone, 0.8 }, { kSizzle, 0.65 },
        { kChokeTime, 8.0 }, { kDrive, 0.0 },
        { kBias, -0.41 }, { kCompression, 0.0 },
        { kRateReduction, 0.35 }, { kBitDepth, 0.0 },
        { kReconstruction, 0.0 }, { kCharacterTone, 0.0 },
        { kStereoWidth, 0.73 }, { kOutputGain, -6.0 },
    });
    ...
    if (saved) {
        std::memcpy(stateHeader.data(), memory.bytes.data(),
            sizeof(stateHeader));
        saved = stateHeader[0u] == 0x48473353u
            && stateHeader[1u] == 2u
    ...
    const bool stateMutated = flushParamsWhileInactive(plugin, params, {
        ...
        { kPedalDecay, 0.08 }, { kChick, 0.02 },
        { kChickTone, 0.0 }, { kChokeTime, 0.5 },
    ...
    const bool restored = saved && mutationConsumed && stagedTriggerRequested
        ...
        && getParam(plugin, params, kPedalDecay, 0.44)
        && getParam(plugin, params, kChick, 0.64)
    ...
    if (!parameterTriggered) {
        std::cerr << "Closed Trigger did not request/process a hi-hat strike\n";
    }
    const AudioBlock parameterClosedBlock = audio;
    resetOnAudioThread(plugin);
    const bool openTriggerAdded = flushParamsOnAudioThread(plugin, params, {
        { kTrigger, 2.0 },
    });
    audio.clear();
    const auto openTriggerStatus = runBlock(plugin, audio);
    const bool parameterOpenTriggered = openTriggerAdded
        && openTriggerStatus != CLAP_PROCESS_ERROR && finiteBlock(audio)
        && blockEnergy(audio) > 1.0e-9
        && !blocksMatchExactly(parameterClosedBlock, audio)
        && getParam(plugin, params, kTrigger, 0.0);
    if (!parameterOpenTriggered) {
        std::cerr << "Open Trigger did not produce a distinct hi-hat strike\n";
    }
    resetOnAudioThread(plugin);
    ...
    audio.clear();
    const auto pedalTriggerStatus = runBlock(plugin, audio);
    const bool parameterPedalTriggered = pedalTriggerAdded
        && pedalTriggerStatus != CLAP_PROCESS_ERROR && finiteBlock(audio)
        && blockEnergy(audio) > 1.0e-9
        && !blocksMatchExactly(parameterClosedBlock, audio)
        && getParam(plugin, params, kTrigger, 0.0);
    if (!parameterPedalTriggered) {
        std::cerr << "Pedal Trigger did not produce a distinct hi-hat strike\n";
    }
    ...
    const bool longVoiceConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kSpread, 1.0 }, { kDensity, 1.0 },
            { kTone, 1.0 }, { kAir, 1.0 }, { kAttack, 1.0 },
            { kClosedDecay, 1.0 }, { kOpenDecay, 6.0 },
            { kPedalDecay, 1.5 }, { kWash, 1.0 }, { kChick, 1.0 },
            { kChickTone, 0.0 }, { kSizzle, 1.0 },
            { kChokeTime, 40.0 }, { kDrive, 0.0 },
        });
    resetOnAudioThread(plugin);
    const uint32_t configuredLongVoiceTail = tail->get(plugin);
    EventList longVoiceStrike;
    const bool addedLongVoice = longVoiceStrike.addNote(46, 1.0);
    audio.clear();
    ...
    const bool shortenedActiveVoice = flushParamsOnAudioThread(
        plugin, params, {
            { kSpread, 0.0 }, { kDensity, 0.0 },
            { kTone, 0.0 }, { kAir, 0.0 }, { kAttack, 0.0 },
            { kClosedDecay, 0.03 }, { kOpenDecay, 0.12 },
            { kPedalDecay, 0.03 }, { kWash, 0.0 }, { kChick, 0.0 },
            { kChickTone, 1.0 }, { kSizzle, 0.0 },
            { kChokeTime, 0.3 },
            { kDrive, 0.0 },
        });
    ...
    const bool activeTailLatched = longVoiceConfigured && addedLongVoice
        ...
        && configuredLongVoiceTail
            >= static_cast<uint32_t>(kSampleRate * 12.0)
    ...
    // The open articulation must be represented in the prospective host tail.
    const bool longOpenConfigured = flushParamsOnAudioThread(plugin, params, {
        { kDensity, 0.7 }, { kAttack, 0.5 }, { kAir, 1.0 },
        { kOpenDecay, 1.0 }, { kPedalDecay, 0.1 }, { kWash, 0.7 },
        { kChick, 0.2 }, { kChickTone, 0.5 }, { kSizzle, 0.2 },
        { kChokeTime, 5.0 },
    });
    const uint32_t longOpenTail = tail->get(plugin);
    const bool openTailCovered = longOpenConfigured
        && longOpenTail >= static_cast<uint32_t>(kSampleRate * 1.8)
        && longOpenTail <= static_cast<uint32_t>(kSampleRate * 4.0);
    if (!openTailCovered) {
        std::cerr << "open lifetime was absent from CLAP tail: "
                  << longOpenTail << "\n";
    }
    ok = ok && openTailCovered;
    ...
    const bool tightDriveBase = flushParamsOnAudioThread(plugin, params, {
        { kSpread, 0.0 }, { kTone, 0.0 }, { kAir, 0.0 },
        { kAttack, 0.0 }, { kClosedDecay, 0.03 }, { kOpenDecay, 0.12 },
        { kPedalDecay, 0.03 }, { kWash, 0.0 }, { kChick, 0.0 },
        { kChickTone, 1.0 }, { kSizzle, 0.0 },
        { kChokeTime, 0.3 }, { kDrive, 0.0 },
    });
    resetOnAudioThread(plugin);
    audio.clear();
    ...
    const bool shortConfigured = flushParamsOnAudioThread(plugin, params, {
        { kAlloy, 0.0 }, { kDensity, 0.0 },
        { kTone, 1.0 }, { kAir, 0.0 }, { kAttack, 0.0 },
        { kClosedDecay, 0.03 }, { kOpenDecay, 0.12 },
        { kPedalDecay, 0.03 }, { kChick, 0.0 },
        { kChickTone, 1.0 }, { kChokeTime, 0.3 },
    ...
    const bool coherenceConfiguredA = flushParamsWhileInactive(
        plugin, params, {
            { kTune, 500.0 }, { kNoteTracking, 0.0 },
            { kAlloy, 0.0 }, { kSpread, 0.10 },
            { kDensity, 0.0 }, { kTone, 1.0 }, { kAir, 0.0 },
            { kAttack, 0.08 }, { kClosedDecay, 0.10 },
            { kOpenDecay, 1.0 }, { kPedalDecay, 0.05 },
            { kWash, 0.0 }, { kChick, 0.02 },
            { kChickTone, 0.0 }, { kSizzle, 0.0 },
            { kChokeTime, 0.5 },
    ...
    const bool coherenceConfiguredB = flushParamsWhileInactive(
        plugin, params, {
            { kTune, 2400.0 }, { kNoteTracking, 1.0 },
            { kAlloy, 1.0 }, { kSpread, 1.0 },
            { kDensity, 1.0 }, { kTone, 0.15 }, { kAir, 1.0 },
            { kAttack, 0.90 }, { kClosedDecay, 1.0 },
            { kOpenDecay, 3.0 }, { kPedalDecay, 1.2 },
            { kWash, 1.0 }, { kChick, 0.90 },
            { kChickTone, 1.0 }, { kSizzle, 1.0 },
            { kChokeTime, 40.0 },
            { kDrive, 0.0 }, { kBias, 0.0 }, { kCompression, 0.0 },
            { kRateReduction, 0.0 }, { kBitDepth, 0.0 },
            { kReconstruction, 0.0 }, { kCharacterTone, 0.0 },
    ...
    if (!ok) {
        std::cerr << "drum hi-hat CLAP smoke failed\n";
        return 1;
    }
    std::cout << "drum hi-hat CLAP smoke passed\n";
    return 0;
}
