...
#include "../plugins/common/s3g_windows_processor_float_mode.h"
#include <cstdio>
#include <limits>
...
int main() {
    ...
    for (unsigned flush : {0u, 0x40u, 0x8000u, 0x8040u}) {
        for (unsigned rounding : {0u, 0x2000u, 0x4000u, 0x6000u}) {
            ...
            _mm_setcsr(initial);
            {
                s3g::clap_detail::ScopedWindowsProcessorFloatMode outer;
                ...
                {
                    s3g::clap_detail::ScopedWindowsProcessorFloatMode nested;
