...
constexpr const char* kPluginId = "org.s3g.s3g-dsp.drum-snare";
constexpr const char* kPluginName = "s3g Drum Snare 2";
constexpr double kSampleRate = 48000.0;
constexpr uint32_t kFrames = 256u;
constexpr uint32_t kChannels = 2u;
constexpr int16_t kBaseMidiNote = 38;
...
constexpr std::array<ParamSpec, 28u> kParamSpecs {{
    { kTune, "Tune", 70.0, 420.0, 180.0, 200.0, false },
    { kNoteTracking, "Note Tracking", 0.0, 1.0, 0.75, 0.37, false },
    { kPitchDrop, "Pitch Drop", -12.0, 36.0, 7.0, 12.5, false },
    { kSweepTime, "Sweep Time", 1.0, 250.0, 22.0, 42.5, false },
    { kShellSpread, "Shell Spread", 0.0, 1.0, 0.40, 0.41, false },
    { kBody, "Body", 0.0, 1.0, 0.58, 0.63, false },
    { kRing, "Ring", 0.0, 1.0, 0.32, 0.29, false },
    { kBodyDecay, "Body Decay", 0.02, 3.0, 0.42, 0.42, false },
    { kPunch, "Punch", 0.0, 1.0, 0.78, 0.71, false },
    { kWires, "Wires", 0.0, 1.0, 0.70, 0.68, false },
    { kWireTone, "Wire Tone", 0.0, 1.0, 0.56, 0.54, false },
    { kWireTension, "Wire Tension", 0.0, 1.0, 0.48, 0.46, false },
    { kWireDecay, "Wire Decay", 0.01, 4.0, 0.34, 0.34, false },
    { kRattle, "Rattle", 0.0, 1.0, 0.18, 0.17, false },
    { kClick, "Click", 0.0, 1.0, 0.10, 0.11, false },
    { kClickTone, "Click Tone", 0.0, 1.0, 0.60, 0.62, false },
    ...
    { kTrigger, "Trigger", 0.0, 1.0, 0.0, 1.0, true },
}};
...
{
    if (argc != 3) {
        std::cerr << "usage: s3g_drum_snare_clap_smoke "
                  << "<bundle-or-binary> <plugin-id>\n";
        return 2;
    }
    if (std::strcmp(argv[2], kPluginId) != 0) {
        std::cerr << "unexpected drum snare plugin id: " << argv[2]
                  << "\n";
        return 1;
    }
    ...
    if (binary.empty()) {
        std::cerr << "could not resolve drum snare binary\n";
        return 1;
    }
    void* library = dlopen(binary.c_str(), RTLD_LOCAL | RTLD_NOW);
    if (!library) {
        std::cerr << "could not load drum snare: " << dlerror() << "\n";
        return 1;
    }
    ...
    if (!entry || !entry->init(binary.c_str())) {
        std::cerr << "invalid drum snare CLAP entry\n";
        dlclose(library);
        return 1;
    }
    ...
    if (!descriptor || !descriptor->id || !descriptor->name
        ...
        std::cerr << "unexpected drum snare factory or descriptor\n";
        entry->deinit();
        dlclose(library);
        return 1;
    }
    ...
    context.host.name = "s3g drum snare smoke";
    ...
    if (!plugin || !plugin->init(plugin)) {
        std::cerr << "could not create drum snare plugin\n";
    ...
    if (!ok) {
        std::cerr << "drum snare CLAP port/parameter contract failed\n";
    ...
    if (!processing) {
        std::cerr << "could not activate drum snare plugin\n";
    ...
    // Width zero is exact dual mono, including wires and the attack layers.
    const bool monoConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBody, 0.8 }, { kRing, 0.7 }, { kWires, 0.9 },
        { kRattle, 0.8 }, { kClick, 0.8 }, { kStereoWidth, 0.0 },
    ...
    // Full width must create side energy in the wire/rattle layers.
    const bool wideConfigured = flushParamsOnAudioThread(plugin, params, {
        { kWires, 1.0 }, { kWireTone, 0.8 },
        { kWireTension, 0.6 }, { kRattle, 1.0 },
        { kClick, 0.7 }, { kClickTone, 0.8 },
        { kStereoWidth, 1.0 },
    });
    resetOnAudioThread(plugin);
    ...
    const auto highNote = renderTonalStrike(
        plugin, params, audio, kBaseMidiNote + 12, 1.0);
    ...
    const bool noteResponse = lowNote.finite && highNote.finite
        && softNote.finite
        && lowNote.frequency >= 130.0 && lowNote.frequency <= 210.0
    ...
    const bool stateConfigured = flushParamsOnAudioThread(plugin, params, {
        { kTune, 271.5 }, { kNoteTracking, 0.0 },
        { kPitchDrop, 0.0 }, { kSweepTime, 1.0 },
        { kShellSpread, 0.0 }, { kBody, 0.0 }, { kRing, 0.0 },
        { kBodyDecay, 1.25 }, { kPunch, 0.0 },
        { kWires, 1.0 }, { kWireTone, 0.56 },
        { kWireTension, 0.48 }, { kWireDecay, 0.34 },
        { kRattle, 0.0 }, { kClick, 0.0 }, { kDrive, 0.0 },
    ...
    if (saved) {
        std::memcpy(stateHeader.data(), memory.bytes.data(),
            sizeof(stateHeader));
        saved = stateHeader[0u] == 0x53473353u
            && stateHeader[1u] == 2u
    ...
    const bool restored = saved && mutationConsumed && stagedTriggerRequested
        ...
        && getParam(plugin, params, kTune, 271.5)
        && getParam(plugin, params, kBodyDecay, 1.25)
        && getParam(plugin, params, kWires, 1.0)
        && getParam(plugin, params, kWireDecay, 0.34)
    ...
    if (!parameterTriggered) {
        std::cerr << "Trigger did not request/process a snare strike\n";
    }
    ...
    const bool longVoiceConfigured = flushParamsOnAudioThread(
        plugin, params, {
            ...
            { kWires, 1.0 }, { kWireDecay, 4.0 },
            { kRattle, 1.0 }, { kClick, 1.0 }, { kDrive, 1.0 },
        });
    resetOnAudioThread(plugin);
    ...
    const bool activeTailLatched = longVoiceConfigured && addedLongVoice
        ...
        && configuredLongVoiceTail
            >= static_cast<uint32_t>(kSampleRate * 20.0)
    ...
    // The independent wire envelope must be included in the advertised tail.
    const bool longWireConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBodyDecay, 0.02 }, { kWireDecay, 4.0 },
        { kWires, 1.0 }, { kRattle, 0.0 }, { kClick, 0.0 },
    });
    const uint32_t longWireTail = tail->get(plugin);
    const bool wireTailCovered = longWireConfigured
        && longWireTail >= static_cast<uint32_t>(kSampleRate * 8.5)
        && longWireTail <= static_cast<uint32_t>(kSampleRate * 8.7);
    if (!wireTailCovered) {
        std::cerr << "wire lifetime was absent from CLAP tail: "
                  << longWireTail << "\n";
    }
    ok = ok && wireTailCovered;
    ...
    const bool tightDriveBase = flushParamsOnAudioThread(plugin, params, {
        { kSweepTime, 1.0 }, { kBody, 0.0 }, { kRing, 0.0 },
        { kBodyDecay, 0.02 }, { kPunch, 1.0 },
        { kWires, 0.0 }, { kWireDecay, 0.01 }, { kRattle, 0.0 },
        { kClick, 0.0 }, { kClickTone, 1.0 }, { kDrive, 0.0 },
    });
    resetOnAudioThread(plugin);
    audio.clear();
    ...
    const bool shortConfigured = flushParamsOnAudioThread(plugin, params, {
        ...
        { kBody, 1.0 }, { kRing, 0.0 }, { kBodyDecay, 0.02 },
        { kPunch, 0.0 }, { kWires, 0.0 }, { kWireDecay, 0.01 },
        { kRattle, 0.0 }, { kClick, 0.0 },
    ...
    const bool coherenceConfiguredA = flushParamsWhileInactive(
        plugin, params, {
            { kTune, 90.0 }, { kNoteTracking, 0.0 },
            { kPitchDrop, -12.0 }, { kSweepTime, 1.0 },
            { kShellSpread, 0.0 }, { kBody, 1.0 }, { kRing, 0.0 },
            { kBodyDecay, 0.08 }, { kPunch, 0.10 },
            { kWires, 0.05 }, { kWireTone, 0.0 },
            { kWireTension, 0.0 }, { kWireDecay, 0.05 },
            { kRattle, 0.0 }, { kClick, 0.0 }, { kClickTone, 0.0 },
    ...
    const bool coherenceConfiguredB = flushParamsWhileInactive(
        plugin, params, {
            { kTune, 390.0 }, { kNoteTracking, 1.0 },
            { kPitchDrop, 36.0 }, { kSweepTime, 250.0 },
            { kShellSpread, 1.0 }, { kBody, 0.15 }, { kRing, 1.0 },
            { kBodyDecay, 2.5 }, { kPunch, 1.0 },
            { kWires, 1.0 }, { kWireTone, 1.0 },
            { kWireTension, 1.0 }, { kWireDecay, 3.0 },
            { kRattle, 1.0 }, { kClick, 1.0 }, { kClickTone, 1.0 },
            { kDrive, 0.0 }, { kBias, 0.0 }, { kCompression, 0.0 },
            { kRateReduction, 0.0 }, { kBitDepth, 0.0 },
            { kReconstruction, 0.0 }, { kCharacterTone, 0.0 },
    ...
    if (!ok) {
        std::cerr << "drum snare CLAP smoke failed\n";
        return 1;
    }
    std::cout << "drum snare CLAP smoke passed\n";
    return 0;
}
