// Compare independently built Cocoa and VSTGUI binaries. This deliberately
// does not include their implementation sources or duplicate their DSP math.
#include <algorithm>
#include <clap/clap.h>
...
int main(int argc, char **argv) {
  ...
  for (unsigned scene = 0; scene < 3; ++scene) {
    ...
    if (id.find("ambisonic-stereo-decoder") != std::string::npos ||
        id.find("ambi-speaker-decoder") != std::string::npos)
