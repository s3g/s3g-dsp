...
constexpr int16_t kBaseMidiNote = 39;
...
constexpr std::array<ParamSpec, 28u> kParamSpecs {{
    { kClapTone, "Tone", 700.0, 10000.0, 3900.0, 4870.0, false },
    { kNoteTracking, "Note Tracking", 0.0, 1.0, 0.20, 0.37, false },
    { kHands, "Hands", 1.0, 8.0, 4.0, 6.0, true },
    { kSpread, "Spread", 0.0, 65.0, 28.0, 41.5, false },
    { kScatter, "Scatter", 0.0, 1.0, 0.45, 0.41, false },
    { kBandwidth, "Bandwidth", 0.0, 1.0, 0.72, 0.63, false },
    { kAir, "Air", 0.0, 1.0, 0.42, 0.29, false },
    { kAttack, "Attack", 0.0, 1.0, 0.65, 0.82, false },
    { kBurstDecay, "Burst Decay", 0.006, 0.18, 0.030, 0.071, false },
    { kTailDecay, "Tail Decay", 0.025, 2.0, 0.18, 0.84, false },
    { kTail, "Tail", 0.0, 1.0, 0.48, 0.38, false },
    { kBody, "Body", 0.0, 1.0, 0.30, 0.61, false },
    { kBodyTune, "Body Tune", 280.0, 3200.0, 940.0, 1270.0, false },
    { kBodyDecay, "Body Decay", 0.012, 0.60, 0.075, 0.27, false },
    { kFlamTime, "Flam Time", 5.0, 120.0, 34.0, 72.0, false },
    { kTexture, "Texture", 0.0, 1.0, 0.56, 0.35, false },
    ...
    { kStereoWidth, "Stereo Width", 0.0, 1.0, 0.36, 0.67, false },
    { kVelocitySensitivity, "Velocity Sensitivity", 0.0, 1.0, 0.90, 0.83, false },
    { kOutputGain, "Output Gain", -36.0, 12.0, -7.5, -4.5, false },
    ...
    const clap_plugin_params_t* params)
{
    ...
    return !params->get_info(plugin,
        ...
        && !params->text_to_value(plugin, kClapTone,
            "not a number", &parsed)
        && !params->text_to_value(plugin, kClapTone,
...
{
    if (argc != 3) {
        std::cerr << "usage: s3g_drum_clap_clap_smoke "
                  << "<bundle-or-binary> <plugin-id>\n";
        return 2;
    }
    if (std::strcmp(argv[2], kPluginId) != 0) {
        std::cerr << "unexpected drum clap plugin id: " << argv[2]
                  << "\n";
        return 1;
    }
    ...
    if (binary.empty()) {
        std::cerr << "could not resolve drum clap binary\n";
        return 1;
    }
    void* library = dlopen(binary.c_str(), RTLD_LOCAL | RTLD_NOW);
    if (!library) {
        std::cerr << "could not load drum clap: " << dlerror() << "\n";
        return 1;
    }
    ...
    if (!entry || !entry->init(binary.c_str())) {
        std::cerr << "invalid drum clap CLAP entry\n";
        dlclose(library);
        return 1;
    }
    ...
    if (!descriptor || !descriptor->id || !descriptor->name
        ...
        std::cerr << "unexpected drum clap factory or descriptor\n";
        entry->deinit();
        dlclose(library);
        return 1;
    }
    ...
    context.host.name = "s3g drum clap smoke";
    ...
    if (!plugin || !plugin->init(plugin)) {
        std::cerr << "could not create drum clap plugin\n";
    ...
    if (!ok) {
        std::cerr << "drum clap CLAP port/parameter contract failed\n";
    ...
    if (!processing) {
        std::cerr << "could not activate drum clap plugin\n";
    ...
    constexpr uint32_t kFlamTimeSample = 151u;
    const bool addedFlam = delayedFlam.addNote(40, 1.0, kFlamTimeSample);
    double flamBefore = 0.0;
    double flamAfter = 0.0;
    const bool flamTiming = addedFlam
        && exactTimedStart(plugin, audio, &delayedFlam.input,
            kFlamTimeSample, flamBefore, flamAfter);
    if (!flamTiming) {
        std::cerr << "sample-accurate flam start failed: "
                  << flamBefore << " / " << flamAfter << "\n";
    }
    ok = ok && flamTiming;
    ...
    const bool monoConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBandwidth, 0.8 }, { kAir, 0.7 }, { kTailDecay, 0.2 },
        { kTail, 0.9 }, { kBody, 0.8 },
        { kBodyDecay, 0.5 }, { kStereoWidth, 0.0 },
    ...
    const bool wideConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBurstDecay, 0.12 }, { kTail, 1.0 },
        { kBody, 0.8 }, { kBodyTune, 1200.0 },
        { kBodyDecay, 0.4 }, { kFlamTime, 62.0 },
        { kTexture, 0.8 },
        { kStereoWidth, 1.0 },
    });
    resetOnAudioThread(plugin);
    ...
    const bool articulationConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kClapTone, 3900.0 }, { kNoteTracking, 0.35 },
            { kHands, 5.0 }, { kSpread, 32.0 },
            { kBandwidth, 0.8 }, { kAir, 0.5 }, { kAttack, 0.7 },
            { kBurstDecay, 0.12 }, { kTailDecay, 1.4 },
            { kTail, 0.28 }, { kBody, 0.65 },
            { kBodyTune, 940.0 }, { kBodyDecay, 0.18 },
            { kFlamTime, 64.0 }, { kTexture, 0.6 },
    ...
    const bool stateConfigured = flushParamsOnAudioThread(plugin, params, {
        { kClapTone, 1527.5 }, { kNoteTracking, 0.0 },
        { kHands, 6.0 }, { kSpread, 31.0 },
        { kScatter, 0.0 }, { kBandwidth, 0.0 }, { kAir, 0.0 },
        { kAttack, 0.85 }, { kBurstDecay, 0.12 },
        { kTailDecay, 0.82 }, { kTail, 0.44 },
        { kBody, 0.48 }, { kBodyTune, 1264.0 },
        { kBodyDecay, 0.28 }, { kFlamTime, 65.0 },
    ...
    const bool stateMutated = flushParamsWhileInactive(plugin, params, {
        { kClapTone, 910.0 }, { kAttack, 0.1 }, { kBias, 0.2 },
        { kTail, 0.08 }, { kBodyTune, 420.0 },
        { kBodyDecay, 0.02 }, { kTexture, 0.5 },
    ...
    const bool restored = saved && mutationConsumed && stagedTriggerRequested
        ...
        && getParam(plugin, params, kClapTone, 1527.5)
        && getParam(plugin, params, kAttack, 0.85)
        && getParam(plugin, params, kTail, 0.44)
        && getParam(plugin, params, kBodyTune, 1264.0)
    ...
    const bool longVoiceConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kSpread, 65.0 }, { kScatter, 1.0 },
            { kBandwidth, 1.0 }, { kAir, 1.0 }, { kAttack, 1.0 },
            { kBurstDecay, 0.18 }, { kTailDecay, 2.0 },
            { kTail, 1.0 }, { kBody, 1.0 }, { kBodyTune, 1200.0 },
            { kBodyDecay, 0.60 }, { kFlamTime, 120.0 },
    ...
    const bool shortenedActiveVoice = flushParamsOnAudioThread(
        plugin, params, {
            { kSpread, 0.0 }, { kScatter, 0.0 },
            { kBandwidth, 0.0 }, { kAir, 0.0 }, { kAttack, 0.0 },
            { kBurstDecay, 0.006 }, { kTailDecay, 0.025 },
            { kTail, 0.0 }, { kBody, 0.0 }, { kBodyTune, 280.0 },
            { kBodyDecay, 0.012 }, { kFlamTime, 5.0 },
            { kTexture, 0.0 },
            { kDrive, 0.0 },
        });
    ...
    const bool longFlamConfigured = flushParamsOnAudioThread(plugin, params, {
        { kScatter, 0.7 }, { kAttack, 0.5 }, { kAir, 1.0 },
        { kTailDecay, 1.0 }, { kTail, 0.7 }, { kBody, 0.7 },
        { kBodyTune, 820.0 }, { kBodyDecay, 0.5 }, { kFlamTime, 80.0 },
        { kTexture, 0.5 },
    });
    ...
    const bool tightDriveBase = flushParamsOnAudioThread(plugin, params, {
        { kSpread, 0.0 }, { kBandwidth, 0.0 }, { kAir, 0.0 },
        { kAttack, 0.0 }, { kBurstDecay, 0.006 }, { kTailDecay, 0.025 },
        { kTail, 0.0 }, { kBody, 0.0 }, { kBodyTune, 280.0 },
        { kBodyDecay, 0.012 }, { kFlamTime, 5.0 },
    ...
    const bool shortConfigured = flushParamsOnAudioThread(plugin, params, {
        { kHands, 1.0 }, { kScatter, 0.0 },
        { kBandwidth, 1.0 }, { kAir, 0.0 }, { kAttack, 0.0 },
        { kBurstDecay, 0.006 }, { kTailDecay, 0.025 },
        { kTail, 0.0 }, { kBodyTune, 280.0 },
        { kBodyDecay, 0.012 }, { kTexture, 0.0 },
    ...
    const bool coherenceConfiguredA = flushParamsWhileInactive(
        plugin, params, {
            { kClapTone, 900.0 }, { kNoteTracking, 0.0 },
            { kHands, 2.0 }, { kSpread, 6.0 },
            { kScatter, 0.0 }, { kBandwidth, 1.0 }, { kAir, 0.0 },
            { kAttack, 0.08 }, { kBurstDecay, 0.025 },
            { kTailDecay, 0.12 }, { kTail, 0.05 },
            { kBody, 0.0 }, { kBodyTune, 520.0 },
            { kBodyDecay, 0.025 }, { kFlamTime, 15.0 },
            { kTexture, 0.2 },
    ...
    const bool coherenceConfiguredB = flushParamsWhileInactive(
        plugin, params, {
            { kClapTone, 7800.0 }, { kNoteTracking, 1.0 },
            { kHands, 8.0 }, { kSpread, 58.0 },
            { kScatter, 1.0 }, { kBandwidth, 0.15 }, { kAir, 1.0 },
            { kAttack, 0.90 }, { kBurstDecay, 0.16 },
            { kTailDecay, 1.6 }, { kTail, 0.9 },
            { kBody, 1.0 }, { kBodyTune, 1800.0 },
            { kBodyDecay, 0.5 }, { kFlamTime, 105.0 },
            { kTexture, 0.9 },
            { kDrive, 0.0 }, { kBias, 0.0 }, { kCompression, 0.0 },
            { kRateReduction, 0.0 }, { kBitDepth, 0.0 },
            { kReconstruction, 0.0 }, { kCharacterTone, 0.0 },
    ...
    if (!ok) {
        std::cerr << "drum clap CLAP smoke failed\n";
        return 1;
    }
    std::cout << "drum clap CLAP smoke passed\n";
    return 0;
}
