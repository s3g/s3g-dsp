...
constexpr int16_t kBaseMidiNote = 43;
...
constexpr std::array<ParamSpec, 27u> kParamSpecs {{
    ...
    { kSweepTime, "Sweep Time", 1.0, 300.0, 45.0, 62.5, false },
    { kShellSpread, "Shell Spread", 0.0, 1.0, 0.42, 0.41, false },
    { kBody, "Body", 0.0, 1.0, 0.70, 0.63, false },
    { kRing, "Ring", 0.0, 1.0, 0.42, 0.29, false },
    { kBodyDecay, "Body Decay", 0.04, 4.0, 0.70, 0.82, false },
    { kPunch, "Punch", 0.0, 1.0, 0.72, 0.71, false },
    { kDamping, "Damping", 0.0, 1.0, 0.35, 0.34, false },
    { kRimLevel, "Rim Level", 0.0, 1.0, 0.55, 0.68, false },
    { kRimCharacter, "Rim Character", 0.0, 1.0, 0.48, 0.61, false },
    { kRimDecay, "Rim Decay", 0.02, 0.35, 0.07, 0.12, false },
    { kStickLevel, "Stick Level", 0.0, 1.0, 0.35, 0.57, false },
    { kStickTone, "Stick Tone", 0.0, 1.0, 0.58, 0.72, false },
    { kStickDecay, "Stick Decay", 0.5, 40.0, 4.5, 8.5, false },
    ...
    int16_t key, double velocity)
{
    ...
    for (size_t index = 1u; index < crossings.size(); ++index) {
        ...
        if (period >= kSampleRate / 900.0
            && period <= kSampleRate / 60.0) {
...
{
    if (argc != 3) {
        std::cerr << "usage: s3g_drum_floor_tom_clap_smoke "
                  << "<bundle-or-binary> <plugin-id>\n";
        return 2;
    }
    if (std::strcmp(argv[2], kPluginId) != 0) {
        std::cerr << "unexpected drum floor tom plugin id: " << argv[2]
                  << "\n";
        return 1;
    }
    ...
    if (binary.empty()) {
        std::cerr << "could not resolve drum floor tom binary\n";
        return 1;
    }
    void* library = dlopen(binary.c_str(), RTLD_LOCAL | RTLD_NOW);
    if (!library) {
        std::cerr << "could not load drum floor tom: " << dlerror() << "\n";
        return 1;
    }
    ...
    if (!entry || !entry->init(binary.c_str())) {
        std::cerr << "invalid drum floor tom CLAP entry\n";
        dlclose(library);
        return 1;
    }
    ...
    if (!descriptor || !descriptor->id || !descriptor->name
        ...
        std::cerr << "unexpected drum floor tom factory or descriptor\n";
        entry->deinit();
        dlclose(library);
        return 1;
    }
    ...
    context.host.name = "s3g drum floor tom smoke";
    ...
    if (!plugin || !plugin->init(plugin)) {
        std::cerr << "could not create drum floor tom plugin\n";
    ...
    constexpr std::array<int16_t, 5u> expectedNamedKeys {{
        37, 41, 43, 53, 55,
    }};
    ...
    if (!ok) {
        std::cerr << "drum floor tom CLAP port/parameter contract failed\n";
    ...
    if (!processing) {
        std::cerr << "could not activate drum floor tom plugin\n";
    ...
    resetOnAudioThread(plugin);
    EventList delayedRim;
    constexpr uint32_t kRimTime = 151u;
    const bool addedRim = delayedRim.addNote(37, 1.0, kRimTime);
    double rimBefore = 0.0;
    double rimAfter = 0.0;
    const bool rimTiming = addedRim
        && exactTimedStart(plugin, audio, &delayedRim.input, kRimTime,
            rimBefore, rimAfter);
    if (!rimTiming) {
        std::cerr << "sample-accurate rim start failed: "
                  << rimBefore << " / " << rimAfter << "\n";
    }
    ok = ok && rimTiming;
    ...
    const bool monoConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBody, 0.8 }, { kRing, 0.7 }, { kDamping, 0.2 },
        { kRimLevel, 0.9 }, { kRimCharacter, 0.8 },
        { kStickLevel, 0.8 }, { kStereoWidth, 0.0 },
    ...
    const bool wideConfigured = flushParamsOnAudioThread(plugin, params, {
        ...
        { kRimCharacter, 0.8 }, { kRimDecay, 0.12 },
        { kStickLevel, 1.0 }, { kStickTone, 0.8 },
        { kStickDecay, 8.0 },
        { kStereoWidth, 1.0 },
    });
    resetOnAudioThread(plugin);
    ...
    // Head and RimStick are distinct articulations. Conventional note 37 and
    // octave-map note 55 must normalize to the same canonical rim pitch,
    // while 53 remains the lower rim voice.
    const bool articulationConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kTune, 72.0 }, { kNoteTracking, 1.0 },
            { kPitchDrop, 4.0 }, { kSweepTime, 30.0 },
            { kBody, 0.8 }, { kRing, 0.5 }, { kBodyDecay, 0.7 },
            { kPunch, 0.8 }, { kDamping, 0.3 },
            { kRimLevel, 0.9 }, { kRimCharacter, 0.65 },
            { kRimDecay, 0.09 }, { kStickLevel, 0.8 },
            { kStickTone, 0.7 }, { kStickDecay, 6.0 },
    ...
    AudioBlock headBlock;
    AudioBlock rim37Block;
    AudioBlock rim53Block;
    AudioBlock rim55Block;
    const bool articulationRouting = articulationConfigured
        && renderFirstBlock(43, headBlock)
        && renderFirstBlock(37, rim37Block)
        && renderFirstBlock(53, rim53Block)
        && renderFirstBlock(55, rim55Block)
        && !blocksMatchExactly(headBlock, rim37Block)
        && blocksMatchExactly(rim37Block, rim55Block)
        && !blocksMatchExactly(rim37Block, rim53Block);
    if (!articulationRouting) {
        std::cerr << "head/rim articulation or MIDI normalization failed\n";
    }
    ok = ok && articulationRouting;

    AudioBlock midiHeadBlock;
    AudioBlock midiRim37Block;
    AudioBlock midiRim53Block;
    AudioBlock midiRim55Block;
    const bool rawMidiRouting = renderFirstMidiBlock(43u, midiHeadBlock)
        && renderFirstMidiBlock(37u, midiRim37Block)
        && renderFirstMidiBlock(53u, midiRim53Block)
        && renderFirstMidiBlock(55u, midiRim55Block)
        && blocksMatchExactly(headBlock, midiHeadBlock)
        && blocksMatchExactly(rim37Block, midiRim37Block)
        && blocksMatchExactly(rim53Block, midiRim53Block)
        && blocksMatchExactly(rim55Block, midiRim55Block);
    if (!rawMidiRouting) {
        std::cerr << "raw MIDI head/rim routing diverged from CLAP notes\n";
    }
    ok = ok && rawMidiRouting;

    // Use chromatic head notes that do not overlap the dedicated rim keys
    // 53 and 55.
    const auto lowNote = renderTonalStrike(
        plugin, params, audio, kBaseMidiNote + 1, 1.0);
    const auto highNote = renderTonalStrike(
        plugin, params, audio, kBaseMidiNote + 13, 1.0);
    const auto softNote = renderTonalStrike(
        plugin, params, audio, kBaseMidiNote + 1, 0.2);
    ...
    const bool noteResponse = lowNote.finite && highNote.finite
        && softNote.finite
        && lowNote.frequency >= 60.0 && lowNote.frequency <= 90.0
    ...
    const bool stateConfigured = flushParamsOnAudioThread(plugin, params, {
        { kTune, 127.5 }, { kNoteTracking, 0.0 },
        { kPitchDrop, 0.0 }, { kSweepTime, 1.0 },
        { kShellSpread, 0.0 }, { kBody, 0.0 }, { kRing, 0.0 },
        { kBodyDecay, 1.25 }, { kPunch, 0.8 },
        { kDamping, 0.2 }, { kRimLevel, 1.0 },
        { kRimCharacter, 0.48 }, { kRimDecay, 0.14 },
        { kStickLevel, 0.8 }, { kStickTone, 0.65 },
        { kStickDecay, 8.0 }, { kDrive, 0.0 },
    ...
    const bool stateMutated = flushParamsWhileInactive(plugin, params, {
        { kTune, 91.0 }, { kBodyDecay, 0.1 }, { kBias, 0.2 },
        { kRimLevel, 0.0 }, { kRimDecay, 0.02 },
        { kStickLevel, 0.0 }, { kStickDecay, 0.5 },
    ...
    const bool restored = saved && mutationConsumed && stagedTriggerRequested
        ...
        && getParam(plugin, params, kTune, 127.5)
        && getParam(plugin, params, kBodyDecay, 1.25)
        && getParam(plugin, params, kRimLevel, 1.0)
        && getParam(plugin, params, kRimDecay, 0.14)
    ...
    if (!parameterTriggered) {
        std::cerr << "Head Trigger did not request/process a floor tom strike\n";
    }
    const AudioBlock parameterHeadBlock = audio;
    resetOnAudioThread(plugin);
    const bool rimTriggerAdded = flushParamsOnAudioThread(plugin, params, {
        { kTrigger, 2.0 },
    });
    audio.clear();
    const auto rimTriggerStatus = runBlock(plugin, audio);
    const bool parameterRimTriggered = rimTriggerAdded
        && rimTriggerStatus != CLAP_PROCESS_ERROR && finiteBlock(audio)
        && blockEnergy(audio) > 1.0e-9
        && !blocksMatchExactly(parameterHeadBlock, audio)
        && getParam(plugin, params, kTrigger, 0.0);
    if (!parameterRimTriggered) {
        std::cerr << "Rim Trigger did not produce a distinct floor tom strike\n";
    }
    ok = ok && parameterTriggered && parameterRimTriggered;
    ...
    const bool longVoiceConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kSweepTime, 300.0 }, { kBody, 1.0 }, { kRing, 1.0 },
            { kBodyDecay, 4.0 }, { kPunch, 1.0 }, { kDamping, 0.0 },
            { kRimLevel, 1.0 }, { kRimDecay, 0.35 },
            { kStickLevel, 1.0 }, { kStickDecay, 40.0 },
            { kDrive, 1.0 },
        });
    resetOnAudioThread(plugin);
    ...
    const bool shortenedActiveVoice = flushParamsOnAudioThread(
        plugin, params, {
            ...
            { kBodyDecay, 0.04 }, { kPunch, 0.0 }, { kDamping, 1.0 },
            { kRimLevel, 0.0 }, { kRimDecay, 0.02 },
            { kStickLevel, 0.0 }, { kStickDecay, 0.5 },
            { kDrive, 0.0 },
        });
    ...
    const bool activeTailLatched = longVoiceConfigured && addedLongVoice
        ...
        && configuredLongVoiceTail
            >= static_cast<uint32_t>(kSampleRate * 13.8)
    ...
    const bool longRimConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBodyDecay, 0.04 }, { kRing, 1.0 }, { kDamping, 1.0 },
        { kRimLevel, 1.0 }, { kRimDecay, 0.35 },
        { kStickLevel, 0.0 }, { kStickDecay, 0.5 },
    });
    const uint32_t longRimTail = tail->get(plugin);
    const bool rimTailCovered = longRimConfigured
        && longRimTail >= static_cast<uint32_t>(kSampleRate * 2.02)
        && longRimTail <= static_cast<uint32_t>(kSampleRate * 2.08);
    if (!rimTailCovered) {
    ...
    const bool tightDriveBase = flushParamsOnAudioThread(plugin, params, {
        { kSweepTime, 1.0 }, { kBody, 0.0 }, { kRing, 0.0 },
        { kBodyDecay, 0.04 }, { kPunch, 1.0 }, { kDamping, 1.0 },
        { kRimLevel, 0.0 }, { kRimDecay, 0.02 },
        { kStickLevel, 0.0 }, { kStickTone, 1.0 },
        { kStickDecay, 0.5 }, { kDrive, 0.0 },
    });
    resetOnAudioThread(plugin);
    audio.clear();
    ...
    const bool shortConfigured = flushParamsOnAudioThread(plugin, params, {
        ...
        { kBody, 1.0 }, { kRing, 0.0 }, { kBodyDecay, 0.04 },
        { kPunch, 0.0 }, { kDamping, 1.0 },
        { kRimLevel, 0.0 }, { kRimDecay, 0.02 },
        { kStickLevel, 0.0 }, { kStickDecay, 0.5 },
    ...
    const bool coherenceConfiguredA = flushParamsWhileInactive(
        plugin, params, {
            { kTune, 45.0 }, { kNoteTracking, 0.0 },
            { kPitchDrop, -6.0 }, { kSweepTime, 1.0 },
            { kShellSpread, 0.0 }, { kBody, 1.0 }, { kRing, 0.0 },
            { kBodyDecay, 0.08 }, { kPunch, 0.10 },
            { kDamping, 1.0 }, { kRimLevel, 0.05 },
            { kRimCharacter, 0.0 }, { kRimDecay, 0.02 },
            { kStickLevel, 0.0 }, { kStickTone, 0.0 },
            { kStickDecay, 0.5 },
    ...
    const bool coherenceConfiguredB = flushParamsWhileInactive(
        plugin, params, {
            { kTune, 170.0 }, { kNoteTracking, 1.0 },
            { kPitchDrop, 30.0 }, { kSweepTime, 300.0 },
            { kShellSpread, 1.0 }, { kBody, 0.15 }, { kRing, 1.0 },
            { kBodyDecay, 2.5 }, { kPunch, 1.0 },
            { kDamping, 0.0 }, { kRimLevel, 1.0 },
            { kRimCharacter, 1.0 }, { kRimDecay, 0.35 },
            { kStickLevel, 1.0 }, { kStickTone, 1.0 },
            { kStickDecay, 40.0 },
            { kDrive, 0.0 }, { kBias, 0.0 }, { kCompression, 0.0 },
            { kRateReduction, 0.0 }, { kBitDepth, 0.0 },
            { kReconstruction, 0.0 }, { kCharacterTone, 0.0 },
    ...
    if (!ok) {
        std::cerr << "drum floor tom CLAP smoke failed\n";
        return 1;
    }
    std::cout << "drum floor tom CLAP smoke passed\n";
    return 0;
}
