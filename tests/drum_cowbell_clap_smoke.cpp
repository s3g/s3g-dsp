...
constexpr const char* kPluginId = "org.s3g.s3g-dsp.drum-cowbell";
constexpr const char* kPluginName = "s3g Drum Cowbell 2";
constexpr double kSampleRate = 48000.0;
constexpr uint32_t kFrames = 256u;
constexpr uint32_t kChannels = 2u;
constexpr int16_t kBaseMidiNote = 56;
...
constexpr std::array<ParamSpec, 28u> kParamSpecs {{
    { kTune, "Tune", 180.0, 2400.0, 560.0, 870.0, false },
    { kNoteTracking, "Note Tracking", 0.0, 1.0, 0.65, 0.37, false },
    { kInterval, "Interval", 1.08, 2.60, 1.52, 1.87, false },
    { kDetune, "Detune", 0.0, 1.0, 0.08, 0.41, false },
    { kShape, "Shape", 0.0, 1.0, 0.68, 0.41, false },
    { kAttack, "Attack", 0.0, 1.0, 0.62, 0.63, false },
    { kDecay, "Decay", 0.025, 2.0, 0.24, 0.71, false },
    { kDamping, "Damping", 0.0, 1.0, 0.46, 0.82, false },
    { kBody, "Body", 0.0, 1.0, 0.62, 0.71, false },
    { kBodyDecay, "Body Decay", 0.02, 1.5, 0.18, 0.84, false },
    { kBrightness, "Brightness", 0.0, 1.0, 0.58, 0.38, false },
    { kNoise, "Noise", 0.0, 1.0, 0.16, 0.61, false },
    { kNoiseDecay, "Noise Decay", 0.004, 0.40, 0.035, 0.17, false },
    { kBend, "Bend", -24.0, 24.0, 0.0, -7.0, false },
    { kBendDecay, "Bend Decay", 0.003, 0.50, 0.030, 0.072, false },
    { kStrikeTone, "Strike Tone", 0.0, 1.0, 0.56, 0.35, false },
...
int main(int argc, char** argv)
{
    ...
    if (std::strcmp(argv[2], kPluginId) != 0) {
        std::cerr << "unexpected drum cowbell plugin id: " << argv[2]
                  << "\n";
        return 1;
    }
    ...
    if (binary.empty()) {
        std::cerr << "could not resolve drum cowbell binary\n";
        return 1;
    }
    void* library = dlopen(binary.c_str(), RTLD_LOCAL | RTLD_NOW);
    if (!library) {
        std::cerr << "could not load drum cowbell: " << dlerror() << "\n";
        return 1;
    }
    ...
    if (!entry || !entry->init(binary.c_str())) {
        std::cerr << "invalid drum cowbell CLAP entry\n";
        dlclose(library);
        return 1;
    }
    ...
    if (!descriptor || !descriptor->id || !descriptor->name
        ...
        std::cerr << "unexpected drum cowbell factory or descriptor\n";
        entry->deinit();
        dlclose(library);
        return 1;
    }
    ...
    context.host.name = "s3g drum cowbell smoke";
    ...
    if (!plugin || !plugin->init(plugin)) {
        std::cerr << "could not create drum cowbell plugin\n";
    ...
    if (!ok) {
        std::cerr << "drum cowbell CLAP port/parameter contract failed\n";
    ...
    if (!processing) {
        std::cerr << "could not activate drum cowbell plugin\n";
    ...
    constexpr uint32_t kBendDecaySample = 151u;
    const bool addedFlam = delayedFlam.addNote(57, 1.0, kBendDecaySample);
    double flamBefore = 0.0;
    double flamAfter = 0.0;
    const bool flamTiming = addedFlam
        && exactTimedStart(plugin, audio, &delayedFlam.input,
            kBendDecaySample, flamBefore, flamAfter);
    if (!flamTiming) {
    ...
    const bool monoConfigured = flushParamsOnAudioThread(plugin, params, {
        { kAttack, 0.8 }, { kDecay, 0.7 }, { kBodyDecay, 0.2 },
        { kBrightness, 0.9 }, { kNoise, 0.8 },
        { kBend, 0.5 }, { kStereoWidth, 0.0 },
    ...
    const bool wideConfigured = flushParamsOnAudioThread(plugin, params, {
        { kBody, 0.12 }, { kBrightness, 1.0 },
        { kNoise, 0.8 }, { kNoiseDecay, 0.12 },
        { kBend, 0.4 }, { kBendDecay, 0.062 },
        { kStrikeTone, 0.8 },
        { kStereoWidth, 1.0 },
    });
    resetOnAudioThread(plugin);
    ...
    // Cowbell, muted and high notes are three distinct articulations, and
    ...
    const bool articulationConfigured = flushParamsOnAudioThread(
        plugin, params, {
            { kTune, 560.0 }, { kNoteTracking, 0.35 },
            { kInterval, 1.52 }, { kDetune, 0.32 },
            { kAttack, 0.8 }, { kDecay, 0.5 }, { kDamping, 0.7 },
            { kBody, 0.12 }, { kBodyDecay, 1.4 },
            { kBrightness, 0.28 }, { kNoise, 0.65 },
            { kNoiseDecay, 0.035 }, { kBend, 0.18 },
            { kBendDecay, 0.064 }, { kStrikeTone, 0.6 },
    ...
    const bool articulationRouting = articulationConfigured
        && renderFirstBlock(56, clapBlock)
        && renderFirstBlock(57, flamBlock)
        && renderFirstBlock(58, tightBlock)
    ...
    if (!articulationRouting) {
        std::cerr << "cowbell/muted/high articulation routing failed\n";
    }
    ...
    const bool rawMidiRouting = renderFirstMidiBlock(56u, midiClapBlock)
        && renderFirstMidiBlock(57u, midiFlamBlock)
        && renderFirstMidiBlock(58u, midiTightBlock)
    ...
    if (!rawMidiRouting) {
        std::cerr << "raw MIDI cowbell routing diverged from CLAP notes\n";
    }
    ok = ok && rawMidiRouting;

    // Use the primary articulation across one chromatic octave.
    const auto lowNote = renderTonalStrike(
        plugin, params, audio, 56, 1.0);
    const auto highNote = renderTonalStrike(
        plugin, params, audio, 68, 1.0);
    const auto softNote = renderTonalStrike(
        plugin, params, audio, 56, 0.2);
    ...
    const bool noteResponse = lowNote.finite && highNote.finite
        && softNote.finite
        && lowNote.frequency >= 350.0 && lowNote.frequency <= 1800.0
        && octaveRatio >= 1.70 && octaveRatio <= 2.30
    ...
    const bool stateConfigured = flushParamsOnAudioThread(plugin, params, {
        ...
        { kInterval, 1.87 }, { kDetune, 0.31 },
        { kShape, 0.0 }, { kAttack, 0.0 }, { kDecay, 0.25 },
        { kDamping, 0.85 }, { kBody, 0.12 },
        { kBodyDecay, 0.82 }, { kBrightness, 0.44 },
        { kNoise, 0.48 }, { kNoiseDecay, 0.1264 },
        { kBend, 0.28 }, { kBendDecay, 0.065 },
        { kStrikeTone, 0.8 }, { kDrive, 0.0 },
    ...
    if (saved) {
        std::memcpy(stateHeader.data(), memory.bytes.data(),
            sizeof(stateHeader));
        saved = stateHeader[0u] == 0x57473353u
            && stateHeader[1u] == 2u
            && stateHeader[2u] == kParamSpecs.size() - 1u
            && stateHeader[3u] == 0u;
    }
    const bool stateMutated = flushParamsWhileInactive(plugin, params, {
        { kTune, 910.0 }, { kDamping, 0.1 }, { kBias, 0.2 },
        { kBrightness, 0.08 }, { kNoiseDecay, 0.020 },
        { kBend, 0.02 }, { kStrikeTone, 0.5 },
    ...
    const bool restored = saved && mutationConsumed && stagedTriggerRequested
        ...
        && getParam(plugin, params, kDamping, 0.85)
        && getParam(plugin, params, kBrightness, 0.44)
        && getParam(plugin, params, kNoiseDecay, 0.1264)
    ...
    if (!ok) {
        std::cerr << "drum cowbell CLAP smoke failed\n";
        return 1;
    }
    std::cout << "drum cowbell CLAP smoke passed\n";
    return 0;
}
