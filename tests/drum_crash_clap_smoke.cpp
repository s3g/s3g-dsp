...
constexpr const char* kPluginId = "org.s3g.s3g-dsp.drum-crash";
constexpr const char* kPluginName = "s3g Drum Crash 2";
constexpr double kSampleRate = 48000.0;
constexpr uint32_t kFrames = 256u;
constexpr uint32_t kChannels = 2u;
constexpr int16_t kBaseMidiNote = 49;
...
constexpr std::array<ParamSpec, 28u> kParamSpecs {{
    { kTune, "Tune", 180.0, 2400.0, 720.0, 970.0, false },
    { kNoteTracking, "Note Tracking", 0.0, 1.0, 0.25, 0.37, false },
    { kSize, "Size", 0.0, 1.0, 0.62, 0.46, false },
    { kAlloy, "Alloy", 0.0, 1.0, 0.58, 0.41, false },
    { kSpread, "Spread", 0.0, 1.0, 0.66, 0.41, false },
    { kDensity, "Density", 4.0, 16.0, 12.0, 15.0, true },
    { kBrightness, "Brightness", 0.0, 1.0, 0.68, 0.73, false },
    { kAttack, "Attack", 0.0, 1.0, 0.72, 0.82, false },
    { kDecay, "Decay", 0.08, 8.0, 1.30, 2.7, false },
    { kDamping, "Damping", 0.0, 1.0, 0.44, 0.36, false },
    { kWash, "Wash", 0.0, 1.0, 0.74, 0.88, false },
    { kWashDecay, "Wash Decay", 0.08, 8.0, 1.45, 3.1, false },
    { kBell, "Bell", 0.0, 1.0, 0.20, 0.61, false },
    { kBellTune, "Bell Tune", 350.0, 6000.0, 1680.0, 2470.0, false },
    { kChokeTime, "Choke Time", 0.5, 150.0, 14.0, 42.0, false },
    { kTexture, "Texture", 0.0, 1.0, 0.58, 0.35, false },
    ...
    { kStereoWidth, "Stereo Width", 0.0, 1.0, 0.68, 0.67, false },
    { kVelocitySensitivity, "Velocity Sensitivity", 0.0, 1.0, 0.90, 0.83, false },
    { kOutputGain, "Output Gain", -36.0, 12.0, -11.0, -4.5, false },
...
int main(int argc, char** argv)
{
    ...
    if (std::strcmp(argv[2], kPluginId) != 0) {
        std::cerr << "unexpected drum crash plugin id: " << argv[2]
                  << "\n";
        return 1;
    }
    ...
    if (binary.empty()) {
        std::cerr << "could not resolve drum crash binary\n";
        return 1;
    }
    void* library = dlopen(binary.c_str(), RTLD_LOCAL | RTLD_NOW);
    if (!library) {
        std::cerr << "could not load drum crash: " << dlerror() << "\n";
        return 1;
    }
    ...
    if (!entry || !entry->init(binary.c_str())) {
        std::cerr << "invalid drum crash CLAP entry\n";
        dlclose(library);
        return 1;
    }
    ...
    if (!descriptor || !descriptor->id || !descriptor->name
        ...
        std::cerr << "unexpected drum crash factory or descriptor\n";
        entry->deinit();
        dlclose(library);
        return 1;
    }
    ...
    context.host.name = "s3g drum crash smoke";
    ...
    if (!plugin || !plugin->init(plugin)) {
        std::cerr << "could not create drum crash plugin\n";
    ...
    if (!ok) {
        std::cerr << "drum crash CLAP port/parameter contract failed\n";
    ...
    if (!processing) {
        std::cerr << "could not activate drum crash plugin\n";
    ...
    const bool flamTiming = addedFlam
        && exactTimedStart(plugin, audio, &delayedFlam.input,
            kChokeTimeSample, flamBefore, flamAfter);
    if (!flamTiming) {
    ...
    // Version-1 states contain the original 26 persistent parameters. They
    // must still load, with the newly appended receive channel set to OMNI.
    MemoryState legacyState = memory;
    std::array<uint32_t, 4u> legacyHeader = stateHeader;
    legacyHeader[1u] = 1u;
    legacyHeader[2u] = 26u;
    std::memcpy(legacyState.bytes.data(), legacyHeader.data(),
        sizeof(legacyHeader));
    legacyState.bytes.resize(sizeof(legacyHeader) + 26u * sizeof(double));
    const bool legacyReceiveChanged = flushParamsOnAudioThread(
        plugin, params, { { kMidiReceive, 9.0 } });
    legacyState.offset = 0u;
    clap_istream_t legacyIstream { &legacyState, stateRead };
    const bool legacyLoaded = legacyReceiveChanged
        && state->load(plugin, &legacyIstream)
        && getParam(plugin, params, kMidiReceive, 0.0);
    audio.clear();
    const bool legacyConsumed = legacyLoaded
    ...
    if (!ok) {
        std::cerr << "drum crash CLAP smoke failed\n";
        return 1;
    }
    std::cout << "drum crash CLAP smoke passed\n";
    return 0;
}
