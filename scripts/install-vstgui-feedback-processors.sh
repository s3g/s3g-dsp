...
export S3G_CLAP_MANIFEST="$script_dir/clap-vstgui-feedback-processors.tsv"
...
export S3G_CLAP_RECEIPT="/Users/s3g/Library/Application Support/s3g-dsp/clap-vstgui-feedback-processors-receipt.tsv"

if [[ $# -gt 1 || ( $# -eq 1 && "$1" != --dry-run ) ]]; then
  echo "Usage: bash scripts/install-vstgui-feedback-processors.sh [--dry-run]" >&2
  exit 2
fi
...
backup_run="$(mktemp -d "$S3G_CLAP_BACKUP_ROOT/feedback-processors.XXXXXX")"
while IFS=$'\t' read -r relative canonical plugin_id host_name; do
  [[ -z "$relative" || "$relative" == \#* ]] && continue
