...
export S3G_CLAP_MANIFEST="$script_dir/clap-vstgui-stereo-processors.tsv"
...
export S3G_CLAP_RECEIPT="/Users/s3g/Library/Application Support/s3g-dsp/clap-vstgui-stereo-processors-receipt.tsv"

if [[ $# -gt 1 || ( $# -eq 1 && "$1" != --dry-run ) ]]; then
  echo "Usage: bash scripts/install-vstgui-stereo-processors.sh [--dry-run]" >&2
  exit 2
fi
...
backup_run="$(mktemp -d "$S3G_CLAP_BACKUP_ROOT/stereo-processors.XXXXXX")"
while IFS=$'\t' read -r relative canonical plugin_id host_name; do
  [[ -z "$relative" || "$relative" == \#* ]] && continue
