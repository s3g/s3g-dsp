...
NSAttributedString* helpDocument()
{
    ...
    [document appendAttributedString:[[NSAttributedString alloc]
        initWithString:
            ...
            @"MIDI RECORD  OFF disarms · STEP, LIVE Q, and LIVE MT immediately monitor incoming note-on/off on the selected lane's MIDI channel · STEP writes note/velocity at the cursor, clears MT there, and advances one row · LIVE Q records at the nearest playback row, wrapped inside the selected NOTE/VOL lengths · LIVE MT also writes the measured offset into an available SEQ pair · Live recording preserves lane lengths and moves the cursor highlight to the written NOTE without advancing · Live modes require REAPER playback and pattern transport\n"
            ...
            @"QUICK ENTRY  NOTE X toggles an anchored hit · R writes RPT · K writes KIL · Delete clears the active cell or every cell in a drag selection · [ and ] adjust VOL/V values · M toggles the selected column mute\n"
