...
NSAttributedString* helpDocument()
{
    ...
    appendGuideSection(document, @"TRACKER GRID WORKFLOW", @[
        ...
        @[ @"PHRASES", @"Select 2–64 rows in one Tracker lane, then right-click and choose PHRASE > CAPTURE SELECTION; Control-Shift-P is the shortcut. Referenced Bursts retain their project-wide slot identity. Control-P copies the selected Phrase at the cursor. The Phrase page uses Tracker's own inline NOTE/VOL/SEQ/GATE entry, with VOL shown as normalized 0.000–1.000. The optional Phrase Library BPM field stores a recommended 20–400 BPM authoring tempo; it never changes host or project playback. Drag cells or Shift-click within a column to select a range; Control-Shift-Arrows extends it. Control-A/C/X/V selects all, copies, cuts, and pastes compatible Phrase cells while retaining their row/column shape. Right-click SEQ1/SEQ2 for the action and MIDI CC menu; right-click a CD value for named conditions. PREVIEW CH routes stopped-transport auditioning without changing the destination lane, while the green playhead advances at project BPM. Placement copies cells directly and creates no hidden link." ],
        @[ @"ASSEMBLE", @"Choose a Phrase and press A or APPEND to build a vertical Phrase chain. REPEAT paints 1×–16× copies. Shift-click selects multiple blocks; drag reorders them, Option-drag duplicates them, and Delete removes them. Preview either the selected Phrase or the complete Assembly at project BPM. PLACE IN PATTERN materializes ordinary Tracker cells using Replace or Merge Empty and Extend, Crop, or Wrap fitting. Assemblies of 2–64 rows can be saved into Project Phrases." ],
        @[ @"LANE ORDER", @"Right-click a lane name at the top of Tracker for Move Lane Left, Right, or To. The complete lane moves with its column cycles, defaults, aliases, MIDI channel, record arm, and SEQ data; Song mute assignments follow the musical lane. The selection transform called Swap Selected Cells affects only the selected cell rectangle." ],
    ...
    appendGuideSection(document, @"TRANSPORT + SONG", @[
        @[ @"TRANSPORT", @"Tempo follows REAPER. The labeled BPM menu selects 1/4×, 1/2×, 2/3×, 1×, 3/2×, 2×, or 4×. BPM, Swing, Gate, Loop In/Out, Record mode, and Record lane use centered labels above one aligned value row, with separators between groups; the strip scrolls in a narrow host window. Space plays/pauses; Shift-Space toggles loop. SYNC ALL restarts every lane and column at row 1 without moving REAPER." ],
        ...
        @[ @"SONG BPM ×", @"Each Song row multiplies the observed REAPER BPM by 1/4×, 1/2×, 2/3×, 1×, 3/2×, 2×, or 4×. This row ratio also composes with the main Tracker BPM multiplier and changes at the Song-row boundary." ],
        @[ @"SONG ROW EDIT", @"ADD inserts after the selection. DUP makes an exact copy. The ↑ and ↓ controls move the selected row. Drag the grip beside a row number to move it, or Option-drag to duplicate it. Stable row identity keeps live reordering aligned with the row that is actually sounding. Arrangement edits remain available while REAPER runs; Tracker hands the updated arrangement to playback safely at the next Song-row boundary without creating a selected-row queue." ],
