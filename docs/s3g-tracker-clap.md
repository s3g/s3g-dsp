...
- editable polymetric NOTE, INS, VOL, and two FX action/value column pairs;
- up to 32 tracks, independent column lengths/phase/stride/direction/mute;
- pattern bank, Song window, prepared pattern-boundary changes, and global
  tracker loop;
...
- mixer, rhythm geometry, envelope/device strip, instrument rack, and editors;
...
Eight active MIDI rack instruments map to eight CLAP note-output buses. Each
instrument owns its bus and one-based MIDI channel; tracker INS cells can
change that indexed instrument per row. REAPER decides which downstream track
or plug-in receives each bus. The tracker exposes no audio ports and never
opens a global macOS MIDI destination.
...
The membrane, DaisySP drum, and sampler editor/state models remain visible so
the tracker project and instrument-index workflow are preserved. In this
MIDI-focused CLAP, note events from those indexed rows are emitted to REAPER;
sound is produced by downstream s3g-dsp drum/sampler CLAP instruments or other
plug-ins. Their synthesis code is not embedded in the tracker.
