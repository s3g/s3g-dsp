...
- Adds **s3g Sample Grains 2** and **s3g Sample Grains 32**, a non-ambisonic four-file granular family built on the same lane waveform, breakpoint path, MIDI, and Project/Link/Embed contracts. Scan, Freeze, Cloud, and Slice choose source time; Parzen, Sine, Hann, Triangle, and Gaussian shape each grain; and a separate Regular/Scatter event clock applies Ordinary, loudness-Sorter, Stutter, progressively accelerating Shrink, or source-time-related Doublets. Its gap-free 1280-by-908 editor gives every slider a full-width track at a consistent 24-pixel row pitch: Source Scan, Grain Source, and Path fill the right column, Grain Process sits beneath the visualization, and Voice or Routing spans both columns as the footer. The editor omits Lanes' selected-lane Timing/Stretch toolbox and the former simulated event graph. Instead, the DSP publishes the active grains themselves over a darker waveform: faint fills accumulate with overlap, elapsed and remaining contours use distinct gray levels, contour thickness follows lane weight, luminance follows current windowed gain, and a white dark-haloed dot shows phase. Matching event rings animate in the read-head path scope. LOOP JOIN now has a fully contained transport row, and the Voice/Pitch/MIDI footer buttons use a lighter neutral fill. Those operation names deliberately parallel Slicer's vocabulary, while remaining a distinct live grain scheduler rather than its slice-rearrangement Mutate engine. Sample Grains 32 either preserves a multichannel source field or explicitly folds and allocates each grain across a selected 2–32-channel destination width; its compact Routing tab uses the same categorical active-output control as Lanes 32, while the stereo edition omits routing controls entirely.
