...
- Reworks **s3g Matrix Upmix 64** into an editable input-by-output gain matrix for expanding stereo, quad, octophonic, and custom layouts into larger speaker arrays. Factory mappings, spatial Auto Fill recipes, per-input normalization, output limiting, custom AED coordinates, and a detachable Tier Rings layout view replace the earlier fixed Format Upscale behavior while retaining its stable CLAP identifier.
- Rebuilds **s3g Sample Circulator 2** from the CRCLTR engine as a file-or-capture stereo dual-loop instrument. Two waveform slots provide replace, overdub, and punch capture, queued loop-window edits, independent rate/direction/join controls, nine crossfade shapes, eight motion models, MIDI capture buttons, and self-contained embedded loop storage while preserving the original plug-in identity and first ten parameter identifiers.
- Adds **s3g Effect Delay Field 16**, consolidating the former Shard Scatter, Orbit Delay, and Cascade Taps plug-ins with a new Iterate model. Its stable sixteen-channel port can fold the complete delay field to stereo, quad, or eight-channel rings, rotate the active output field, and switch models with a click-safe handoff.
- Consolidates twelve separate Array HPF, Delay, and Trim bundles into **s3g Array Calibrate 16/26/32/64**, with the three calibration stages in fixed signal order and independently editable per-speaker controls.
...
- Corrects Cartography's world-to-AED direction mapping and Ambi Decoder Head's listener-relative pitch rotation. Shared Ambisonic orientation guides and consistent Top, Side, and 3/4 cameras now keep the displayed axes aligned across the encoder family.
- Sample Wavesets uses a short old/new handoff when Mono, Legato, or voice stealing replaces a sounding route. Delay Field's Cascade engine also keeps wrapped interpolation positions inside its delay buffer at the negative boundary.
...
Apple silicon macOS CLAP collection and optional No Input Mixer standalone pre-release, released August 21, 2026.
...
- Added Sample Player 2/16, Sample Doubles 2, Sample Wavesets 2/32, and the revised Sample Slicer 16, together with their shared waveform, MIDI, editor, routing, and saved-source conventions.
- Extended Tracker step recording and editing, Processor Stack score and host-sync behavior, Membrane Kick MIDI handling, and shared direct/quad/stereo ring-output rendering.
- Passed the complete Release, standalone, sanitizer, CLAP validation, callback-allocation, realtime, documentation, and packaging rehearsal gates recorded in the 0.8 release checklist.
