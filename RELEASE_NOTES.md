...
- `s3g-dsp-macos-clap-0.8.0-pre.zip` — 130 CLAP bundles containing 136 plug-ins for REAPER
...
- Adds **s3g Sample Lanes 2** and **s3g Sample Lanes 32**, four-file loop instruments with one normalized two-dimensional read head. Independently sized sources occupy four visible lanes; shaped Down, Up, Triangle, Sine, stepped, seeded-random, and editable manual breakpoint paths either equal-power crossfade or click-smooth jump between them. Random playback uses the exact eight-step path drawn in the scope, while Manual supports click-to-add, drag-to-shape, and right-click deletion of up to 16 state-persistent points. Each lane adds independent Speed, pitch-preserving Stretch, and visibly rotating signed wraparound timing Nudge. The wide two-column editor moves timing, transport, and path toolboxes beside the source displays and reduces the native height to 724 pixels. The 32-channel edition accepts one-to-sixteen-channel files and separates channel-locked **Preserve Field** routing from explicit mono/stereo object folding and shared 2–32-channel distribution. A Motion-style Routing tab now keeps this topology off the normal voice page and exposes active output count as a categorical 02–32 menu.
- Adds **s3g Sample Grains 2** and **s3g Sample Grains 32**, a non-ambisonic four-file granular family built on the same lane waveform, breakpoint path, MIDI, and Project/Link/Embed contracts. Scan, Freeze, Cloud, and Slice choose source time; Parzen, Sine, Hann, Triangle, and Gaussian shape each grain; and a separate Regular/Scatter event clock applies Ordinary, loudness-Sorter, Stutter, progressively accelerating Shrink, or source-time-related Doublets. Its gap-free 1280-by-932 editor gives every slider a full-width track at a consistent 24-pixel row pitch: Source Scan, Grain Source, and Path fill the right column, Grain Process sits beneath the visualization, and Voice or Routing spans both columns as the footer. The editor omits Lanes' selected-lane Timing/Stretch toolbox and the former simulated event graph. Instead, the DSP publishes the active grains themselves over a darker waveform: faint fills accumulate with overlap, elapsed and remaining contours use distinct gray levels, contour thickness follows lane weight, luminance follows current windowed gain, and a white dark-haloed dot shows phase. Matching event rings animate in the read-head path scope. LOOP JOIN now has a fully contained transport row, and the Voice/Pitch/MIDI footer buttons use a lighter neutral fill. Sample Grains 2 adds Preserve Origins, Mono Sum, Left, Right, Mid, and Side interpretation, linked or independent stereo grain trajectories, and per-grain Mono Spread; its MIDI voice selector is now labelled Polyphony, and it neither publishes nor honors the 32-channel allocator controls. Amount, Regions, and the other sliders now use the exact visible track for both hit-testing and continued dragging, including the wide Grain Process panel. Those operation names deliberately parallel Slicer's vocabulary, while remaining a distinct live grain scheduler rather than its slice-rearrangement Mutate engine. Sample Grains 32 either preserves a multichannel source field or explicitly folds and allocates each grain across a selected 2–32-channel destination width; its compact Routing tab uses the same categorical active-output control as Lanes 32.
...
- The canonical manifest freezes 130 bundles, including 128 non-NIM bundles, and packaging verifies all 136 runtime plug-in descriptors rather than inferring descriptor count from bundle count.
...
- Documentation adds complete Sample Player, Sample Doubles, Sample Slicer, Sample Wavesets, Sample Motion, Sample Lanes, and Sample Grains guides and current native captures, reorganizes the Sample-family and CRCLTR navigation, and updates the Tracker, Processor Stack, multichannel-output, and No Input Mixer material. The static audit checks 128 HTML pages and 4,973 local references.
